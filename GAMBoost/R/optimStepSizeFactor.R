optimStepSizeFactor <- function(x=NULL,y,x.linear=NULL,direction=c("down","up","both"),start.stepsize=0.1,iter.max=10,
                                constant.cv.res=NULL,parallel=FALSE,trace=FALSE,...) 
{
    ...
    while (i <= length(factor.list) && iter.count < iter.max) {
        ...
        if (factor.list[i] == 1 && !is.null(constant.cv.res)) {
            ...
            cv.res.act.path <- cv.GAMBoost(x=x,y=y,x.linear=x.linear,stepsize.factor.linear=factor.list[i],trace=trace,
                                           just.criterion=TRUE,parallel=parallel,upload.x=FALSE,folds=folds,...)
        }

        critmat <- rbind(critmat,-0.5*cv.res.act.path$criterion)
    ...
    list(factor.list=factor.list,
         ...
         optimal.step=which.max(critmat[optimal.factor.index,]))
}
