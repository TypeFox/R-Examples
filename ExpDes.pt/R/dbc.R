dbc <-
function(trat, bloco, resp, quali=TRUE, mcomp='tukey', sigT=0.05, sigF=0.05) {
...
#                     else dunnett(resp,Trat,tab[[1]][3,1],tab[[1]][3,2],cont=cont,proc="dnt",alpha=sigT)}
#  if(mcomp=="sddnt"){if(length(cont)==0) stop('Informe o nome do tratamento controle!')
#                     else dunnett(resp,Trat,tab[[1]][3,1],tab[[1]][3,2],cont=cont,proc="sddnt",alpha=sigT)}
                     }

else reg<-reg.poly(resp, trat, tab[[1]][3,1], tab[[1]][3,2], tab[[1]][1,1], tab[[1]][1,2])
