fat2.ad.dbc <-
function(fator1, fator2, bloco, resp, respAd, quali=c(TRUE,TRUE), mcomp='tukey', 
...
"QM"=c(round(c(QMB,QMa,QMb,QMab,QMad,QME,QMT),5)),
...
#                    else dunnett(resp,fatores[,i],anavaT[6,1],anavaT[6,2],cont=cont,proc="dnt",alpha=sigT)}
# if(mcomp=="sddnt"){if(length(cont)==0) stop('Informe o nome do tratamento controle!')
#                    else dunnett(resp,fatores[,i],anavaT[6,1],anavaT[6,2],cont=cont,proc="sddnt",alpha=sigT)}
...
"QM"=c(round(c(QMB,QMb,QMf1,QMad,QME,QMT),5)),
...
"QM"=c(round(c(QMB,QMa,QMf2,QMad,QME,QMT),5)),
