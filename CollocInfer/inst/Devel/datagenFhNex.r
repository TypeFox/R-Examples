...
for (i in 2:40001)
{ z[i,1]=z[i-1,1]+.0005*3*(z[i-1,1]- z[i-1,1]^3/3+z[i-1,2])+ .011*w[i-1,1]
  z[i,2]=z[i-1,2]-.0005*(z[i-1,1]-.2+.2* z[i-1,2])/3+ .011*w[i-1,2]
}
...
profile.obj = sse.setup(pars=pars,fn=make.fhn(),lambda=10000,times=t,fd.obj=DEfd)
lik = profile.obj$lik
...
Ires1	= Smooth.sse(make.fhn(),data,t,pars=spars,coefs,bbasis,lambda=lambda,
  in.meth='nlminb',control.in=control.in)
  

Ores1 = Profile.sse(make.fhn(),data=data,times=t,pars=spars,coefs=coefs,
...
Ores2 = outeropt(data=data,times=t,pars=pars,coefs=coefs,lik=lik,proc=proc,
  in.meth="nlminb",out.meth="nls",control.in=control.in,control.out=control.out)
...
Ires1	= Smooth.sse(make.fhn()$fn,data,t,pars=spars,fd.obj=DEfd,lambda=lambda,
  in.meth='nlminb',control.in=control.in)
  

Ores1 = Profile.sse(make.fhn()$fn,data=data,times=t,pars=spars,fd.obj=DEfd,
