...
Smooth.sse <- function(fn, data, times, pars, coefs=NULL, basisvals=NULL,
                       lambda,fd.obj=NULL,more=NULL,weights=NULL,
	                     quadrature=NULL, in.meth='nlminb', control.in, eps=1e-6,
                       pos=0, discrete=0, names=NULL)
{

    profile.obj = sse.setup(pars, coefs, fn, basisvals, lambda,fd.obj, more,
    ...
    if(!is.null(fd.obj)){
      ncoefs = array(ncoefs,c(row(ncoefs)/dims[2],dims[2],dims[3]))
      fd.obj = fd(ncoefs,fd.obj$basis)
      return( list(fd=fd.obj,lik=lik,proc=proc,res=Ires) )
    }
    else{
      return( list(coefs=ncoefs,lik=lik,proc=proc,res=Ires,data=data,
                   times=times) )
    }
}
...
Profile.sse <- function(fn,data,times,pars,coefs=NULL,basisvals=NULL,lambda,
...
sse.setup = function(pars, coefs=NULL, fn, basisvals=NULL, lambda, fd.obj=NULL,
