...
{ 
  ...
  g = as.vector( t(bvals$bvals)%*%diag(more$weights)%*%g1 + t(bvals$dbvals)%*%diag(more$weights)%*%g2 )
...
{ 
  ...
  gg = more$dfdp(ddevals,more$qpts,devals,pars,more$more)

  g = rep(0,dim(g)[3])
  for(i in 1:length(g)){ g[i] = sum(g*more$weights) }
  return(g)
}
...
#  H = array(0,c(rep(dim(bvals$bvals)[2],2),rep(dim(devals)[2],2)))
 ...
 H = list(len=dim(bvals$bvals)[2])
  ...
  H[[i]] = list(len=dim(devals))
    for(j in 1:dim(devals)[2]){
        H[[i]][[j]] = t(bvals$bvals)%*%diag(H1[,i,j])%*%bvals$bvals +
            t(bvals$bvals)%*%diag(more$weights)%*%diag(H2[,i,j])%*%diag(more$weights)%*%bvals$dbvals +
            t(bvals$dbvals)%*%diag(more$weights)%*%diag(H2[,j,i])%*%diag(more$weights)%*%bvals$bvals +
            t(bvals$dbvals)%*%diag(more$weights)%*%diag(H3[,i,j])%*%diag(more$weights)%*%bvals$dbvals
    }
  }
...
{ 
  ...
  for(i in 1:length(pars)){
	  H = cbind(H,as.vector(t(bvals$bvals)%*%H1[,,i] +
			t(bvals$dbvals)%*%diag(more$weights)%*%H2[,,i]))
  }
