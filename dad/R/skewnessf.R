skewnessf <- function(fold, na.rm = FALSE) {
  # 'mean' method for objects of class 'folder': colMeans per data frame.
  
  # Check the arguments
  if (!attr(fold, "same.cols"))
    stop("'skewessf' cannot be applied to a folder with different column names.")
  ...
  return(lapply(fold.num, function(x) apply(x, 2, skewness, na.rm = na.rm)))
}