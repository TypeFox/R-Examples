...
function(x)  {
  ...
  nbL<-by(x,INDICES=lot,FUN=length);
  wL<-bandwidth.parameter(1,nbL)
  ...
  W[1, 1] = l2d.kgw.u(xi, varLwL[[1]], xi, varLwL[[1]])
  for (i in 2:nb.groups)
    {xi = x[lot == levels(lot)[i]]
    W[i, i] = l2d.kgw.u(xi, varLwL[[i]], xi, varLwL[[i]])
    for (j in 1:(i-1))
      {xj = x[lot == levels(lot)[j]]
      W[i, j] = W[j, i] = l2d.kgw.u(xi, varLwL[[i]], xj, varLwL[[j]])
    }
  }
  W
}
