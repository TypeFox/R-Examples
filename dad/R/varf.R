varf <- function(fold, use = "everything") {
  # Objects of class 'folder': variance matrix per data frame.
  # - fold: object of class 'folder'.
  # - use:  see 'var()'. Default: "everything".
  
  # Check the arguments
  if (!attr(fold, "same.cols"))
    stop("'varf' cannot be applied to a folder with different column names.")
  ...
  return(lapply(fold.num, var, use = use))
}
