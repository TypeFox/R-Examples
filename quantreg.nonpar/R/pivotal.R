                    ...
                    uniform=F, se="unconditional", average=T, nderivs=1, method="fn")
{
  ...
  if (average==T & se=="unconditional") { # generate estimate of standard error of the mean (part of unconditional standard error estimation)
    if (basis.type=="fda") {
  ...
  pivot	<- list(qfits=NULL, point.est=NULL, var.unique=NULL, CI=NULL, CI.oneSided=NULL, std.error=NULL, pvalues=NULL, load=NULL) # set up output vector
  pivot$qfits <- vector("list",length.taus) # quantile regression fits
  
  U		<- matrix(runif(nObs * B), nrow = nObs, ncol = B) # uniform random matrix  
  Pstat <- array(0,dim=c(length.taus, ncol=B, loadLength)) # pivotal statistic array
  Hinvs <- vector("list",length.taus) # inverse hessians for each fit
  err<-F
  for (i in 1:length.taus) {
    pivot$qfits[[i]]<-tryCatch(expr=rq(f,tau=taus[i],data,method=method),error=function(e) {
      err<-T
      fit<-rq(f,tau=taus[i],data)
      return(fit)
    })
    Hinvs[[i]] <- summary.rq(pivot$qfits[[i]], se= "ker", covariance = T)$Hinv
  }
  if (err==T) {
    message("Warning: rq method chosen produced an error; using method=br when necessary")
  }
  for (j in 1:loadLength) { 
    for (i in 1:length.taus) { # generate statistics
     Pstat[i, ,j]  <- load[j,] %*%  Hinvs[[i]] %*% t(modelMatrix) %*% ((U <= taus[i]) - taus[i])                           
    }      
  }  
  ...
  for (i in 1:length.taus){ # predict fitted values, evaluated at loading vector
    fittedValues[,i] <- load %*% pivot$qfits[[i]]$coef
  }
  ...
  for (i in 1:length.taus){ # generate estimates of standard errors
    ...
    Covs[i, ,] <- summary.rq(pivot$qfits[[i]], se = "ker", covariance = T)$cov
    for (j in 1:loadLength){
      if (se=="unconditional") {
        stdError[j,i] <- sqrt((t(load[j,]) %*% Covs[i, ,] %*% load[j,]) + t(pivot$qfits[[i]]$coef) %*% covMat %*% pivot$qfits[[i]]$coef / nObs)
      } else { 
    ...
    for (j in 1:loadLength){ # generate t statistics
      ...
      Tn[j,i, ] <- abs(Pstat[i, ,j] / tDenom[j,i])
      Tn.oneSided[j,i, ,1] <- Pstat[i, ,j] / tDenom[j,i]
      Tn.oneSided[j,i, ,2] <- Pstat[i, ,j] / tDenom[j,i]
    }
  }
  ...
  k.star.onesided <- apply((Pstat-aperm(array(fittedValues,dim=c(loadLength,length.taus,B)),perm=c(2,3,1)))/aperm(array(stdError,dim=c(loadLength,length.taus,B)),perm=c(2,3,1)), c(1,2), max, na.rm=T)
  ...
  k.star.twosided <- apply(abs((Pstat-aperm(array(fittedValues,dim=c(loadLength,length.taus,B)),perm=c(2,3,1)))/aperm(array(stdError,dim=c(loadLength,length.taus,B)),perm=c(2,3,1))), c(1,2), max, na.rm=T)
  pValue.twosided <- mean(k.star.twosided >= k.twosided)
  
  pvalues <- c(pValue.onesided.neg, pValue.onesided.pos, pValue.twosided)
  
  pivot$var.unique <- z.unique
  
  pivot$point.est <- matrix(NA, nrow=loadLength, ncol=length.taus) 
  pivot$point.est <- fittedValues
  
  pivot$CI <- CI
  pivot$CI.oneSided <- CI.oneSided
  pivot$std.error <- stdError
  pivot$pvalues <- pvalues
  pivot$load <- load
