                       ...
                       uniform=F, average=T, nderivs=1, method="fn")
{
  ...
  gboot	<- list(qfits=NULL, point.est=NULL, var.unique=NULL, CI=NULL, CI.oneSided=NULL, std.error=NULL, pvalues=NULL, load=NULL) # set up output vector

  gboot$qfits <- vector("list",length.taus)
  
  GBstat <- NULL
  
  mat.qfits <- matrix(NA, ncol=length.taus, nrow=nVars.withNonpar)
  
  bigmats.qfits <- vector("list",length.taus)

  err<-F
  for (i in 1:length.taus) {
    gboot$qfits[[i]]<-tryCatch(expr=rq(f,tau=taus[i],data,method=method),error=function(e) {
      err<-T
      fit<-rq(f,tau=taus[i],data)
      return(fit)
    })
    mat.qfits[,i] <- gboot$qfits[[i]]$coef
    bigmats.qfits[[i]] <- matrix(mat.qfits[,i], nrow=length(mat.qfits[,i]),ncol=B)
  }
  ...
  GBstat<- array(0,dim=c(length.taus, B, loadLength))
  GBvar.incomplete <- array(0,dim=c(length.taus,B,loadLength))
  variances <- matrix(0, nrow=loadLength, ncol=length.taus )
  rqBootCoefs <- matrix(NA, nrow=length.taus*B, ncol=nVars.withNonpar)
  for (i in 1:length.taus) {
    rq.temp <- boot.rq(modelMatrix, Y, tau = taus[i], R = B, bsmethod = "pwy")
    rqBootCoefs[((i-1)*B+1):(i*B),]<-rq.temp
  }
  for (j in 1:loadLength) {
    for (i in  1:length.taus){
      GBstat[i, ,j]  <- t(load[j,] %*% (t(rqBootCoefs[(1+(i-1)*B):(i*B), ])- bigmats.qfits[[i]]))
    }
  } 

  for (j in 1:loadLength) {
    variances[j,] <- apply(GBstat[, ,j] + matrix(t(load[j,] %*% mat.qfits),nrow=length.taus,ncol=B), 1, "var", na.rm=T)
  }
  ...
  for (i in 1:length.taus){ # predict fitted values, evaluated at loading vector
    fittedValues[,i] <- load %*% gboot$qfits[[i]]$coef
  }
  ...
  for (i in 1:length.taus){ # generate estimates of standard errors
    ...
    for (j in 1:loadLength){ # generate t statistics
      ...
      Tn[j,i, ] <- abs(GBstat[i, ,j] / tDenom[j,i])
      Tn.oneSided[j,i, ,1] <- GBstat[i, ,j] / tDenom[j,i]
      Tn.oneSided[j,i, ,2] <- GBstat[i, ,j] / tDenom[j,i]
    }
  }
  ...
  k.star.onesided <- apply((GBstat-aperm(array(fittedValues,dim=c(loadLength,length.taus,B)),perm=c(2,3,1)))/aperm(array(stdError,dim=c(loadLength,length.taus,B)),perm=c(2,3,1)), c(1,2), max, na.rm=T)
  ...
  k.star.twosided <- apply(abs((GBstat-aperm(array(fittedValues,dim=c(loadLength,length.taus,B)),perm=c(2,3,1)))/aperm(array(stdError,dim=c(loadLength,length.taus,B)),perm=c(2,3,1))), c(1,2), max, na.rm=T)
  pValue.twosided <- mean(k.star.twosided >= k.twosided)
  
  pvalues <- c(pValue.onesided.neg, pValue.onesided.pos, pValue.twosided)
  
  gboot$var.unique <- z.unique
  
  gboot$point.est <- matrix(NA, nrow=loadLength, ncol=length.taus) 
  gboot$point.est <- fittedValues
  
  gboot$CI <- CI
  gboot$CI.oneSided <- CI.oneSided
  gboot$std.error <- stdError
  gboot$pvalues <- pvalues
  gboot$load <- load
