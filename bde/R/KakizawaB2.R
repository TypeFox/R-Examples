#************************************************************************
# Kakizawa's Bernstein Polynomial approximation (B2 estimator) as 
# described in Kakizawa's 2004 paper:
#
#	@article{Kakizawa04,
...
setMethod(
  ...
  definition = function(x,values,scaled = FALSE) {    
	...
	aux.pointsForEstimation <- matrix(rep(j / .Object@m, x.new.length), ncol = x.new.length)
	# The estimated density value at point j/m
...
kakizawaB2 <- function(dataPoints,estimator=NULL,m=round(length(dataPoints)^(2/5)),  dataPointsCache=NULL, lower.limit=0,upper.limit=1){
  #cat("~~~~~~ KakizawaB2: constructor ~~~~~~\n")
 ...
 if(is.null(dataPointsCache)){
  ...
  polinomialModel <- new(Class="KakizawaB2",dataPoints = dataPoints.scaled, m = m, dataPointsCache = dataPointsCache.scaled,
