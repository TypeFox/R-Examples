#************************************************************************
#  Jone's nonnegative boundary correction for Muller91's Boundary Kernel
#  Density estimator
#
# @article{Jones1996,
...
setClass(
  ...
  contains = "Muller91BoundaryKernel"
)


setMethod(
  f = "density", 
  signature = "JonesCorrectionMuller91BoundaryKernel",
  definition = function(x,values,scaled = FALSE) {    
    ...
    if(x.new.length > 0){
      ...
      # the density value of the Muller91BoundaryKernel
...
jonesCorrectionMuller91BoundaryKernel <- function(dataPoints, mu=1, b=length(dataPoints)^(-2/5), dataPointsCache=NULL, lower.limit=0,upper.limit=1){
  #cat("~~~~~~ Jones Correction for Muller91 Boundary Kernel: constructor ~~~~~~\n")
  ...
  kernel <- new(Class="JonesCorrectionMuller91BoundaryKernel",dataPoints = dataPoints.scaled, b = b, dataPointsCache = dataPointsCache.scaled, mu = mu, normalizedKernel=kernel.localRenormalized, lower.limit=lower.limit,upper.limit=upper.limit)
