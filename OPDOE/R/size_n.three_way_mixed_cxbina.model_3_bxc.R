#(size n.three way mixed cx(bina). model 3 bxc)
# Section 3.4.4.2 test interaction BxC in A

# Three way mixed classification. (A>B)xC, Model IV
# Factor A and C are fixed, factor B is random. Determining n.
# a b and c are given. Testing hypothesis about interaction BxC within A
size_n.three_way_mixed_cxbina.model_3_bxc <-  function(alpha, beta, delta, a, b, c, cases)
{
	n <- 2
	dfn <- a*(b-1)*(c-1)
	dfd <- a*b*c*(n-1)
...
# example
# size.3_4_4_2.interaction.BC(0.05, 0.1, 0.5, 6, 5, 4, "maximin")
# size.3_4_4_2.interaction.BC(0.05, 0.1, 0.5, 6, 5, 4, "minimin")
