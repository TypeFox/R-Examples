#(size n.three way mixed cx(bina). model 3 b)
# Section 3.4.4.2 test factor B in A

# Three way mixed classification. (A>B)xC, Model III
# Factor A is random, factor B and C are fixed. Determining n.
# a b and c are given. Testing hypothesis about factor B in A
size_n.three_way_mixed_cxbina.model_3_b <-  function(alpha, beta, delta, a, b, c, cases)
{
	n <- 2
	dfn <- a*(b-1)
	dfd <- a*b*c*(n-1)
...
# example
# size.3_4_4_2.test_factor_BinA(0.05, 0.1, 0.5, 6, 5, 4, "maximin")
# size.3_4_4_2.test_factor_BinA(0.05, 0.1, 0.5, 6, 5, 4, "minimin")
