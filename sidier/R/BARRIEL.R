...
colnames(mat_alin)<-c(1:ncol(mat_alin))
...
single<-c()
for (i12 in 1:ncol(GAPSunique))
{if(abs(GAPSunique[1,i12]-GAPSunique[2,i12])==1) single<-c(single,i12)}
if(length(single)>0)
{
GAPSsingle<-as.matrix(GAPSunique[,c(single)])
GAPSunique<-as.matrix(GAPSunique[,-c(single)])
}
...
if(exists("GAPSsingle"))
{
if(length(single)>0)
{
alinSingle<-as.matrix(mat_alin[,(GAPSsingle[1,]+1)])
alinCodified<-as.matrix(mat_alin[,-(GAPSsingle[1,]+1)])
}
colnames(alinSingle)<-(GAPSsingle[1,]+1)
}
