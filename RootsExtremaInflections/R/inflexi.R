...
function(x,y,i1,i2,nt,alpha,xlb,ylb,xnd,ynd){
  #Find inflection of discrete data (xi,yi) with Taylor regression and degree of polynomial nt
  ...
  #Search for all available inflection-points p=x_i
  for (i in (i1:i2)){
    ii<-ii+1;
    ...
    aml<-list(aml,am);
    v0n[ii]=abs(c1$coeff[3]);
  }
  mans<-matrix(cbind(v0n,vpr),ncol=2,dimnames=list(c(),c("|a_2|","point")));
  rownames(mans) <- rownames(mans, do.NULL = FALSE, prefix = "#")
  n0<-which(v0n == min(v0n));
  ipc<-mans[which.min(v0n),2];
  ...
  stit<-paste0('Data for [',toString(round(x1[1],digits=2)),',',toString(round(x1[length(x1)],digits=2)),'] \n');
  title(paste0(stit,'Taylor Regression n = ',nt,' , a = ',alpha,' % ','\n Find inflection'),cex.main=0.7)
  #title(paste0(stit,'Taylor Regression n = ',nt,'\n Find inflection'),cex.main=0.7)
  box()
  plot(vpr,v0n,col='blue',pch=19,cex=.5,axes=FALSE,ylab=expression(paste('|',alpha[2],'|')) ,xlab=expression(rho));
