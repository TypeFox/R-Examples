cv.bal.psa <- function(covariates, treatment, propensity, strata = NULL, 
...
if  (is.null(trt.value)) trt.value=treat.lev[2]
...
if  (trt.value==treat.lev[1]){treat.lev <-treat.lev[c(2,1)]}
      }
######################################################## BEGIN C
# Call cstrat.psa for definitions of strata
cstrat <- cstrata.psa(treatment = treatment, propensity = propensity, strata = strata, int = int, 
       tree = tree, minsize = minsize, graphic = FALSE)
...
#ha  is (size ith strata by 2)-matrix that picks off values of jth covariate and treatment in ith stratum 
      ...
      if (use.trt.var)
         ...
         {          
          ...
          var.cov.by.strattreat[j, i + n.strata] =  var( ha[ha[,2]==treat.lev[2],1] ) 
          sd.adj[j,i] = sqrt((var.cov.by.strattreat[j, i] + var.cov.by.strattreat[j, i + n.strata])/2)
         } 
     }
...
# mean.diff.unadj and sd.un are mean.diff and sd.adj for each covariate without propensity score adjustment. 
   ...
   if (use.trt.var){sd.un[j] = sd(XX[XX[, n.cov + 1] == treat.lev[2] ,j ])}else{
...
#uess[j,2] is going to be shown as weighted-avg of above dots from effect.sizeji in the final graphic. 
   ...
   if (universal.psd == TRUE) {sd.adj[j,] = sd.un[j]}
   for (i in 1:n.strata)
...
w/ & w/o PS adjustment"
if(absolute.es == TRUE){main <- paste("Absolute", main)}
...
plot(final, y.coord, xlim = xlim, ylim = NULL, axes = FALSE,
   ...
   xlab= paste("Standardized Effect Sizes: treatment", treat.lev[2], 
        "- treatment ", treat.lev[1]), ylab=' ', 
   main = main, font = 2, cex = 0,  ...)

axis(1, font=2, las = 1) 
axis(2, 1:n.cov, dimnames(ord.uess)[[1]], font = 2, las = 1, tick = FALSE, cex.axis = .79) 
...
#pch = 94 + m,
if(plot.strata){
for (m in 3:(2 + n.strata)){points(final[,m], y.coord[,m], col = "blue", pch = 94 + m, cex = .66)}
               }
abline(v = 0, lty = 2, lwd = 1.2)
abline(v = seq(round(xlim[1]-1,0), round(xlim[2]+1,0), .5 ), lty = 3, lwd = .7)
box()
####################### END PLOTTING ################################
