...
#' "experimentalImpulse" for expected change in processes given an exogenous input of +1 for each process 
#' (alternate characterisation of autoregressive and cross regressive plots),
...
plotSingle<-function(x,resolution=50,plotType,impulseIndex=NULL,xlim,colVector='auto',ltyVector='auto',...){
  ...
  if(plotType=='AR'){
    ar<- matrix(apply(plottimes,1,function(x) c(diag(OpenMx::expm(DRIFT*x)))),ncol=nrow(DRIFT),byrow=T)
    plotdata<-ar
    legend<-paste0(c(diag(DRIFTlabels)))
  }
  ...
  if(colVector=='auto') colVector <- grDevices::rainbow(ncol(plotdata),v=.8) #set plot colours
  if(ltyVector=='auto') ltyVector <- rep('l',ncol(plotdata))
  
  graphics::plot(plottimes,plotdata[,1],col=colVector[1],ltyVector[1],...)
  if(ncol(plotdata) > 1) {
    for(rowi in 2:ncol(plotdata)){
      graphics::points(plottimes, plotdata[,rowi],col=colVector[rowi],ltyVector[rowi],...)
    }
  }
