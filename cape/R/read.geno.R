read.geno <-
function(file.format = c("cape", "csv", "rdata"), filename = NULL, geno.col = NULL, delim = ",", na.strings = "-", check.chr.order = TRUE) {
	...
	if(file.format == "csv"){
	   ...
	   if(is.null(geno.col)){
		...
		sex.locale <- c(which(geno[1,] == "X"), which(geno[1,] == "Y"), which(geno[1,] == 0))
		if(length(sex.locale) > 0){
			geno <- geno[,-sex.locale]
			}
			
		#sort the markers by chromosome then position
		marker.info <- t(geno[1:2,])
		sorted.snps <- sortByThenBy(marker.info, sort.cols = c(1,2), col.type = c("n", "n"))
	
		#make sure the genotypes are in the same order
		snp.order <- match(rownames(sorted.snps), colnames(geno))
		geno <- geno[,snp.order]
					
		results <- list(geno[3:dim(geno)[1],], colnames(geno), as.numeric(geno[1,]), as.numeric(geno[2,]), 1:dim(geno)[2])
