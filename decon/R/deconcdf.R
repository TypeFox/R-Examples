         ...
         grid=100,ub=2,...)
{
  ...
  fhat=switch(tolower(error2),
    normal=.C("DKESupport",as.double(y),as.integer(length(y)),
      y=as.double(x),as.integer(length(x)),
      as.double(bw),as.double(sig),as.integer(1)),
    snormal=.C("DKEGauss",as.double(y),as.integer(length(y)),
      y=as.double(x),as.integer(length(x)),
      as.double(bw),as.double(sig),as.integer(1)),
    laplacian=.C("DKELaplace",as.double(y),as.integer(length(y)),
      y=as.double(x),as.integer(length(x)),
      as.double(bw),as.double(sig),as.integer(1)),
    hnormal=.C("DKEHSupport",as.double(y),as.integer(length(y)),
      y=as.double(x),as.integer(length(x)),
      as.double(bw),as.double(sig),as.integer(1)),
