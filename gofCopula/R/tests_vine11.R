gofKendallCvM = function(copula, x, param = 0, param.est = T, margins = "ranks", M = 100, execute.times.comp = T){
  if (is.matrix(x) == F){stop("x must be a matrix")}
  if (dim(x)[2] != 2){stop("x must be of dimension 2")}
  
  if (execute.times.comp == T & M >= 100){
    times.comp = gofCheckTime(copula = copula, x=x, test = "gofKendallCvM", M = M, print.res = F)
...
#   else if (copula == "270 bb8"){fam =40}
  ...
  else {stop("This copula is not implemented for gofKendallCvM.")}
  res = BiCopGofTest(u1 = x[,1], u2 = x[,2], family = fam, par = param, par2 = 0, method = "kendall", B = M)
    ...
    structure(class = "gofCOP", 
              list(method = sprintf("Parametric bootstrap goodness-of-fit test (Cramer-von Mises) based on Kendall's process"), 
                   erg.tests = matrix(c(res$p.value.CvM, res$statistic.CvM), ncol = 2, 
                                      dimnames = list("KendallCvM", c("p.value", "test statistic")))))
}
