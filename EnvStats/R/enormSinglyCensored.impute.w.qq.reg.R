enormSinglyCensored.impute.w.qq.reg <-
function (x, censored, N, T1, n.cen, censoring.side, ci, ci.method = "normal.approx", 
    ...
    ub.impute = Inf) 
{
    parameters <- enormSinglyCensored.qq.reg(x, censored, N, 
        T1, n.cen, censoring.side, ci = FALSE, plot.pos.con = plot.pos.con)$parameters
    x.obs <- x[!censored]
    index <- 1:n.cen
