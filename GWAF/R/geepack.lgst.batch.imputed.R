...
#famid is cluster id
geepack.lgst.batch.imputed=function(genfile,phenfile,pedfile,outfile,phen,covars=NULL,col.names=T,sep.ped=",",sep.phe=",",sep.gen=","){
...
## sex is one of the columns in the phenotype file
  ...
  if (is.null(covars)){
     ...
     if (length(snplist)<2) { 
        temp.out <- c(phen,snplist,geepack.lgst.imputed(snp=test.dat[,snplist],phen=phen,test.dat=test.dat))
     } else {
        temp.out <-as.data.frame(apply(test.dat[,phensnp.dat$snps],2,geepack.lgst.imputed,phen=phen,test.dat=test.dat))
     ...
     if (length(snplist)<2) { 
        temp.out <- c(phen,snplist,geepack.lgst.imputed(snp=test.dat[,snplist],phen=phen,test.dat=test.dat,covar=covars))
     } else {
        temp.out <-as.data.frame(apply(test.dat[,phensnp.dat$snps],2,geepack.lgst.imputed,phen=phen,test.dat=test.dat,covar=covars))
  	 temp.out <-cbind(rep(phen,ncol(temp.out)),colnames(temp.out),t(temp.out))
       }
     final1 <- rbind(final1,temp.out)
  }
