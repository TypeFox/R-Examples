...
#famid is cluster id
glmm.lgst.batch=function(genfile,phenfile,pedfile,outfile,phen,covars=NULL,model="a",col.names=T,sep.ped=",",sep.phe=",",sep.gen=","){
  print(paste("phenotype data = ", phenfile))
  print(paste("genotype data = ", genfile))
  print(paste("pedigree data = ", pedfile))
  print(paste("Result of GLMM analyses =",outfile))
  if(is.null(covars)){
...
## sex is one of the columns in the phenotype file
  ...
  if (is.null(covars)){
     ...
     if (length(snplist)<2) { 
        temp.out <- c(phen,snplist,glmm.lgst(snp=test.dat[,snplist],phen=phen,test.dat=test.dat,model=model))
     } else {
        temp.out <-as.data.frame(apply(test.dat[,phensnp.dat$snps],2,glmm.lgst,phen=phen,test.dat=test.dat,model=model))
     ...
     if (length(snplist)<2) { 
        temp.out <- c(phen,snplist,glmm.lgst(snp=test.dat[,snplist],phen=phen,test.dat=test.dat,covar=covars,model=model))
     } else {
        temp.out <-as.data.frame(apply(test.dat[,phensnp.dat$snps],2,glmm.lgst,phen=phen,test.dat=test.dat,covar=covars,model=model))
  	 temp.out <-cbind(rep(phen,ncol(temp.out)),colnames(temp.out),t(temp.out))
       }
     final1 <- rbind(final1,temp.out)
  }
