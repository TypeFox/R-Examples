apval_aSPU_diffcov <- function(sam1, sam2, pow = c(1:6, Inf), sam.cov1, sam.cov2, cov1.est, cov2.est, cv.fold = 5, norm = "F", optim.bandwidth1, optim.bandwidth2, output.opt.bw1, output.opt.bw2){
	n1 <- dim(sam1)[1]
	n2 <- dim(sam2)[1]
	p <- dim(sam1)[2]
	...
	for(i in 1:length(pow)){
		if(pow[i] != Inf){
			L[i] <- sum(diff^(pow[i]))
			L.e[i] <- SPU_E_diffcov(pow[i], n1, n2, cov1.est, cov2.est)
			L.var[i] <- SPU_Var_diffcov(pow[i], n1, n2, cov1.est, cov2.est)
			...
			L[i] <- max(diff^2/(diag1/n1 + diag2/n2))
	...
	for(s in odd.ga){
		for(t in odd.ga){
			if(s != t){
				L.cov <- SPU_Cov_diffcov(s, t, n1, n2, cov1.est, cov2.est)
				io <- which(odd.ga == s)
				jo <- which(odd.ga == t)
				i <- which(pow == s)
				j <- which(pow == t)
