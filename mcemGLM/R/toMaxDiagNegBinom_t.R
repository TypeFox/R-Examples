...
toMaxDiagNegBinom_t <- function(pars, df, u, sigmaType, kKi, kLh, kLhi, kY, kX, kZ) {
  ...
  return(qFunctionDiagNegBinomCpp_t(beta = beta, sigma = ovSigma, alpha = alpha, sigmaType = sigmaType, u = u, df = df, kKi = kKi, kLh = kLh, kLhi = kLhi, kY, kX, kZ))
}
