         ...
         na.action=na.fail)
{
	 ...
	 mf <- eval(mf, parent.frame())
       ...
       #########################################################################################
         ...
         thetasave<-matrix(0,nrow=nsave,ncol=(nfixed+5))
    	 ...
    	 if(status==TRUE)
	 {
	        if(sigmarand==1)
	        {
                   wsigma <- 1/rgamma(1,shape=tau1/2,scale=tau2/2)
                }
                else
                {
                   wsigma <- prior$sigma2
...
"print.DPmeta"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"summary.DPmeta"<-function(object, hpd=TRUE, ...) 
{
    stde<-function(x)
    {
    	n<-length(x)
...
"print.summaryDPmeta"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"plot.DPmeta"<-function(x, hpd=TRUE, ask=TRUE, nfigr=2, nfigc=2, param=NULL, col="#bdfcc9", ...)
{
