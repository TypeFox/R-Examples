### DPMolmm.R                    
### Fit a ordinal linear mixed model with a Dirichlet Process mixture of
### normals prior for the random effect distribution.
###
       ...
       #########################################################################################
         ...
         for(i in 1:q)
         {
            for(j in i:q)
            {
               ...
               pnames2<-c(pnames2,aa)            
            }
         }
 	 
         pnames3 <- paste("mub",colnames(z),sep="-") 	     	 
...
###
### Tools for DPMolmm: anova, print, summary, plot
###
### Copyright: Alejandro Jara, 2006-2007
### Last modification: 19-04-2007.

"anova.DPMolmm"<-function(object, ...)
{
...
"print.DPMolmm"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"summary.DPMolmm"<-function(object, hpd=TRUE, ...) 
{
    stde<-function(x)
    {
    	n<-length(x)
...
### Kernel variance
    dimen2<-object$nrandom*(object$nrandom+1)/2
...
### Precision parameter
    ...
    {
      dimen5<-1
      coef.p<-object$coefficients[(dimen1+dimen2+dimen3+dimen4+1)]
      mat<-matrix(thetasave[,(dimen1+dimen2+dimen3+dimen4+1):(dimen1+dimen2+dimen3+dimen4+1)],ncol=1)
    }
    else
    {
      dimen5<-2
      coef.p<-object$coefficients[(dimen1+dimen2+dimen3+dimen4+1):length(object$coefficients)]
    ...
    class(ans) <- "summaryDPMolmm"
    return(ans)
}


"print.summaryDPMolmm"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"plot.DPMolmm"<-function(x, hpd=TRUE, ask=TRUE, nfigr=2, nfigc=2, param=NULL, col="#bdfcc9", ...)
{
