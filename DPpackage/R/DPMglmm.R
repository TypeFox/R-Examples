...
"print.DPMglmm" <- function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"summary.DPMglmm"<-function(object, hpd=TRUE, ...) 
{
    stde<-function(x)
    {
    	n<-length(x)
...
### Precision parameter
    ...
    class(ans) <- "summaryDPMglmm"
    return(ans)
}


"print.summaryDPMglmm"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
    ...
    {
       cat("\nDispersion parameter:\n")
       print.default(format(x$dispp, digits = digits), print.gap = 2, 
             quote = FALSE)
    }         
...
"plot.DPMglmm"<-function(x, hpd=TRUE, ask=TRUE, nfigr=2, nfigc=2, param=NULL, col="#bdfcc9", ...)
{
