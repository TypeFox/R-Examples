### DPMrasch.R                   
### Fit a Rasch model with a Dirichlet mixture of normals prior
...
DPMrasch <- function(y,prior,mcmc,offset=NULL,state,status,
         ...
         compute.band=FALSE)
{
         ...
         #########################################################################################
           ...
           model.name<-"Bayesian Rasch Model using a DPM of normals prior"		
...
"print.DPMrasch" <- function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"summary.DPMrasch"<-function(object, hpd=TRUE, ...) 
{
    stde<-function(x)
    {
    	n<-length(x)
...
### Baseline Information
    ...
    class(ans) <- "summaryDPMrasch"
    return(ans)
}


"print.summaryDPMrasch"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"plot.DPMrasch"<-function(x, hpd=TRUE, ask=TRUE, nfigr=2, nfigc=2, param=NULL, col="#bdfcc9", ...)
{
