         ...
         na.action=na.fail)
{
       ...
       #########################################################################################
         ...
         else
         {
            ...
            alpha<-rgamma(1,shape=prior$a0,rate=prior$b0)
            alphapr<-1
       ...
       #########################################################################################
         ...
         thetasave<-matrix(0,nrow=nsave,ncol=q+nfixed+dispp+q+nuniq+1+q*q)
       ...
       #########################################################################################
         ...
         dimen<-q+nfixed+dispp+q+nuniq+1+q*q
         ...
         pnames4<- c("alpha",paste("ortho",seq(q*q)))
         ...
         if(family$family=="Gamma")
         {
         	colnames(thetasave)<-c(pnames1,"phi",pnames2,pnames3,pnames4,pnames5)
         }
         else
         {
...
"print.PTglmm"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
...
"plot.PTglmm"<-function(x, hpd=TRUE, ask=TRUE, nfigr=2, nfigc=2, param=NULL, col="#bdfcc9", ...)
{
...
"summary.PTglmm"<-function(object, hpd=TRUE, ...) 
{
    stde<-function(x)
    {
    	n<-length(x)
...
### Partition
    dimen4<-object$nrandom*object$nrandom
    ...
    if(object$typepr==1)
    {
       ...
       {
          mat<-matrix(thetasave[,(dimen1+dimen2+dimen3+1):(dimen1+dimen2+dimen3+dimen4)],ncol=dimen4) 
       }  
...
### Model comparison
    ...
    class(ans) <- "summaryPTglmm"
    return(ans)
}


"print.summaryPTglmm"<-function (x, digits = max(3, getOption("digits") - 3), ...) 
{
    cat("\n",x$modelname,"\n\nCall:\n", sep = "")
    print(x$call)
    cat("\n")
