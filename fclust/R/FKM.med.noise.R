FKM.med.noise <-
function (X, k, m, delta, RS, stand, startU, conv, maxit)
{
if (missing(X))
...
{
m=1.5
cat("The parameter of fuzziness m is not numeric: the default value m=1.5 will be used ",fill=TRUE)
}
if (m<=1) 
{
m=1.5
cat("The parameter of fuzziness m must be >1: the default value m=1.5 will be used ",fill=TRUE)
}
if (missing(RS))
{
RS=1
...
if (missing(delta))
{
  Hd=FKM.med(X,k,m,RS=1,stand,conv=1e-6)$H
  for (i in 1:n) 
  {
    for (c in 1:k) 
    {
...
if (!is.numeric(delta)) 
{
  cat("The noise distance delta must is not numeric: the default value (see ?FKM.med.noise) will be used ",fill=TRUE)
  Hd=FKM.med(X,k,m,RS=1,stand,conv=1e-6)$H
  for (i in 1:n) 
  {
    for (c in 1:k) 
...
if (delta<0)
{
  cat("The noise distance delta must be non negative: the default value (see ?FKM.med.noise) will be used ",fill=TRUE)
  Hd=FKM.med(X,k,m,RS=1,stand,conv=1e-6)$H
  for (i in 1:n) 
  {
    for (c in 1:k) 
...
if (delta==0)
  stop("When delta=0, the standard algorithm is applied: run the function FKM.med")
...
iter=iter+1
U.old=U
medoid=vector(mode='numeric',length=k)
for (c in 1:k)
{
min.MED.old=10^5*sum(X^2)
for (i in 1:n)
{
min.MED=0
for (ii in 1:n) 
{
min.MED=min.MED+(U[ii,c]^m)*sum((X[i,]-X[ii,])^2)
}
if ((min.MED<min.MED.old) & match(i,medoid,nomatch=0)==0)
{
min.MED.old=min.MED;
medoid[c]=i
}
...
{
U[i,]=0
U[i, which(medoid==i,arr.ind=TRUE)]=1
}
Uout[i]=1-sum(U[i,])
}
}
}) 
func=sum((U^m)*D)+sum((Uout^m)*(delta^2))
cput[rs]=cputime[1]
value[rs]=func
it[rs]=iter
if (func<func.opt) 
