FKM.gk.noise <-
function (X, k, m, vp, delta, RS, stand, startU, conv, maxit)
{
if (missing(X))
...
if (missing(delta))
{
  Hd=FKM.gk(X,k,m,vp,RS=1,stand,conv=1e-6)$H
  for (i in 1:n) 
  {
    for (c in 1:k) 
    {
...
if (!is.numeric(delta)) 
{
  cat("The noise distance delta must is not numeric: the default value (see ?FKM.gk.noise) will be used ",fill=TRUE)
  Hd=FKM.gk(X,k,m,vp,RS=1,stand,conv=1e-6)$H
  for (i in 1:n) 
  {
    for (c in 1:k) 
...
if (delta<0)
{
  cat("The noise distance delta must be non negative: the default value (see ?FKM.gk.noise) will be used ",fill=TRUE)
  Hd=FKM.gk(X,k,m,vp,RS=1,stand,conv=1e-6)$H
  for (i in 1:n) 
  {
    for (c in 1:k) 
...
if (delta==0)
  stop("When delta=0, the standard algorithm is applied: run the function FKM.gk")
