...
function(parameters)
{
    ...
    ## different repair types (sets of DTA data) in the model
    dta.types <- parameters$dta.types

    ## pod threshold matrix which determines a repair type
    pod.threshold <- parameters$pod.threshold
    
    ## is this an importance sampling run?
    ismc.bool <- parameters$ismc.bool

    ## generate Kc vector
    kc <- parameters$dta[[1]]$kc.rsamp(Np)

    ## generate eifs vector (and weights if ISMC)
    if( ismc.bool == FALSE )
        {
            eifs   <- parameters$dta[[1]]$ifs.ractual(Np)
        } else {
            eifs   <- parameters$dta[[1]]$ifs.rsamp(Np)
            weight <- parameters$dta[[1]]$ifs.dactual(eifs) / parameters$dta[[1]]$ifs.dsamp(eifs)
        }
    ...
    ## placeholder for PCD results which track the repair type
    mult.repair.count <- matrix(0, nrow=n.int, ncol=dta.types+1)
    ...
    for(iii in 1:Np)
        {
            ...
            ## current repair type - as-manufactured at run start
            typ <- 1

            ## current pod.threshold vector to use is first row (typ=1 at start)
            temp.threshold <- pod.threshold[1,]
            ...
            for(jjj in 1:n.int)
                {
                    ...
                    first.flight <- approx(x=parameters$dta[[typ]]$cg$crack,
                                           y=parameters$dta[[typ]]$cg$flight,
                                           xout=last.crack, rule=2)$y
                    
                    ## crack length at the end of each flight
                    seq.crack <- approxExtrap(x=parameters$dta[[typ]]$cg$flight,
                                              y=parameters$dta[[typ]]$cg$crack,
                                              xout=seq.flight+first.flight)$y
                    

                    ## find K/Sigma for each flight
                    ## this should be approx XXX
                    seq.ksig <- approxExtrap(x=parameters$dta[[typ]]$geo$crack,
                                             y=parameters$dta[[typ]]$geo$ksig,
                                             xout=seq.crack)$y
                    ...
                    if( ff.bool )
                        {
                            ...
                            if( runif(1) < pod )
                                {
                                    ## repair type is a function of the crack size
                                    typ <- findInterval(last.crack, temp.threshold) + typ
                                    ## count repairs by type
                                    mult.repair.count[jjj, typ] <- mult.repair.count[jjj, typ] + 1
                                    ## if typ is largest category, it's a part replacement
                                    if( typ == (dta.types+1) ) typ <- 1
                                    ## update the temporary threshold to use
                                    temp.threshold <- pod.threshold[typ,typ:dta.types]
                                    ## count number of repairs for this inspection
                                    repair.count[jjj] <- repair.count[jjj] + 1
                                    ## perform repair
                                    kc[iii]    <- parameters$dta[[typ]]$kc.rsamp(1)
                                    last.crack <- parameters$dta[[typ]]$ifs.ractual(1)
                                }
                        }
                }
        }
    ...
    for(jjj in 1:n.int)
        {
            ...
            pcd.temp <- mult.repair.count[jjj,] / sum( first.fail > flight.cum[[jjj]] )
            names(pcd.temp) <- paste("pcd", 1:(dta.types+1), sep="")
            results$pcd.new <- rbind(results$pcd.new,
                                     data.frame(flight=flight.cum[[jjj]],
                                                insp.type=inspections$type[jjj],
                                                t(pcd.temp)))
