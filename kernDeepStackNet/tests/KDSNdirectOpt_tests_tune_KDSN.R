...
tunedKDSN <- tuneKDSN (y=XORdat4Dim[, 1], X=as.matrix(XORdat4Dim[, -1]), maxRuns=2, repetitions=2, 
           maxLevels=2, gammaPar=1, fineTuneIt=25, tol_input=.Machine$double.eps^0.1, 
           addInfo=TRUE, dimMax=dim(as.matrix(XORdat4Dim[, -1]))[1]/2)
library(pROC)
