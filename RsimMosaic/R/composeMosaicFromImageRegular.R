...
# 
# A function to compose the mosaic of an Image based on regular tiles. This function will compute the mosaic regularly 
# replacing the pixels of the original image.
# 
...
# 
	...
	if(verbose) {
		cat(paste("\n ------------------------------------------------ \n"))
		cat(paste("    R Simple Mosaic composer - regular version   \n"))
		cat(paste(" ------------------------------------------------ \n\n"))
	}
	...
	for(i in 2:(xOrigImgSize-1) )  {
		for(j in 2:(yOrigImgSize-1) ) {
			# for each pixel in the original image, get a close match tile.
			# the tile is selected in the space defined by the RGB value of the pixel 
			# and of its Le, ULe, U, UR, R, LoR, Lo, and LoLe, neighbours

			# create the vector with the statistical quantites computed for the pixel
			pixelRGBandNeigArray <- computeStatisticalQuantitiesPixel(i, j, originalImage, useGradients)
			...
			startI <- (i-2)*xTileSize + 1
			startJ <- (j-2)*yTileSize + 1
			...
			if(removeTiles) {
				...
				if(length(libForMosaic[,1]) < (fracLibSizeThreshold*length(libForMosaicFull[,1])) ){
					...
					idxs <- runif(round(repFracSize*length(libForMosaicFull[,1])), 1, length(removedList))
