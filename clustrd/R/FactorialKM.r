FactorialKM <- function(data,nclus,ndim,nstart=100,smartStart=FALSE) {
#Factorial k-means, minimize f(A,U,Y)=|| XA - UY ||^2
  ...
  for (run in c(1:nstart)) {
    ...
    if(smartStart==TRUE){  # k-means starting solution
      ...
      s = t(data)%*%U%*%D%*%t(U)%*%data-t(data)%*%data
