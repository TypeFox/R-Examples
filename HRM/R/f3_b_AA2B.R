...
#' @keywords internal
hrm.2w.1f.AA2B = function(X, alpha, group , subgroup, factor, subject, data ){
  ...
  output = data.frame(hypothesis=paste(as.character(group), ":", as.character(subgroup), ":", as.character(factor)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
