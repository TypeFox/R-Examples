#' Test for interaction of factor A and C
#' 
...
#' @keywords internal
hrm.1w.2f.AC = function(X, alpha, group , factor1, factor2, subject, data ){
  #Pa - 1/d*Jd - Pc
  ...
  test = (t(X_bar)%*%K_AC%*%X_bar)/(t(rep(1,dim(K_AC)[1]))%*%(K_AC*direct)%*%(rep(1,dim(K_AC)[1])))
  p.value=1-pf(test,f,f0)
  output = data.frame(hypothesis=paste(as.character(group),":",as.character(factor2)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
