#' Test for interaction of factor A, B and C
#' 
...
#' @keywords internal
hrm.1w.2f.ABC = function(X, alpha, group , factor1, factor2, subject, data ){
  #Pa - Pd - Pc
  ...
  for(i in 1:a){
    f_1 = f_1 + ((1-1/a)*1/n[i])^2*.E1(n,i,V[[i]])
    j=i+1
    while(j<=a){
      f_1 = f_1 + 2*((1-1/a)*1/n[i])*((1-1/a)*1/n[j])*.E3(V[[i]],V[[j]])
      j=j+1
    }
  }
  
  for(i in 1:a){
    f_2 = f_2 + ((1-1/a)*1/n[i])^2*.E2(n,i,V[[i]])
    j=i+1
    while(j<=a){
      f_2 = f_2 + 2*1/(a^2*n[i]*n[j])*.E4(1/(n[i]-1)*P(n[i])%*%X[[i]],1/(n[j]-1)*K%*%t(X[[j]])%*%P(n[j])%*%X[[j]]%*%K%*%t(X[[i]])%*%P(n[i]))
      j=j+1
    }
  }
  ...
  for(i in 1:a){
    f0_2 = f0_2 + ((1-1/a)*1/n[i])^2*1/(n[i]-1)*.E2(n,i,V[[i]])
  }
  ...
  test = (t(X_bar)%*%K_ABC%*%X_bar)/(t(rep(1,dim(K_ABC)[1]))%*%(K_ABC*direct)%*%(rep(1,dim(K_ABC)[1])))
  p.value=1-pf(test,f,f0)
  output = data.frame(hypothesis=paste(as.character(group),":",as.character(factor1),":",as.character(factor2)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
