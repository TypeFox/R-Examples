...
#' @keywords internal
hrm.2w.2f.A2C = function(X, alpha, group , subgroup, factor1, factor2, subject, data ){
  ...
  test = (t(X_bar)%*%K_A2C%*%X_bar)/(t(rep(1,dim(K_A2C)[1]))%*%(K_A2C*direct)%*%(rep(1,dim(K_A2C)[1])))
  p.value=1-pf(test,f,f0)
  output = data.frame(hypothesis=paste(as.character(subgroup),":",as.character(factor2)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
