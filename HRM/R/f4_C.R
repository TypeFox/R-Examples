...
#' @keywords internal
hrm.2w.2f.C = function(X, alpha, group , subgroup, factor1, factor2, subject, data ){
  ...
  test = (t(X_bar)%*%K_C%*%X_bar)/(t(rep(1,dim(K_C)[1]))%*%(K_C*direct)%*%(rep(1,dim(K_C)[1])))
  p.value=1-pf(test,f,f0)
  output = data.frame(hypothesis=paste(as.character(factor2)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
