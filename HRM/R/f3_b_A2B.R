...
#' @keywords internal
hrm.2w.1f.A2B = function(X, alpha, group , subgroup, factor, subject, data ){
  ...
  output = data.frame(hypothesis=paste(as.character(subgroup), ":", as.character(factor)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
