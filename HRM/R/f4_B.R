...
#' @keywords internal
hrm.2w.2f.B = function(X, alpha, group , subgroup, factor1, factor2, subject, data ){
  ...
  output = data.frame(hypothesis=paste(as.character(factor1)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
