...
#' @keywords internal
hrm.2w.2f.AA2BC = function(X, alpha, group , subgroup, factor1, factor2, subject, data ){
  ...
  test = (t(X_bar)%*%K_AA2BC%*%X_bar)/(t(rep(1,dim(K_AA2BC)[1]))%*%(K_AA2BC*direct)%*%(rep(1,dim(K_AA2BC)[1])))
  p.value=1-pf(test,f,f0)
  output = data.frame(hypothesis=paste(as.character(group), ":", as.character(subgroup), ":", as.character(factor1),":",as.character(factor2)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
