...
#' @keywords internal
hrm.1w.2f.AB = function(X, alpha, group , factor1, factor2, subject, data ){
  #Pa - Pd - 1/c Jc
  ...
  output = data.frame(hypothesis=paste(as.character(group),":",as.character(factor1)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
