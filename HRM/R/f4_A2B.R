...
#' @keywords internal
hrm.2w.2f.A2B = function(X, alpha, group , subgroup, factor1, factor2, subject, data ){
  ...
  test = (t(X_bar)%*%K_A2B%*%X_bar)/(t(rep(1,dim(K_A2B)[1]))%*%(K_A2B*direct)%*%(rep(1,dim(K_A2B)[1])))
  p.value=1-pf(test,f,f0)
  output = data.frame(hypothesis=paste(as.character(subgroup),":",as.character(factor1)),df1=f,df2=f0, crit=crit, test=test, p.value=p.value, sign.code=.hrm.sigcode(p.value))
