...
myModeling13 <- function(myData, AIC.k, delay.confidence){
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "43, model = e0~fNTAFD + emax, bsv = e0 + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.Emax.fun(Emax, EC50, Conc=EXPOSURE),
                    data=myData,
  ...
  tmp <- dfEmpty
  tmp$MODEL <- "44, model = e0~fNTAFD + sigmoidal emax, bsv = e0 + ec50 + emax"
  suppressWarnings(try({
    my.nlme <- nlme(RESPONSE~.my.intercept.fun(Int)+.my.sigEmax.fun(Emax, EC50, Hill, Conc=EXPOSURE),
                    data=myData,
  ...
  #Return results
  return(list(results.nlme.ETA.BL.SLO.EC50.EMAX.noBOV, saved.models))
}
