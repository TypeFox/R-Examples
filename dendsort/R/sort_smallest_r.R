...
sort_smallest_r <- function(d) {
  ...
  if (is.leaf(left) && is.leaf(right)) {
    ...
    n <- merge(right, sort_smallest_r(left),height=a$height)
    attr(n,"min") <- min(a$height, attributes(n[[2]])$min)
  } else if (is.leaf(left) && !is.leaf(right)) {
    n <- merge(left, sort_smallest_r(right), height=a$height)
    attr(n,"min") <- min(a$height, attributes(n[[2]])$min)
  } else {
    lft  <- sort_smallest_r(left)
    rght <- sort_smallest_r(right)
