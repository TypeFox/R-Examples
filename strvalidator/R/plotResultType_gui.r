...
plotResultType_gui <- function(env=parent.frame(), savegui=NULL, debug=FALSE, parent=NULL){
  ...
  # Main window.
  w <- gwindow(title="Plot result type", visible=FALSE)
  ...
  .plot <- function(){
    ...
    if (!is.na(.gData) && !is.null(.gData)){
      ...
      if(val_titles){
        ...
        mainTitle <- paste("Analysis of", numberOfSamples, "samples")
        xTitle <- "Result type"
        if(val_prop){
          yTitle <- "Proportion"
        } else {
      ...
      if(val_print){
        ...
        for(t in seq(along=type)){
          .gData[.gData$Type == type[t], ]$Pos <- 
            cumsum(.gData[.gData$Type == type[t], ]$freq)
        }
        .gData$Pos <- .gData$Pos - (.gData$freq / 2)
        if(debug){
          print(.gData)
        }
