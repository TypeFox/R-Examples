...
# 19-09-2013: Changed some typos
#             Adjusted the output

jt.star.gmw <- function(X,g,goi,type,nper,alternative,mc,PARAMETERS,output, keepPM=keepPM){
...
# Case: permutation, two sided, X is vector
	    ...
	    for(testRun in 1:nrow(diffTests))
	    {
	      ...
	      #names(res)[testRun] <- paste(diffTests[testRun,],collapse="")
              names(res)[testRun] <- "H1: sum(P_tt') != #t/2 , with t<t'"
	    }
	    if(output=="min")
	    {
...
# Case: asymptotic, two sided, X is vector
	    res <- c()
            stop("We do not have a two-sided version for the jt* test, sorry!")

	  } else  if(type=="external"){
#----------------------------------------------------------------------------------------------------------------------------------------
	    res <- c()
	    stop("We do not have an external version of the jt* test, sorry!")

          } else {
#----------------------------------------------------------------------------------------------------------------------------------------
# Case: other options, two sided, X is vector
	    res <- c()
	    stop("We do not have this kind of type for the jt* test, sorry!")
	  }
##---------------------------------------------------------------------------------------------------------------------------------------
...
# Case: asymptotic, greater, X is vector
	    res <- c()
            stop("We do not have an asymptotic version for the jt* test, sorry!")
          
	  } else  if(type=="external"){
#----------------------------------------------------------------------------------------------------------------------------------------
	    res <- c()
	    stop("We do not have external type for the jt* test, sorry!")
	  } else {
#----------------------------------------------------------------------------------------------------------------------------------------
# Case: other options, greater, X is vector
	    res <- c()
	    stop("We do not have this kind of type for the triple test, sorry!")
	  }
...
#----------------------------------------------------------------------------------------------------------------------------------------
	    res <- c()
	    stop("We do not have an external type for the jt* test, sorry!")
          } else {
#----------------------------------------------------------------------------------------------------------------------------------------
...
# Case: asymptotic, two sided, X is matrix
	    res <- c()
	    warning("We do not have an asymptotic test for the jt* test, sorry!")
...
# Case: other, greater, X is matrix
	    ...
	    res <- c()
	    warning("We do not have this kind of type for the jt* test, sorry!")
	  }
    } else if(alternative=="smaller"){
	  if(type=="permutation"){
#----------------------------------------------------------------------------------------------------------------------------------------
...
# Case: asymptotic, smaller, X is matrix
	    res <- c()
	    warning("We do not have an asymptotic version for the jt* test, sorry!")
