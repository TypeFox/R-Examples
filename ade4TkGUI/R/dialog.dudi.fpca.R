################################
# GUI for dudi.fca function
################################
"dialog.dudi.fpca" <- function(show, history)
{
#
# Main dialog window with title
#
	tt <- tktoplevel()
	tkwm.title(tt,"dudi.fpca")	
#
...
#
	...
	tkgrid(tklabel(TFrame,text="Fuzzy principal components analysis", font="Times 18", foreground="red"), labh)
	tkbind(labh, "<Button-1>", function() print(help("dudi.fpca")))
	tkgrid(TFrame)
#
# Dataframe and dudi
#
...
################################
	...
	#
		substitute(dudi.fpca(df = df, scannf = scannf, nf = nf))
	}
