...
#
	...
	tkgrid(tklabel(TFrame,text="Canonical correspondence analysis", font="Times 18", foreground="red"), labh)
	tkbind(labh, "<Button-1>", function() print(help("cca")))
	tkgrid(TFrame)
#
# Dataframe and dudi
#	
	...
	df1.entry <- tkentry(IOFrame, textvariable=df1var)
	...
	choose1.but <- tkbutton(IOFrame, text="Set", command=function() choosedf(df1.entry, dfnr1.label, dfnc1.label))
	choose2.but <- tkbutton(IOFrame, text="Set", command=function() choosedf(df2.entry, dfnr2.label, dfnc2.label))
	tkgrid(tklabel(IOFrame,text="First input dataframe name : "), df1.entry, choose1.but, dfnr1.label, dfnc1.label)
	tkgrid(tklabel(IOFrame,text="Second input dataframe name : "), df2.entry, choose2.but, dfnr2.label, dfnc2.label)
...
################################
	...
	{
	#
	# Check that the df1 is not empty and get its name
	#
		if (tclvalue(df1var) != "") {
			df1  <- parse(text=tclvalue(df1var))[[1]]
		} else {
			return(0)
		}
	#
