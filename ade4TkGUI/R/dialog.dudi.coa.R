...
#
	...
	tkgrid(tklabel(TFrame,text="Correspondence analysis", font="Times 18", foreground="red"), labh)
	tkbind(labh, "<Button-1>", function() print(help("dudi.coa")))
	tkgrid(TFrame)
#
# Dataframe and dudi
#
...
#	tkgrid(tklabel(frame2,text="Number of principal axes:", foreground="blue"), sticky="w", columnspan=2)
#	nf.entry <- tkentry(frame2, textvariable=nfvar, width=4, state="disabled")
#	#scannf.cbut <- tkcheckbutton(frame2,text="Ask number of axes interactively", variable=scannfvar)
#	scannf.cbut <- tkcheckbutton(frame2,text="Ask number of axes interactively", variable=scannfvar,
#		command=function() if (!as.logical(tclObj(scannfvar))) tkconfigure(nf.entry, state="normal") else tkconfigure(nf.entry, state="disabled") )
#	tkgrid(scannf.cbut, columnspan=2, sticky="w")
#	tkgrid(tklabel(frame2,text="Number of axes : "), nf.entry, sticky="w")
