################################
# GUI for discrimin function
################################
"dialog.discrimin" <- function(show, history)
{
#
# Main dialog window with title
#
	tt <- tktoplevel()
	tkwm.title(tt,"discrimin")	
#
...
#
	...
	tkgrid(tklabel(TFrame,text="Discriminant analysis", font="Times 18", foreground="red"), labh)
	tkbind(labh, "<Button-1>", function() print(help("discrimin")))
	tkgrid(TFrame)
#
...
#
	...
	tkgrid(tklabel(GFFrame,text="- Groups -", foreground="blue"), columnspan=5)
...
################################
	...
	#
		substitute(discrimin(dudi = dudi, fac = fac, scannf = scannf, nf = nf))
	}
