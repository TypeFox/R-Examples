################################
# GUI for dudi.mix function
################################
"dialog.dudi.mix" <- function(show, history)
{
#
# Main dialog window with title
#
	tt <- tktoplevel()
	tkwm.title(tt,"dudi.mix")	
#
...
#
	...
	tkgrid(tklabel(TFrame,text="Mixed (numeric + factor) analysis", font="Times 18", foreground="red"), labh)
	tkbind(labh, "<Button-1>", function() print(help("dudi.mix")))
	tkgrid(TFrame)
#
# Dataframe and dudi
#
...
# Add squares
#
	ASFrame <- tkframe(tt, relief="groove", borderwidth=2)
	tkgrid(tklabel(ASFrame,text="- Numeric variables -", foreground="blue"), columnspan=2)
	addsq.cbut <- tkcheckbutton(ASFrame,text="Add squares", variable=addsqvar)
	tkgrid(addsq.cbut, columnspan=2)
	tkgrid(ASFrame)
#
# Local variables
#
...
################################
	...
	#
	# Get add squares checkboxe state
	#
		addsq <- as.logical(tclObj(addsqvar))
	#
	# Make the command line
	#
		substitute(dudi.mix(df = df, add.square = addsq, scannf = scannf, nf = nf))
	}
