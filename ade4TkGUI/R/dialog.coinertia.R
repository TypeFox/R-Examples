################################
# GUI for coinertia function
################################
"dialog.coinertia" <- function(show, history)
{
#
# Main dialog window with title
#
	tt <- tktoplevel()
	tkwm.title(tt,"coinertia")	
#
...
#
	...
	tkgrid(tklabel(TFrame,text="Coinertia analysis", font="Times 18", foreground="red"), labh)
	tkbind(labh, "<Button-1>", function() print(help("coinertia")))
	tkgrid(TFrame)
#
# Dataframe and dudi
#	
	...
	dudi2.entry <- tkentry(IOFrame, textvariable=dudi2var)
	...
	choose2.but <- tkbutton(IOFrame, text="Set", command=function() choosedudirc(dudi2.entry, dfnr2.label, dfnc2.label))
	tkgrid(tklabel(IOFrame,text="First input dudi name : "), dudi1.entry, choose1.but, dfnr1.label, dfnc1.label)
	tkgrid(tklabel(IOFrame,text="Second input dudi name : "), dudi2.entry, choose2.but, dfnr2.label, dfnc2.label)
...
################################
	...
	#
	# Check that the dudi2 is not empty and get its name
	#
		if (tclvalue(dudi2var) != "") {
			dudi2  <- parse(text=tclvalue(dudi2var))[[1]]
		} else {
			return(0)
		}
	#
	...
	#
		substitute(coinertia(dudiX = dudi1, dudiY = dudi2, scannf = scannf, nf = nf))
	}
