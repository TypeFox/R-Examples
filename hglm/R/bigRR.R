...
function (formula = NULL, y, X, Z, data = NULL, shrink = NULL, weight = NULL,
	...
	tol.conv = 1e-8, only.estimates = FALSE, ...) 
{
	...
	wZt <- sqrt(w)*t(Z)
	G <- crossprod(wZt) #else G <- gpuMatMult(t(wZt), wZt)
