...
ModuleLabels <- function(colr, IsChain, IsList){
  ...
  for(k in 1:5) {
    ...
    if(IsList[k]) 	text(x1[k], yL - 10, "List", col = colr, adj = 0, font = 3)
  }
...
#         }
    ...
    if( IsList[i] && ListAlready == 0) ListAlready = 1
  } # for i
} # end function 'Boxed'
...
CallLister <- function( callList ){
  ...
  for(j in 1:5){
    ...
    if( length(callList [[j]])>1) {
      if( is.null(attr(callList [[j]], "chain"))) isList[j] <- TRUE else isChain [j] <- TRUE
    }
...
plot.zoonWorkflow <- function(x, ...) {
  ...
  ModuleLabels(colr = "wheat4",
               IsChain,
               IsList)
  
  Boxed2(NoOfModules = NoOfModules,
