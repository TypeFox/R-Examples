...
mylocfdr <-
function(zz, bre = 120, df = 7, pct = 0, pct0 = 1/4, nulltype = 1, type = 0, plot = 1, mult, mlests, main = " ", sw = 0, silently = FALSE) 
# New argument: silently  !!
{
  call = match.call()
	if(length(bre) > 1) {
		lo <- min(bre)
...
#           warning("CM estimation failed, middle of histogram non-normal")				# Code modification  !!
	    otp <- list(errmsg="mylocfdr: CM estimation failed, middle of histogram non-normal")	# Code modification  !!
...
#         else warning("ML Estimation failed")
		...
		otp <- list(errmsg="mylocfdr: ML Estimation failed")		# Code modification  !!
		if (!silently) warning(otp$errmsg)				# Code modification  !!
		class(otp) = "error2"						# Code modification  !!
		return(otp)							# Code modification  !!
	  }									# Code modification  !!
