...
{
   if (!is.matrix(B)) stop("B argument of SigFq must be a matrix.\n")
   if (length(D)!=p || nrow(B)!=p || ncol(B)!=q) stop("Wrong argument dimensions.\n")
   if (any(D<=0.)) stop("All specific variances must be positive.\n") 
...
as.matrix.SigFq <- function(x,...) 
{
   	if (is.null(x$B))  stop("Trying to convert a NULL SigFq object to a matrix.\n")
...
RightMult.SigFq <- function(x,a)
{
   ...
   if (is.matrix(a))  { 
	...
	if (x$p!=length(a))  stop("Argument dimensions do not match.\n")
   	matrix(a*x$D,x$p,1) + x$B %*% (t(x$B)%*%a)  # return( matrix(a*x$D,1,p) + x$B %*% (t(x$B)%*%a) )
   }
}
