...
#' 
#' @details
#' Given a data frame that includes all of the FACT/GOG-Ntx-11 (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT/GOG-Ntx-11 
...
#' data.  As its default, \code{scoreFACTGOG_Ntx11} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACTGOG_Ntx11}.
#' 
#' 
#' @param df A data frame with the FACT-GOG_Ntx items, appropriately-named.
...
#'   \item{NtxS11}{FACT/GOG-Ntx-11 subscale}
#'   \item{FACTGOG_Ntx11_TOTAL}{FACT/GOG-Ntx-11 Total Score (i.e., PWB+SWB+EWB+FWB+NtxS11)}
#'   \item{FACTGOG_Ntx11_TOI}{FACT/GOG-Ntx-11 Trial Outcome Index (e.g., PWB+FWB+NtxS11)}
#' }
#' 
#' @references FACT/GOG-Ntx-11 Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACTGOG_Ntx11 <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    FACTGOG_Ntx11_TOTAL_N <- TOTAL_N
    NtxS11_N <- AC_N
    NtxS11 <- AC
    FACTGOG_Ntx11_TOTAL <- TOTAL
    FACTGOG_Ntx11_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, NtxS11_N, FACTGOG_Ntx11_TOTAL_N, 
            NtxS11, FACTGOG_Ntx11_TOTAL, FACTGOG_Ntx11_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, NtxS11, FACTGOG_Ntx11_TOTAL, 
            FACTGOG_Ntx11_TOI))
    }
    return(dfOut)
} 
