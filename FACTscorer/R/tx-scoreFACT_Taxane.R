...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Taxane (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Taxane 
...
#' data.  As its default, \code{scoreFACT_Taxane} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Taxane}.
#' 
#' 
#' @param df A data frame with the FACT-Taxane items, appropriately-named.
...
#'   \item{TaxS}{Taxane subscale}
#'   \item{FACT_Taxane_TOTAL}{FACT-Taxane Total Score (i.e., PWB+SWB+EWB+FWB+TaxS)}
#'   \item{FACT_Taxane_TOI}{FACT-Taxane Trial Outcome Index (e.g., PWB+FWB+TaxS)}
#' }
#' 
#' @references FACT-Taxane Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('Ntx1', 'Ntx2', 'Ntx3', 'Ntx4', 'Ntx5', 'HI12', 'Ntx6', 'Ntx7', 'Ntx8', 'Ntx9', 
#'    'An6', 'Tax1', 'Tax2', 'Tax3', 'Tax4', 'Tax5')
...
scoreFACT_Taxane <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    FACT_Taxane_TOTAL_N <- TOTAL_N
    TaxS_N <- AC_N
    TaxS <- AC
    FACT_Taxane_TOTAL <- TOTAL
    FACT_Taxane_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, TaxS_N, FACT_Taxane_TOTAL_N, 
            TaxS, FACT_Taxane_TOTAL, FACT_Taxane_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, TaxS, FACT_Taxane_TOTAL, FACT_Taxane_TOI))
    }
    return(dfOut)
} 
