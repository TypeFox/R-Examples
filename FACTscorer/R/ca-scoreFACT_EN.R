...
#' 
#' @details
#' Given a data frame that includes all of the FACT-En (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-En 
...
#' data.  As its default, \code{scoreFACT_En} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_En}.
#' 
#' 
#' @param df A data frame with the FACT-En items, appropriately-named.
...
#'   \item{ENCS}{Endometrial Cancer subscale}
#'   \item{FACT_En_TOTAL}{FACT-En Total Score (i.e., PWB+SWB+EWB+FWB+ENCS)}
#'   \item{FACT_En_TOI}{FACT-En Trial Outcome Index (e.g., PWB+FWB+ENCS)}
#' }
#' 
#' @references FACT-En Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('O1', 'O3', 'Hep8', 'ES6', 'ES4', 'Hep1', 'ES1', 'ES2', 'ES3', 'HI7',
#'    'ES8', 'En1', 'B1', 'Cx6', 'Bl2', 'En2')
...
scoreFACT_En <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("O1", "O3", "Hep8", "ES6", "ES4", "Hep1", "ES1", 
        "ES2", "ES3", "HI7", "ES8", "En1", "B1", "Cx6", "Bl2", "En2"))
    revNames <- toupper(c("O1", "O3", "Hep8", "ES6", "ES4", "Hep1", "ES1", 
        "ES2", "ES3", "HI7", "ES8", "En1", "B1", "Cx6", "Bl2", "En2"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ENCS_N, FACT_En_TOTAL_N, ENCS, 
            FACT_En_TOTAL, FACT_En_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ENCS, FACT_En_TOTAL, FACT_En_TOI))
    }
    return(dfOut)
} 
