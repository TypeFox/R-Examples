...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Lymphoma (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Lymphoma 
...
#' data.  As its default, \code{scoreFACT_Lymphoma} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Lymphoma}.
#' 
#' 
#' @param df A data frame with the FACT-Lymphoma items, appropriately-named.
...
#'   \item{LYMS}{Lymphoma subscale}
#'   \item{FACT_Lymphoma_TOTAL}{FACT-Lymphoma Total Score (i.e., PWB+SWB+EWB+FWB+LYMS)}
#'   \item{FACT_Lymphoma_TOI}{FACT-Lymphoma Trial Outcome Index (e.g., PWB+FWB+LYMS)}
#' }
#' 
#' @references FACT-Lymphoma Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_Lymphoma <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- c("P2", "LEU1", "BRM3", "ES3", "LYM1", "LYM2", "BMT6", 
        "C2", "GA1", "HI8", "N3", "LEU6", "LEU7", "BRM9", "LEU4")
    revNames <- c("P2", "LEU1", "BRM3", "ES3", "LYM1", "LYM2", "BMT6", 
        "C2", "GA1", "HI8", "N3", "LEU6", "LEU7", "BRM9", "LEU4")
    ...
    FACT_Lymphoma_TOTAL_N <- TOTAL_N
    LYMS_N <- AC_N
    LYMS <- AC
    FACT_Lymphoma_TOTAL <- TOTAL
    FACT_Lymphoma_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, LYMS_N, FACT_Lymphoma_TOTAL_N, 
            LYMS, FACT_Lymphoma_TOTAL, FACT_Lymphoma_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, LYMS, FACT_Lymphoma_TOTAL, FACT_Lymphoma_TOI))
    }
    return(dfOut)
} 
