...
#' 
#' @details
#' Given a data frame that includes all of the FACT-C (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-C 
...
#' data.  As its default, \code{scoreFACT_C} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_C}.
#' 
#' 
#' @param df A data frame with the FACT-C items, appropriately-named.
...
#'   \item{CCS}{Colorectal Cancer subscale}
#'   \item{FACT_C_TOTAL}{FACT-C Total Score (i.e., PWB+SWB+EWB+FWB+CCS)}
#'   \item{FACT_C_TOI}{FACT-C Trial Outcome Index (e.g., PWB+FWB+CCS)}
#' }
#' 
#' @references FACT-CCS Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7')
...
scoreFACT_C <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("C1", "C2", "C3", "C4", "C5", "C6", "C7"))
    revNames <- toupper(c("C1", "C2", "C5"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, CCS_N, FACT_C_TOTAL_N, CCS, FACT_C_TOTAL, 
            FACT_C_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, CCS, FACT_C_TOTAL, FACT_C_TOI))
    }
    return(dfOut)
} 
