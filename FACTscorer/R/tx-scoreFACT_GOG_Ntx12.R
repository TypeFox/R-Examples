...
#' 
#' @details
#' Given a data frame that includes all of the FACT/GOG-Ntx-12  (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT/GOG-Ntx-12 
...
#' data.  As its default, \code{scoreFACTGOG_Ntx12} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACTGOG_Ntx12}.
#' 
#' 
#' @param df A data frame with the FACT/GOG-Ntx-12 items, appropriately-named.
...
#'   \item{NtxS12}{FACT/GOG-Ntx-12 subscale}
#'   \item{FACTGOG_Ntx12_TOTAL}{FACT/GOG-Ntx-12 Total Score (i.e., PWB+SWB+EWB+FWB+NtxS12)}
#'   \item{FACTGOG_Ntx12_TOI}{FACT/GOG-Ntx-12 Trial Outcome Index (e.g., PWB+FWB+NtxS12)}
#' }
#' 
#' @references FACT/GOG-Ntx-12 Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACTGOG_Ntx12 <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    FACTGOG_Ntx12_TOTAL_N <- TOTAL_N
    NtxS12_N <- AC_N
    NtxS12 <- AC
    FACTGOG_Ntx12_TOTAL <- TOTAL
    FACTGOG_Ntx12_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, NtxS12_N, FACTGOG_Ntx12_TOTAL_N, 
            NtxS12, FACTGOG_Ntx12_TOTAL, FACTGOG_Ntx12_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, NtxS12, FACTGOG_Ntx12_TOTAL, 
            FACTGOG_Ntx12_TOI))
    }
    return(dfOut)
} 
