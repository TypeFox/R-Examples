...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Bl (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Bl 
...
#' data.  As its default, \code{scoreFACT_Bl} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Bl}.
#' 
#' 
#' @param df A data frame with the FACT-Bl items, appropriately-named.
...
#'   \item{BlCS}{Bladder Cancer subscale}
#'   \item{FACT_Bl_TOTAL}{FACT-Bl Total Score (i.e., PWB+SWB+EWB+FWB+BlCS)}
#'   \item{FACT_Bl_TOI}{FACT-Bl Trial Outcome Index (e.g., PWB+FWB+BlCS)}
#' }
#' 
#' @references FACT-Bl Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_Bl <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("BL1", "C2", "C3", "BL2", "C5", "C6", "C7", "BL3", 
        "BL4", "BL5", "C8", "C9"))
    revNames <- toupper(c("BL1", "C2", "BL2", "C5", "BL3", "C8", "C9"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, BlCS_N, FACT_Bl_TOTAL_N, BlCS, 
            FACT_Bl_TOTAL, FACT_Bl_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, BlCS, FACT_Bl_TOTAL, FACT_Bl_TOI))
    }
    return(dfOut)
} 
