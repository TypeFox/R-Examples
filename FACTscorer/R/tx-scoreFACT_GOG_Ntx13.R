...
#' 
#' @details
#' Given a data frame that includes all of the FACT/GOG-Ntx-13 (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT/GOG-Ntx-13 
...
#' data.  As its default, \code{scoreFACTGOG_Ntx13 } DOES NOT replace any of your
...
#' frame returned by \code{scoreFACTGOG_Ntx13 }.
#' 
#' 
#' @param df A data frame with the FACT/GOG-Ntx-13 items, appropriately-named.
...
#'   \item{NtxS13}{GOG_Ntx13 subscale}
#'   \item{FACTGOG_Ntx13_TOTAL}{FACT/GOG-Ntx-13 Total Score (i.e., PWB+SWB+EWB+FWB+NtxS13)}
#'   \item{FACTGOG_Ntx13_TOI}{FACT/GOG-Ntx-13 Trial Outcome Index (e.g., PWB+FWB+NtxS13)}
#' }
#' 
#' @references FACT/GOG-Ntx-13 Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('Ntx1', 'Ntx2', 'Ntx3', 'Ntx4', 'Ntx5', 'HI12', 'Ntx6', 'Ntx7',
#'     'Ntx8', 'Ntx9', 'An6', 'Ntx10', 'Ntx11')
...
scoreFACTGOG_Ntx13 <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("Ntx1", "Ntx2", "Ntx3", "Ntx4", "Ntx5", "HI12", 
        "Ntx6", "Ntx7", "Ntx8", "Ntx9", "An6", "Ntx10", "Ntx11"))
    revNames <- toupper(c("Ntx1", "Ntx2", "Ntx3", "Ntx4", "Ntx5", "HI12", 
        "Ntx6", "Ntx7", "Ntx8", "Ntx9", "An6", "Ntx10", "Ntx11"))
    ...
    FACTGOG_Ntx13_TOTAL_N <- TOTAL_N
    NtxS13_N <- AC_N
    NtxS13 <- AC
    FACTGOG_Ntx13_TOTAL <- TOTAL
    FACTGOG_Ntx13_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, NtxS13_N, FACTGOG_Ntx13_TOTAL_N, 
            NtxS13, FACTGOG_Ntx13_TOTAL, FACTGOG_Ntx13_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, NtxS13, FACTGOG_Ntx13_TOTAL, 
            FACTGOG_Ntx13_TOI))
    }
    return(dfOut)
} 
