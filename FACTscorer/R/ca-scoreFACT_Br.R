...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Br (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Br 
...
#' data.  As its default, \code{scoreFACT_Br} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Br}.
#' 
#' 
#' @param df A data frame with the FACT-Br items, appropriately-named.
...
#'   \item{BrCs}{Brain Cancer subscale}
#'   \item{FACT_Br_TOTAL}{FACT-Br Total Score (i.e., PWB+SWB+EWB+FWB+BrCS)}
#'   \item{FACT_Br_TOI}{FACT-Br Trial Outcome Index (e.g., PWB+FWB+BrCS)}
#' }
#' 
#' @references FACT-Br Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('Br1', 'Br2', 'Br3', 'Br4', 'Br5', 'Br6', 'Br7', 'Ntx6', 
#' 'Br8', 'Br9', 'Br10', 'Br11', 'Br12', 'Br13', 'Br14', 'Br15', 'Br16', 'Br17', 'Br18')
...
scoreFACT_Br <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("Br1", "Br2", "Br3", "Br4", "Br5", "Br6", "Br7", 
        "Ntx6", "Br8", "Br9", "Br10", "Br11", "Br12", "Br13", "Br14", "Br15", 
        "Br16", "Br17", "Br18"))
    revNames <- toupper(c("Br2", "Br4", "Br5", "Br6", "Ntx6", "Br9", "Br10", 
        "Br12", "Br14"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, BrCs_N, FACT_Br_TOTAL_N, BrCs, 
            FACT_Br_TOTAL, FACT_Br_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, BrCs, FACT_Br_TOTAL, FACT_Br_TOI))
    }
    return(dfOut)
} 
