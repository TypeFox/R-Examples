...
#' 
#' @details
#' Given a data frame that includes all of the FACT-AntiA (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-AntiA 
...
#' data.  As its default, \code{scoreFACT_AntiA} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_AntiA}.
#' 
#' 
#' @param df A data frame with the FACT-AntiA items, appropriately-named.
...
#'   \item{AntiAS}{AntiA subscale}
#'   \item{FACT_AntiA_TOTAL}{FACT-AntiA Total Score (i.e., PWB+SWB+EWB+FWB+AntiAS)}
#'   \item{FACT_AntiA_TOI}{FACT-AntiA Trial Outcome Index (e.g., PWB+FWB+AntiAS)}
#' }
#' 
#' @references FACT-AntiA Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('HI7', 'HI12', 'AA1', 'AA2', 'AA3', 'AA4', 'AA5', 'AA6', 'AA7', 'C5', 
#'  'AA8', 'BMT13', 'BRM5', 'MS3', 'BRM1', 'Cx6', 'AA9', 'TH5', 'B5', 'AA10', 
#'  'GA1', 'B1', 'O2')
...
scoreFACT_AntiA <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("HI7", "HI12", "AA1", "AA2", "AA3", "AA4", "AA5", 
        "AA6", "AA7", "C5", "AA8", "BMT13", "BRM5", "MS3", "BRM1", "Cx6", 
        "AA9", "TH5", "B5", "AA10", "GA1", "B1", "O2"))
    revNames <- toupper(c("HI7", "HI12", "AA1", "AA2", "AA3", "AA4", "AA5", 
        "AA6", "AA7", "C5", "AA8", "BMT13", "BRM5", "MS3", "BRM1", "Cx6", 
        "AA9", "TH5", "B5", "AA10", "GA1", "B1", "O2"))
    ...
    FACT_AntiA_TOTAL_N <- TOTAL_N
    AntiAS_N <- AC_N
    AntiAS <- AC
    FACT_AntiA_TOTAL <- TOTAL
    FACT_AntiA_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, AntiAS_N, FACT_AntiA_TOTAL_N, 
            AntiAS, FACT_AntiA_TOTAL, FACT_AntiA_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, AntiAS, FACT_AntiA_TOTAL, FACT_AntiA_TOI))
    }
    return(dfOut)
} 
