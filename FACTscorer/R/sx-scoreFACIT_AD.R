...
#' 
#' @details
#' Given a data frame that includes all of the FACIT-AD (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACIT-AD 
...
#' data.  As its default, \code{scoreFACIT_AD} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACIT_AD}.
#' 
#' 
#' @param df A data frame with the FACIT-AD items, appropriately-named.
...
#'   \item{ADS}{Abdominal Discomfort subscale}
#'   \item{FACIT_AD_TOTAL}{FACIT-AD Total Score (i.e., PWB+SWB+EWB+FWB+ADS)}
#'   \item{FACIT_AD_TOI}{FACIT-AD Trial Outcome Index (e.g., PWB+FWB+ADS)}
#' }
#' 
#' @references FACIT-AD Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACIT_AD <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("O3", "ACT11", "AD1"))
    revNames <- toupper(c("O3", "ACT11", "AD1"))
    ...
    FACIT_AD_TOTAL_N <- TOTAL_N
    ADS_N <- AC_N
    ADS <- AC
    FACIT_AD_TOTAL <- TOTAL
    FACIT_AD_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ADS_N, FACIT_AD_TOTAL_N, ADS, 
            FACIT_AD_TOTAL, FACIT_AD_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ADS, FACIT_AD_TOTAL, FACIT_AD_TOI))
    }
    return(dfOut)
} 
