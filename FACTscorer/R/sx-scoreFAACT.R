...
#' 
#' @details
#' Given a data frame that includes all of the FAACT (Version 4) items as 
#' variables, appropriately named, this function generates all of the FAACT 
...
#' data.  As its default, \code{scoreFAACT} DOES NOT replace any of your
...
#' frame returned by \code{scoreFAACT}.
#' 
#' 
#' @param df A data frame with the FAACT items, appropriately-named.
...
#'   \item{ACS}{Anorexia Cachexia subscale}
#'   \item{FAACT_TOTAL}{FAACT Total Score (i.e., PWB+SWB+EWB+FWB+ACS)}
#'   \item{FAACT_TOI}{FAACT Trial Outcome Index (e.g., PWB+FWB+ACS)}
#' }
#' 
#' @references FAACT Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFAACT <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("C6", "ACT1", "ACT2", "ACT3", "ACT4", "ACT6", 
        "ACT7", "ACT9", "O2", "ACT10", "ACT11", "ACT13"))
    revNames <- toupper(c("ACT2", "ACT3", "ACT4", "ACT6", "ACT7", "ACT9", 
        "O2", "ACT10", "ACT11"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ACS_N, FAACT_TOTAL_N, ACS, FAACT_TOTAL, 
            FAACT_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ACS, FAACT_TOTAL, FAACT_TOI))
    }
    return(dfOut)
} 
