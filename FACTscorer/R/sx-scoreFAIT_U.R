...
#' 
#' @details
#' Given a data frame that includes all of the FAIT-U (Version 4) items as 
#' variables, appropriately named, this function generates all of the FAIT-U 
...
#' data.  As its default, \code{scoreFAIT_U} DOES NOT replace any of your
...
#' frame returned by \code{scoreFAIT_U}.
#' 
#' 
#' @param df A data frame with the FAIT-U items, appropriately-named.
...
#'   \item{UIS}{Urinary Incontinence subscale}
#'   \item{FAIT_U_TOTAL}{FAIT-U Total Score (i.e., PWB+SWB+EWB+FWB+UIS)}
#'   \item{FAIT_U_TOI}{FAIT-U Trial Outcome Index (e.g., PWB+FWB+UIS)}
#' }
#' 
#' @references FAIT-U Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('BL1', 'ITU1', 'BL2', 'ITU2', 'ITU3', 'ITU4', 'ITU5', 'ITU6', 'ITU7', 'ITU8', 'ITU9')
...
scoreFAIT_U <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("BL1", "ITU1", "BL2", "ITU2", "ITU3", "ITU4", 
        "ITU5", "ITU6", "ITU7", "ITU8", "ITU9"))
    revNames <- toupper(c("BL1", "BL2", "ITU2", "ITU3", "ITU4", "ITU5", 
        "ITU6", "ITU7", "ITU8", "ITU9"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, UIS_N, FAIT_U_TOTAL_N, UIS, FAIT_U_TOTAL, 
            FAIT_U_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, UIS, FAIT_U_TOTAL, FAIT_U_TOI))
    }
    return(dfOut)
} 
