...
#' 
#' @details
#' Given a data frame that includes all of the FACT-L (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-L 
...
#' data.  As its default, \code{scoreFACT_L} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_L}.
#' 
#' 
#' @param df A data frame with the FACT-L items, appropriately-named.
...
#'   \item{LCS}{Lung Cancer subscale}
#'   \item{FACT_L_TOTAL}{FACT-L Total Score (i.e., PWB+SWB+EWB+FWB+LCS)}
#'   \item{FACT_L_TOI}{FACT-L Trial Outcome Index (e.g., PWB+FWB+LCS)}
#' }
#' 
#' @references FACT-L Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('B1', 'C2', 'L1', 'L2', 'C6', 'L3', 'L4')
...
scoreFACT_L <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, LCS_N, FACT_L_TOTAL_N, LCS, FACT_L_TOTAL, 
            FACT_L_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, LCS, FACT_L_TOTAL, FACT_L_TOI))
    }
    return(dfOut)
} 
