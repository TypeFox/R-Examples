...
#' 
#' 
#' @param df A data frame with the FACT-An items, appropriately-named.
...
#' }
#' 
#' @references FACT-An Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_An <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    AC_names <- toupper(c("HI7", "HI12", "An1", "An2", "An3", "An4", "An5", 
        ...
        "An13", "An14", "An15", "An16"))
    revNames <- toupper(c("HI7", "HI12", "An1", "An2", "An3", "An4", "An6", 
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, AnS_N, FACT_An_TOTAL_N, AnS, 
            FACT_An_TOTAL, FACT_An_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, AnS, FACT_An_TOTAL, FACT_An_TOI))
    }
    return(dfOut)
} 
