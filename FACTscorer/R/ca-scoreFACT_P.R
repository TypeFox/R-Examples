...
#' 
#' @details
#' Given a data frame that includes all of the FACT-P (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-P 
...
#' data.  As its default, \code{scoreFACT_P} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_P}.
#' 
#' 
#' @param df A data frame with the FACT-P items, appropriately-named.
...
#'   \item{PCS}{Prostate Cancer subscale}
#'   \item{FACT_P_TOTAL}{FACT-P Total Score (i.e., PWB+SWB+EWB+FWB+PCS)}
#'   \item{FACT_P_TOI}{FACT-P Trial Outcome Index (e.g., PWB+FWB+PCS)}
#' }
#' 
#' @references FACT-P Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_P <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("C2", "C6", "P1", "P2", "P3", "P4", "P5", "P6", 
        "P7", "BL2", "P8", "BL5"))
    revNames <- toupper(c("C2", "P1", "P2", "P3", "P6", "P7", "BL2", "P8"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, PCS_N, FACT_P_TOTAL_N, PCS, FACT_P_TOTAL, 
            FACT_P_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, PCS, FACT_P_TOTAL, FACT_P_TOI))
    }
    return(dfOut)
} 
