...
#' 
#' @details
#' Given a data frame that includes all of the FACT-E (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-E 
...
#' data.  As its default, \code{scoreFACT_E} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_E}.
#' 
#' 
#' @param df A data frame with the FACT-E items, appropriately-named.
...
#'   \item{ECS}{Esophagus Cancer subscale}
#'   \item{FACT_E_TOTAL}{FACT-E Total Score (i.e., PWB+SWB+EWB+FWB+ECS)}
#'   \item{FACT_E_TOI}{FACT-E Trial Outcome Index (e.g., PWB+FWB+ECS)}
#' }
#' 
#' @references FACT-E Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('HN1', 'HN2', 'HN3', 'HN4', 'HN5', 'HN10', 'HN7', 'E1', 'E2', 'E3', 'E4',
#'    'E5', 'E6', 'C6', 'E7', 'ACT11', 'C2')
...
scoreFACT_E <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("HN1", "HN2", "HN3", "HN4", "HN5", "HN10", "HN7", 
        "E1", "E2", "E3", "E4", "E5", "E6", "C6", "E7", "ACT11", "C2"))
    revNames <- toupper(c("HN2", "HN3", "E1", "E2", "E3", "E4", "E5", "E7", 
        "ACT11", "C2"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ECS_N, FACT_E_TOTAL_N, ECS, FACT_E_TOTAL, 
            FACT_E_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ECS, FACT_E_TOTAL, FACT_E_TOI))
    }
    return(dfOut)
} 
