#' @title Score the FANLTC
#' 
#' @description
#' Generates all of the scores of the Functional Assessment of Cancer 
#' Therapy-General (FANLTC) from item responses.
#' 
#' @details
#' Given a data frame that includes all of the FANLTC items as variables,
#' appropriately named, this function generates all of the FANLTC scale
...
#' data.  As its default, \code{scoreFANLTC} DOES NOT replace any of your
...
#' frame returned by \code{scoreFANLTC}.
#' 
#' 
#' @param df A data frame with the FANLTC items, appropriately-named.
...
#'   \item{FANLTC}{FANLTC Total Score (i.e., PWB+SWB+EWB+FWB)}
#' }
#' 
#' @references FANLTC Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFANLTC <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    dfup <- df
    names(dfup) <- toupper(names(df))
    items <- dfup[, c("GP1", "GP2", "GP3", "GP4", "GP5", "GP6", "GP7", 
        "GS1", "GS2", "GS3", "GS4", "GS5", "GS6", "GS7", "GE1", "GE2", 
        "GE3", "GE4", "GE6", "GF1", "GF2", "GF3", "GF4", "GF5", "GF6", 
        "GF7")]
    ...
    EWB_names <- c("GE1", "GE2", "GE3", "GE4", "GE6")
    ...
    FANLTC_N <- PWB_N + SWB_N + EWB_N + FWB_N
    ...
    FANLTC <- round(PWB + SWB + EWB + FWB, 3)
    FANLTC[FANLTC_N/ncol(items) <= 0.8] <- NA
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(df, PWB_N, SWB_N, EWB_N, FWB_N, FANLTC_N, 
            PWB, SWB, EWB, FWB, FANLTC))
    } else {
        dfOut <- as.data.frame(cbind(df, PWB, SWB, EWB, FWB, FANLTC))
    }
    return(dfOut)
} 
