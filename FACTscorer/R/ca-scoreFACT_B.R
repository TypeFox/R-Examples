...
#' 
#' @details
#' Given a data frame that includes all of the FACT-B (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-B 
...
#' data.  As its default, \code{scoreFACT_B} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_B}.
#' 
#' 
#' @param df A data frame with the FACT-B items, appropriately-named.
...
#' }
#' 
#' @references FACT-B Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_B <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8", 
        "B9", "P2"))
    revNames <- toupper(c("B1", "B2", "B3", "B5", "B6", "B7", "B8", "P2"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, BCS_N, FACT_B_TOTAL_N, BCS, FACT_B_TOTAL, 
            FACT_B_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, BCS, FACT_B_TOTAL, FACT_B_TOI))
    }
    return(dfOut)
} 
