...
#' 
#' @details
#' Given a data frame that includes all of the FACT-N (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-N 
...
#' data.  As its default, \code{scoreFACT_N} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_N}.
#' 
#' 
#' @param df A data frame with the FACT-N items, appropriately-named.
...
#'   \item{NS}{Neutropenia subscale}
#'   \item{FACT_N_TOTAL}{FACT-N Total Score (i.e., PWB+SWB+EWB+FWB+NS)}
#'   \item{FACT_N_TOI}{FACT-N Trial Outcome Index (e.g., PWB+FWB+NS)}
#' }
#' 
#' @references FACT-N Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('N1', 'N2', 'P1', 'An14', 'N3', 'N4', 'An5', 'BRM3', 'BRM2', 'ES3', 'An16', 'N5', 
#'   'An1', 'An13', 'N6', 'N7', 'N8', 'An3', 'MS3')
...
scoreFACT_N <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("N1", "N2", "P1", "An14", "N3", "N4", "An5", 
        "BRM3", "BRM2", "ES3", "An16", "N5", "An1", "An13", "N6", "N7", 
        "N8", "An3", "MS3"))
    revNames <- toupper(c("N1", "N2", "P1", "An14", "N3", "N4", "BRM3", 
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, NS_N, FACT_N_TOTAL_N, NS, FACT_N_TOTAL, 
            FACT_N_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, NS, FACT_N_TOTAL, FACT_N_TOI))
    }
    return(dfOut)
} 
