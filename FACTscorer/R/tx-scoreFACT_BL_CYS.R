...
#' 
#' @details
#' Given a data frame that includes all of the FACT-BL-CYS (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-BL-CYS 
...
#' data.  As its default, \code{scoreFACT_BL-CYS} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_BL-CYS}.
#' 
#' 
#' @param df A data frame with the FACT-BL-CYS items, appropriately-named.
...
#'   \item{BL_CYS}{BL-CYS subscale}
#'   \item{FACT_BL_CYS_TOTAL}{FACT-BL-CYS Total Score (i.e., PWB+SWB+EWB+FWB+BL_CYS)}
#'   \item{FACT_BL_CYS_TOI}{FACT-BL-CYS Trial Outcome Index (e.g., PWB+FWB+BL_CYS)}
#' }
#' 
#' @references FACT-BL-CYS Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_BL_CYS <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("C2", "C3", "C4", "C6", "C7", "BL1", "ITU7", 
        "ITU6", "C9", "ITU3", "ITU4", "ITU5", "ITU1", "VC1", "ITU2"))
    revNames <- toupper(c("C2", "BL1", "ITU7", "ITU6", "C9", "ITU3", "ITU4", 
        "ITU5", "ITU2"))
    ...
    FACT_BL_CYS_TOTAL_N <- TOTAL_N
    BL_CYS_N <- AC_N
    BL_CYS <- AC
    FACT_BL_CYS_TOTAL <- TOTAL
    FACT_BL_CYS_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, BL_CYS_N, FACT_BL_CYS_TOTAL_N, 
            BL_CYS, FACT_BL_CYS_TOTAL, FACT_BL_CYS_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, BL_CYS, FACT_BL_CYS_TOTAL, FACT_BL_CYS_TOI))
    }
    return(dfOut)
} 
