...
#' data.  As its default, \code{scoreFACT_NP} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_NP}.
#' 
#' 
#' @param df A data frame with the FACT-NP items, appropriately-named.
...
#'   \item{NPS}{Nasopharyngeal Cancer subscale}
#'   \item{FACT_NP_TOTAL}{FACT-NP Total Score (i.e., PWB+SWB+EWB+FWB+NPS)}
#'   \item{FACT_NP_TOI}{FACT-NP Trial Outcome Index (e.g., PWB+FWB+NPS)}
#' }
#' 
#' @references FACT-NP Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_NP <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("HN1", "HN2", "HN4", "HN5", "HN7", "HN6", "HN10", 
        "HN11", "HN12", "NP1", "NP2", "NTX6", "NP3", "NP4", "NP5", "NP6"))
    revNames <- toupper(c("HN2", "HN6", "HN12", "NP1", "NP2", "NTX6", "NP3", 
        "NP4", "NP6"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, NPS_N, FACT_NP_TOTAL_N, NPS, 
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, NPS, FACT_NP_TOTAL, FACT_NP_TOI))
    }
    return(dfOut)
} 
