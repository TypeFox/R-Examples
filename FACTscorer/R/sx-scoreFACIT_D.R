...
#' 
#' @details
#' Given a data frame that includes all of the FACIT-D (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACIT-D 
...
#' data.  As its default, \code{scoreFACIT_D} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACIT_D}.
#' 
#' 
#' @param df A data frame with the FACIT-D items, appropriately-named.
...
#'   \item{ADS}{Diarrhea subscale}
#'   \item{FACIT_D_TOTAL}{FACIT-D Total Score (i.e., PWB+SWB+EWB+FWB+ADS)}
#'   \item{FACIT_D_TOI}{FACIT-D Trial Outcome Index (e.g., PWB+FWB+ADS)}
#' }
#' 
#' @references FACIT-D Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACIT_D <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("C3", "ITF1", "ITU2", "D1", "D2", "D3", "D4", 
        "D5", "D6", "ITF3", "ITF5"))
    revNames <- toupper(c("ITF1", "ITU2", "D1", "D2", "D3", "D4", "D5", 
        "D6", "ITF3", "ITF5"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ADS_N, FACIT_D_TOTAL_N, ADS, 
            FACIT_D_TOTAL, FACIT_D_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ADS, FACIT_D_TOTAL, FACIT_D_TOI))
    }
    return(dfOut)
} 
