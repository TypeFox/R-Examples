...
#' data.  As its default, \code{scoreFACTG} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACTG}.
#'
#'
#' @param df A data frame with the FACT-G items, appropriately-named.
...
scoreFACTG <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    dfup <- df
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(df, PWB_N, SWB_N, EWB_N, FWB_N, FACTG_N,
            PWB, SWB, EWB, FWB, FACTG))
    } else {
        dfOut <- as.data.frame(cbind(df, PWB, SWB, EWB, FWB, FACTG))
    }
    return(dfOut)
}
