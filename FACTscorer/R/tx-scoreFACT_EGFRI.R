...
#' 
#' @details
#' Given a data frame that includes all of the FACT-EGFRI items as variables,
#' appropriately named, this function generates all of the FACT-EGFRI scale
...
#' named ST4, the second ST5, and so on.  Please refer to the materials provided 
...
#' data.  As its default, \code{scoreFACT_EGFRI} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_EGFRI}.
#' 
#' 
...
#' summary(scoredDat[, c('FACT_EGFRI')])
scoreFACT_EGFRI <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    items <- df[, c("ST4", "ST5", "ST6", "ST7", "ST9", "ST32", "ST22", 
    ...
    EGFRI_names <- c("ST4", "ST5", "ST6", "ST7", "ST9", "ST32", "ST22", 
        "ST17", "ST24", "ST37", "ST26", "ST34", "ST38", "ST16", "ST15", 
        "ST29", "B5", "ST11")
    revNames <- c("ST4", "ST5", "ST6", "ST7", "ST9", "ST32", "ST22", "ST17", 
    ...
    EGFRI_N <- rowSums(valid_N[, EGFRI_names])
    FACT_EGFRI_N <- EGFRI_N
    FACT_EGFRI <- round(rowMeans(items[, EGFRI_names], na.rm = TRUE) * 
        length(EGFRI_names), 3)
    FACT_EGFRI[FACT_EGFRI_N/length(EGFRI_names) <= 0.5] <- NA
    if (updateItems) {
        df[, names(items)] <- items
    }
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(df, FACT_EGFRI_N, FACT_EGFRI))
    } else {
        dfOut <- as.data.frame(cbind(df, FACT_EGFRI))
    }
    return(dfOut)
} 
