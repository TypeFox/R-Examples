...
#' of Cancer Therapy- Bone Pain (FACT-BP) from item responses.
#'
#' @details
#' Given a data frame that includes all of the FACT-BP items as variables,
#' appropriately named, this function generates all of the FACT-BP scale
#' scores.  It is crucial that the item variables in the supplied data frame
#' are named according to FACT conventions.  For example, the first bone pain item
#' should be named Q7, the second GF7, and so on.  Please
...
#' data.  As its default, \code{scoreFACT_BP} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_BP}.
#'
#'
#' @param df A data frame with the FACT-BP items, appropriately-named.
...
#'
#' \describe{
#'   \item{BPS}{Bone Pain subscale}
#' }
#'
#' @references FACT-BP Scoring Guidelines, available at \url{http://www.facit.org}
#'
#' @export
#'
#' @examples
...
scoreFACT_BP <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    dfup <- df
    names(dfup) <- toupper(names(df))
    items <- df[, c("GF7", "P2", "BP1", "BP2", "BP3", "BP4", "BP5", "BP6",
    ...
    BPS_names <- c("GF7", "P2", "BP1", "BP2", "BP3", "BP4", "BP5", "BP6",
        "BP7", "BP8", "BP9", "BP10", "BP11", "BP12", "BP13")
    revNames <- c("P2", "BP1", "BP2", "BP3", "BP4", "BP5", "BP6", "BP7",
        "BP8", "BP9", "BP10", "BP11", "BP12", "BP13")
    ...
    BPS_N <- rowSums(valid_N[, BPS_names])
    FACT_BP_N <- BPS_N
    BPS <- round(rowMeans(items[, BPS_names], na.rm = TRUE) * length(BPS_names),
        3)
    BPS[BPS_N/length(BPS_names) <= 0.5] <- NA
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(df, BPS_N, BPS))
    } else {
        dfOut <- as.data.frame(cbind(df, BPS))
    }
    return(dfOut)
}
