...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Hep (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Hep 
...
#' data.  As its default, \code{scoreFACT_Hep} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Hep}.
#' 
#' 
#' @param df A data frame with the FACT-Hep items, appropriately-named.
...
#'   \item{HCS}{Hepatobiliary Cancer subscale}
#'   \item{FACT_Hep_TOTAL}{FACT-Hep Total Score (i.e., PWB+SWB+EWB+FWB+HCS)}
#'   \item{FACT_Hep_TOI}{FACT-Hep Trial Outcome Index (e.g., PWB+FWB+HCS)}
#' }
#' 
#' @references FACT-Hep Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_Hep <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("C1", "C2", "C3", "C4", "C5", "C6", "Hep1", "Cns7", 
        "Cx6", "HI7", "An7", "Hep2", "Hep3", "Hep4", "Hep5", "Hep6", "HN2", 
        "Hep8"))
    revNames <- toupper(c("C1", "C2", "C5", "Hep1", "Cns7", "Cx6", "HI7", 
    ...
    FACT_Hep_TOTAL_N <- TOTAL_N
    HCS_N <- AC_N
    HCS <- AC
    FACT_Hep_TOTAL <- TOTAL
    FACT_Hep_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, HCS_N, FACT_Hep_TOTAL_N, HCS, 
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, HCS, FACT_Hep_TOTAL, FACT_Hep_TOI))
    }
    return(dfOut)
} 
