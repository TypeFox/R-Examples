...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Gastric (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Gastric 
...
#' data.  As its default, \code{scoreFACT_Gastric} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Gastric}.
#' 
#' 
#' @param df A data frame with the FACT-Gastric items, appropriately-named.
...
#'   \item{GaCs}{Gastric Cancer subscale}
#'   \item{FACT_Gastric_TOTAL}{FACT-Gastric Total Score (i.e., PWB+SWB+EWB+FWB+GaCs)}
#'   \item{FACT_Gastric_TOI}{FACT-Gastric Trial Outcome Index (e.g., PWB+FWB+GaCs)}
#' }
#' 
#' @references FACT-Gastric Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('C2', 'Ga1', 'Ga2', 'HN1', 'Ga6', 'Ga5', 'C1', 'Ga12', 'Ga4', 'E6',
#'    'Ga10', 'Ga9', 'Ga7', 'Hep8', 'Ga14', 'C5', 'An2', 'HI12', 'Leu4')
...
scoreFACT_Gastric <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("C2", "Ga1", "Ga2", "HN1", "Ga6", "Ga5", "C1", 
        "Ga12", "Ga4", "E6", "Ga10", "Ga9", "Ga7", "Hep8", "Ga14", "C5", 
        "An2", "HI12", "Leu4"))
    revNames <- toupper(c("C2", "Ga1", "Ga2", "Ga6", "Ga5", "C1", "Ga12", 
    ...
    FACT_Gastric_TOTAL_N <- TOTAL_N
    GaCs_N <- AC_N
    GaCs <- AC
    FACT_Gastric_TOTAL <- TOTAL
    FACT_Gastric_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, GaCs_N, FACT_Gastric_TOTAL_N, 
            GaCs, FACT_Gastric_TOTAL, FACT_Gastric_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, GaCs, FACT_Gastric_TOTAL, FACT_Gastric_TOI))
    }
    return(dfOut)
} 
