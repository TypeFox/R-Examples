...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Cx (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Cx 
...
#' data.  As its default, \code{scoreFACT_Cx} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Cx}.
#' 
#' 
#' @param df A data frame with the FACT-Cx items, appropriately-named.
...
#'   \item{CxCs}{Cervix Cancer subscale}
#'   \item{FACT_Cx_TOTAL}{FACT-Cx Total Score (i.e., PWB+SWB+EWB+FWB+CxCs)}
#'   \item{FACT_Cx_TOI}{FACT-Cx Trial Outcome Index (e.g., PWB+FWB+CxCs)}
#' }
#' 
#' @references FACT-Cx Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('Cx1', 'Cx2','Cx3', 'B4', 'Cx4', 'BMT7', 'Cx5', 'BL4', 'C7', 'Cx6', 'C6', 'BL1',
#'    'BL3', 'Cx7', 'HN1')
...
scoreFACT_Cx <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("Cx1", "Cx2", "Cx3", "B4", "Cx4", "BMT7", "Cx5", 
        "BL4", "C7", "Cx6", "C6", "BL1", "BL3", "Cx7", "HN1"))
    revNames <- toupper(c("Cx1", "Cx2", "Cx3", "Cx4", "BMT7", "Cx5", "Cx6", 
        "BL1", "BL3", "Cx7"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, CxCs_N, FACT_Cx_TOTAL_N, CxCs, 
            FACT_Cx_TOTAL, FACT_Cx_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, CxCs, FACT_Cx_TOTAL, FACT_Cx_TOI))
    }
    return(dfOut)
} 
