...
#' data.  As its default, \code{scoreFACT_M} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_M}.
#' 
#' 
#' @param df A data frame with the FACT-M items, appropriately-named.
...
#'   \item{MS}{Melanoma subscale}
#'  \item{MSS}{Melanoma Surgery Scale}
#'   \item{FACT_M_TOTAL}{FACT-M Total Score (i.e., PWB+SWB+EWB+FWB+MS+MSS)}
#'   \item{FACT_M_TOI}{FACT-M Trial Outcome Index (e.g., PWB+FWB+MS+MSS)}
#' }
#' 
#' @references FACT-M Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_M <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names1 <- toupper(c("M1", "M2", "M3", "B1", "ITU4", "An10", "Hep3", 
        "C1", "C6", "M5", "M6", "ITU3", "MS8", "M8", "M9", "HI7"))
    AC_names2 <- toupper(c("M10", "M11", "M12", "M13", "M14", "M15", "M16", 
        "M17"))
    AC_names <- c(AC_names1, AC_names2)
    revNames <- toupper(c("M1", "M2", "M3", "B1", "ITU4", "An10", "Hep3", 
        "C1", "M5", "M6", "ITU3", "MS8", "M8", "M9", "HI7", "M10", "M11", 
        "M12", "M13", "M14", "M15", "M16"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, MS_N, MSS_N, FACT_M_TOTAL_N, 
            MS, MSS, FACT_M_TOTAL, FACT_M_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, MS, MSS, FACT_M_TOTAL, FACT_M_TOI))
    }
    return(dfOut)
} 
