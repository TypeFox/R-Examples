...
#'
#' @details
#' Given a data frame that includes all of the FACT-HN (Version 4) items as
#' variables, appropriately named, this function generates all of the FACT-HN
...
#' data.  As its default, \code{scoreFACT_HN} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_HN}.
#'
#'
#' @param df A data frame with the FACT-HN items, appropriately-named.
...
#'   \item{HNCS}{Head and Neck Cancer subscale}
#'   \item{FACT_HN_TOTAL}{FACT-HN Total Score (i.e., PWB+SWB+EWB+FWB+HNCS)}
#'   \item{FACT_HN_TOI}{FACT-HN Trial Outcome Index (e.g., PWB+FWB+HNCS)}
#' }
#'
#' @references FACT-HN Scoring Guidelines, available at \url{http://www.facit.org}
#'
#' @export
#'
#' @examples
...
#'              paste0('GE', 1:6),
#'              paste0('GF', 1:7))
#' AC_names <- c('HN1', 'HN2', 'HN3', 'HN4', 'HN5', 'HN6', 'HN7', 'HN10', 'HN11', 'HN12')
...
scoreFACT_HN <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6),
        paste0("GF", 1:7))
    AC_names <- toupper(c("HN1", "HN2", "HN3", "HN4", "HN5", "HN6", "HN7",
        "HN10", "HN11", "HN12"))
    revNames <- toupper(c("HN2", "HN3", "HN6", "HN12"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, HNCS_N, FACT_HN_TOTAL_N, HNCS,
            FACT_HN_TOTAL, FACT_HN_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, HNCS, FACT_HN_TOTAL, FACT_HN_TOI))
    }
    return(dfOut)
}
