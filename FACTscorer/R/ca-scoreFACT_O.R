...
#' 
#' @details
#' Given a data frame that includes all of the FACT-O (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-O 
...
#' data.  As its default, \code{scoreFACT_O} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_O}.
#' 
#' 
#' @param df A data frame with the FACT-O items, appropriately-named.
...
#'   \item{OCS}{Ovarian Cancer subscale}
#'   \item{FACT_O_TOTAL}{FACT-O Total Score (i.e., PWB+SWB+EWB+FWB+OCS)}
#'   \item{FACT_O_TOI}{FACT-O Trial Outcome Index (e.g., PWB+FWB+OCS)}
#' }
#' 
#' @references FACT-O Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('O1', 'C2', 'C3', 'O2', 'B5', 'C6', 'C7', 'BMT5', 'B9', 'O3', 'BL4')
...
scoreFACT_O <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("O1", "C2", "C3", "O2", "B5", "C6", "C7", "BMT5", 
        "B9", "O3", "BL4"))
    revNames <- toupper(c("O1", "C2", "O2", "B5", "O3"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, OCS_N, FACT_O_TOTAL_N, OCS, FACT_O_TOTAL, 
            FACT_O_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, OCS, FACT_O_TOTAL, FACT_O_TOI))
    }
    return(dfOut)
} 
