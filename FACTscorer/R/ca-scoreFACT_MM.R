...
#' 
#' @details
#' Given a data frame that includes all of the FACT-MM (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-MM 
...
#' data.  As its default, \code{scoreFACT_MM} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_MM}.
#' 
#' 
#' @param df A data frame with the FACT-MM items, appropriately-named.
...
#'   \item{MMS}{Multiple Myeloma subscale}
#'   \item{FACT_MM_TOTAL}{FACT-MM Total Score (i.e., PWB+SWB+EWB+FWB+MMS)}
#'   \item{FACT_MM_TOI}{FACT-MM Trial Outcome Index (e.g., PWB+FWB+MMS)}
#' }
#' 
#' @references FACT-MM Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('P2', 'HI12', 'BMT6', 'HI8', 'N3', 'LEU3', 'LEU4', 'LEU6',
#' 'BRM9', 'BP1', 'An14', 'MM1', 'HI7', 'ES10')
...
scoreFACT_MM <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("P2", "HI12", "BMT6", "HI8", "N3", "LEU3", "LEU4", 
        "LEU6", "BRM9", "BP1", "An14", "MM1", "HI7", "ES10"))
    revNames <- toupper(c("P2", "HI12", "BMT6", "HI8", "N3", "LEU3", "LEU4", 
        "LEU6", "BRM9", "BP1", "An14", "MM1", "HI7", "ES10"))
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, MMS_N, FACT_MM_TOTAL_N, MMS, 
            FACT_MM_TOTAL, FACT_MM_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, MMS, FACT_MM_TOTAL, FACT_MM_TOI))
    }
    return(dfOut)
} 
