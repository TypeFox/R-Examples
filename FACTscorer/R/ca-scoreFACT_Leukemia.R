...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Leukemia (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Leukemia 
...
#' data.  As its default, \code{scoreFACT_Leukemia} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACT_Leukemia}.
#' 
#' 
#' @param df A data frame with the FACT-Leukemia items, appropriately-named.
...
#'   \item{LEUS}{Leukemia subscale}
#'   \item{FACT_Leukemia_TOTAL}{FACT-Leukemia Total Score (i.e., PWB+SWB+EWB+FWB+LEUS)}
#'   \item{FACT_Leukemia_TOI}{FACT-Leukemia Trial Outcome Index (e.g., PWB+FWB+LEUS)}
#' }
#' 
#' @references FACT-Leukemia Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('BRM3', 'P2', 'BRM2', 'ES3', 'LEU1', 'TH1', 'TH2', 'HI12', 'BMT6', 'C2', 'C6',
#'    'An7', 'N3', 'LEU5', 'LEU6', 'BRM9', 'LEU7')
...
scoreFACT_Leukemia <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("BRM3", "P2", "BRM2", "ES3", "LEU1", "TH1", "TH2", 
        "HI12", "BMT6", "C2", "C6", "An7", "N3", "LEU5", "LEU6", "BRM9", 
        "LEU7"))
    revNames <- toupper(c("BRM3", "P2", "BRM2", "ES3", "LEU1", "TH1", "TH2", 
    ...
    FACT_Leukemia_TOTAL_N <- TOTAL_N
    LEUS_N <- AC_N
    LEUS <- AC
    FACT_Leukemia_TOTAL <- TOTAL
    FACT_Leukemia_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, LEUS_N, FACT_Leukemia_TOTAL_N, 
            LEUS, FACT_Leukemia_TOTAL, FACT_Leukemia_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, LEUS, FACT_Leukemia_TOTAL, FACT_Leukemia_TOI))
    }
    return(dfOut)
} 
