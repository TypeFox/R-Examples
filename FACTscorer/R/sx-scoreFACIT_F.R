...
#' 
#' @details
#' Given a data frame that includes all of the FACIT-F (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACIT-F 
...
#' data.  As its default, \code{scoreFACIT_F} DOES NOT replace any of your
...
#' frame returned by \code{scoreFACIT_F}.
#' 
#' 
#' @param df A data frame with the FACIT-F items, appropriately-named.
...
#'   \item{FACIT_F_TOTAL}{FACIT-F Total Score (i.e., PWB+SWB+EWB+FWB+FS)}
#'   \item{FACIT_F_TOI}{FACIT-F Trial Outcome Index (e.g., PWB+FWB+FS)}
#' }
#' 
#' @references FACIT-F Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'              paste0('GE', 1:6), 
#'              paste0('GF', 1:7))
#' AC_names <- c('HI7', 'HI12', 'An1', 'An2', 'An3', 'An4', 'An5', 'An7', 'An8',
#'    'An12', 'An14', 'An15', 'An16')
...
scoreFACIT_F <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("HI7", "HI12", "An1", "An2", "An3", "An4", "An5", 
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, FS_N, FACIT_F_TOTAL_N, FS, FACIT_F_TOTAL, 
            FACIT_F_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, FS, FACIT_F_TOTAL, FACIT_F_TOI))
    }
    return(dfOut)
} 
