...
#' 
#' 
#' @param df A data frame with the FACT-V items, appropriately-named.
...
#' }
#' 
#' @references FACT-V Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
scoreFACT_V <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    G_names <- c(paste0("GP", 1:7), paste0("GS", 1:7), paste0("GE", 1:6), 
        paste0("GF", 1:7))
    AC_names <- toupper(c("V1", "V2", "Cx3", "V3", "Cx4", "V4", "Cx5", 
        "BL4", "C7", "Cx6", "C6", "BL1", "V5", "Cx7", "V6", "V7", "V8", 
        "V9", "HN1"))
    revNames <- toupper(c("V1", "V2", "Cx3", "V3", "Cx4", "V4", "Cx5", 
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, VCS_N, FACT_V_TOTAL_N, VCS, FACT_V_TOTAL, 
            FACT_V_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        ...
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, VCS, FACT_V_TOTAL, FACT_V_TOI))
    }
    return(dfOut)
} 
