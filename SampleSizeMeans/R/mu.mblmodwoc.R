...
function(len,alpha,beta,level=0.95,worst.level=0.95,m=50000,mcs=3)
{
  ...
  step <- ceiling(log(mu.modwoc(len,alpha,beta,0,level,worst.level))/log(2))
  ...
  while(step>=1)
  {
    while(sign(factor*(threshold-quantity.to.measure)) == direction && step >= 1)
    {
      ...
      ns2 <- rgg(m,alpha,2*beta,(n1-1)/2)
      worst.ns2 <- sort(ns2)[worst.level*m]

      quantity.to.measure <- 2*pt(len/2*sqrt((n1-1)*n1/worst.ns2),n1-1)-1
