...
function(len,alpha,beta,level=0.95,worst.level=0.95,m=50000,mcs=3)
{
  ...
  #**************************************************************************
  # Define initial step
  # (as a function of any frequentist sample size)
  step <- ceiling(log(mudiff.modwoc.equalvar(len,alpha,beta,1,1,level,worst.level)[1])
               /log(2))
  ...
  while(step>=1)
  {
    while(sign(factor*(threshold-quantity.to.measure)) == direction && step >= 1)
    {
      ...
      sp <- sort(sp)[worst.level*m]

      # Estimated worst coverage is

      quantity.to.measure <- 2*pt(len/2*sqrt(n1*(n1-1)/sp),2*(n1-1))-1
