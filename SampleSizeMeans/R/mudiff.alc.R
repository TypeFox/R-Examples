...
function(len,alpha1,beta1,alpha2,beta2,n01,n02,level=0.95,equal=TRUE,m=10000,mcs=3)
{
  ...
  while(step>=1)
  {
    while(sign(factor*(threshold-quantity.to.measure)) == direction && step >= 1)
    {
      ...
      # Here,level is the probability that the true parameter falls
      # in the confidence interval
      # (thus length changes from a sample to another)
      ...
      # Return the resulting average length

      quantity.to.measure <- 2*mean(sqrt(dn))*qnorm((level+1)/2)
