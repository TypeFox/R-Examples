...
print(c("Overall small EM : run ", iter))

#conds = L
n.comp<-K
n.init=t1
for (j in 1:q){
#print(c("1st small EM: initializion condition ",j) )
respj<-y[,index[j]:(index[j]+L[j]-1)]
...
n.init=t1
for (j in 1:q){
#print(c("initializing condition ",j) )
respj<-y[,index[j]:(index[j]+L[j]-1)]
if (L[j]==1)dim(respj)<-c(length(respj),1)
print(c("Sub-Small EM for the initializion of condition ",j))
condsj = c(L[j])
