ss.aipe.smd.upper <- function(delta, conf.level, width, ...)
{
alpha <- 1-conf.level
...
# Initial half-width for upper limit.
Diff.width.Upper.Bound <- abs(Limits.0$Upper - delta) - width

while(Diff.width.Upper.Bound > 0)
{
n <- n + 1
lambda <- delta*sqrt(n/2)
Limits <- ci.smd(ncp=lambda, n.1=n, n.2=n, conf.level=1-alpha)
Current.Upper.width <- abs(Limits$Upper-delta)
Diff.width.Upper.Bound <- Current.Upper.width - width
}
return(n)
}
