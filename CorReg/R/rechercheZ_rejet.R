# ' recherche de structure (mode rejet)
# '@param X
# '@param Z Z est une matrice nulle si on ne lui a pas mis de valeur
# '@param bic_vide_vect vecteur BIC de la matrice nulle
# '@param methode_tirage 0:ligne et colonne,-1:seulement la colonne,-2:tout les candidats, entier>0:nombre aleatoire de candidats
...
# '@param nb_opt_max nombre maximum de fois ou on reste stationnaire
# '@param Mixmod
# '@return etape 0:suppression,1 ajout,2 stationarite
# '
rechercheZ_rejet<-function(X=X,Z=NULL,bic_vide_vect=bic_vide_vect,methode_tirage=0,methode_BIC=1,Rmax=5,p2max=NULL,Maxiter=Maxiter,plot=F,best=T,better=F,random=T,bla=1,nb_opt_max=NULL,Mixmod=T,exact=F){
if(is.null(p2max)){
  p2max=ncol(X)+1 
}
