deletion3 <- function(string){
  ...
  bucket<-append(bucket,unique(as.vector(sapply(as.vector(sapply(as.vector(sapply(string, function(x) paste0(str_sub(x,1,i-1),str_sub(x,i+1,nchar(x))))), function(x) paste0(str_sub(x,1,j-1),str_sub(x,j+1,nchar(x))))), function(x) paste0(str_sub(x,1,k-1),str_sub(x,k+1,nchar(x)))))))
  bucket
}
