...
#get non-adaptive fit 
  ...
  aspm.obj <- aspmOS(spm.info, random, group, family, spar.method, omit.missing,correlation=correlation,weights=weights,control=control)
...
#redefine correlation matrix in case of correlated errors
  ...
  else if(!is.null(aspm.obj$fit$sigma)) y.cov <- aspm.obj$fit$sigma^2*diag(rep(1,nrow(Xb)))
  else y.cov <- NULL
...
#iterate for the next estimate of the variance of random effects
	...
	#get the new estimate for the spline coefficients
      ...
      theta.obj <- aspGetThetaOS(theta.info, niter.var, family, weights,spar.method,returnFit,tol.theta=tol.theta)
	...
	#obtain remaining estimate with the estimated variance of random effects
      ...
      aspm.obj <- aspmOS(spm.info, random, group, family, spar.method, omit.missing, Si.b = ran.var, weights = weights,correlation=correlation,control=control)
      if (family != "gaussian") 
...
#extract objects for the output related to the adaptive fit
    ...
    if(!is.null(correlation)) y.cov <- aspm.obj$fit$sigma^2*corMatrix(aspm.obj$fit$modelStruct$corStruct)
    else                      y.cov <- aspm.obj$fit$sigma^2*diag(rep(1,nrow(Xb)))
