                     ...
                     ask = TRUE, ...) 
{
  ...
  switch(type, simultaneous = {
           if (what == "signal") {
             ...
             legend("topleft", legend = c("Reference", "Sample", 
                                   "Warped sample"), lty = rep(1, 3), col = 1:3)
           }
        if (what == "function") {
          ...
          matplot(time, t(w.time), type = "n", xlab = "'Time'", 
                  ylab = ylbl, ...)
          abline(h = 0, col = "gray", ...)
          matlines(t(w.time), lty = 1, type = "l", col = rainbow(nrow(x$warp.fun)), 
                   ...)
        }
         }, {
           ...
           if (what == "signal") {
             for (i in 1:nrow(x$sample)) {
               if (nrow(x$reference) == 1) {
                 matplot(cbind(x$reference[1, ], x$sample[i, 
                                                          ], x$warped.sample[i, ]), type = "n", xlab = "'Time'", 
                         ylab = "Signal", ...)
               } else {
                 matplot(cbind(x$reference[i, ], x$sample[i, 
                                                          ], x$warped.sample[i, ]), type = "n", xlab = "'Time'", 
                         ylab = "Signal", ...)
               }
               if (nrow(x$reference) == 1) {
               ...
               lines(x$warped.sample[i, ], lty = 1, col = 3, 
                     ...)
               legend("topleft", legend = c("Reference", "Sample", 
                                     "Warped sample"), lty = rep(1, 3), col = 1:3)
             }
           ...
           if (what == "function") {
             ...
             for (i in 1:nrow(x$warp.fun)) {
               plot(time, w.time[i, ], type = "n", xlab = "'Time'", 
                    ylab = ylbl, ...)
               abline(h = 0, col = "gray", ...)
               lines(time, w.time[i, ], col = rainbow(nrow(x$warp.fun))[i], 
                     ...)
             }
           }
         })
