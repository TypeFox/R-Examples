loadingsplot2D <- function(object, comps = c(1, 2)) {
  if(length(comps) == 1) {
    df <- data.frame(A = object$loadings[, comps[1]], label = 1:length(object$loadings[, comps[1]]))
    ...
    df <- data.frame(A = object$loadings[, comps[1]], B = object$loadings[, comps[2]])
    df$label <- row.names(df)
    row.names(df) <- NULL
