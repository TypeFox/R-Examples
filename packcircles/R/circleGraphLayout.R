#' Find an arrangement of circles conforming to specified sizes and adjacencies.
#' 
#' Attempts to derive an arrangement of circles satisfying prior conditions for
#' size and adjacency. Unlike the \code{\link{circleLayout}} function, this is a
#' deterministic algorithm. Circles are classified as either internal or
#' external. Viewing the pattern of adjacencies as a triangulated mesh, external
#' circles are those on the boundary. Under the version of the algorithm
...
#' derived as part of the fitting algorithm. The function will issue an error
#' if any internal circle IDs are present in the \code{external} data.
#' 
#' @return
...
#'   Computational Geometry Theory and Applications 25:233-256.
#' 
#' @param internal A list of vectors of circle ID values where, in each
#'   vector, the first element is the ID of an internal circle and the
#'   remaining elements are the IDs of that circle's neighbours arranged
#'   as a cycle. The cycle may be clockwise or anti-clockwise but the
#'   same orering must be used for all vectors.
#'   
#' @param external A data.frame or matrix of external circle radii,
#'   with circle IDs in the first column and radii in the second column.
#'   
#' @return The output arrangement as a data.frame with columns for
#'   circle ID, centre X and Y ordinates, and radius. For external
#'   circles the radius will equal input values.
#' 
#' @examples
#' \dontrun{
