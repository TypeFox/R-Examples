...
#list(fi=fi,di=di,ci=ci,ri=ri,fj=fj,cj=cj,rj=rj,dj=dj,t=taus,M=M,W=W,pdq=gpdq_results)
  ...
  res.tepGPLS <- x
  if (!inherits(res.tepGPLS, "tepGPLS")) stop ("no convenient data")
  cat("**Results for Partial Least Squares**\n")
  cat ("The analysis was performed on ", nrow(res.tepGPLS$lx),
       "individuals, described by", nrow(res.tepGPLS$fi), "variables from DATA1 and\n" , nrow(res.tepGPLS$fj), "variables from DATA2\n")
