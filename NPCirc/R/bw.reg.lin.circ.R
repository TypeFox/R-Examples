bw.reg.lin.circ<-function(x,y,method="LL",option=1,lower=0,upper=50,tol=1e-2){
	...
	if (option==1){
		cv<-function(bw){
			error<-numeric(n)
			for (j in 1:n){
				error[j]<- -cos(y[j]-RegLinCirc(x[-j],y[-j],x[j],bw,method=method))
			}
			return(sum(error))
		}
	} else if (option==2){
		cv<-function(bw){
			error<-numeric(n)
			for (j in 1:n){
				dif <- abs(y[j]-RegLinCirc(x[-j],y[-j],x[j],bw,method=method))
