...
fsRankRpartAUC_fct = function(data,fold,ncl,cv.out,cv.in,nr.var,t=1,sd1=0.9,c.time,...){
  ...
  output = foreach::foreach(i = 1:cv.out, .packages=c('SurvRank','rpart'),.errorhandling = "pass",.verbose = F) %dopar% {
    innerAUC_fct(f=fsSurvRankRpart,data=data,t=t,cv.out=cv.out,cv.in=cv.in,i=i,fold=fold,data.out=data.out,nr.var=nr.var,out.s=out.s,sd1=sd1,c.time=c.time,
