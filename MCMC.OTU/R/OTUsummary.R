OTUsummary <-
function(model,data,otus=NA,relative=FALSE,
log.base=10,summ.plot=TRUE,ptype="z",xgroup=NULL,...) {
	
#model=mm0;data=gss0;otus=acpass;relative=FALSE;log.base=10;summ.plot=TRUE;ptype="z";xgroup=NULL

	mm=model;base=log.base
	otu.results=list()
	trms=attr(terms(mm$Fixed$formula),"term.labels")[grep("otu:",attr(terms(mm$Fixed$formula),"term.labels"))]
	trms=sub("otu:","",trms)
	sols=colnames(mm$Sol)
	if (is.na(otus[1])) { otus=sub("otu","",sols[grep("otu[^:]*$",sols)])}
	facts =list()
	for (t in trms) {
	...
	if (nfactors>2) { 
		stop("not implemented for more than 2 factors")
	}

	if(otus[1]=="summ") { otus=otus[-1] }

	interaction=0
	if (nfactors==2) {
		sol=paste("otu",otus[2],":",names(facts)[1],facts[[1]][2],":",names(facts)[2],facts[[2]][2],sep="")
		if (sol %in% sols) {
			interaction=1
		}
	}
	...
	for (otu in otus) {
		...
		otu.results=append(otu.results,list(gres))
		names(otu.results)[length(otu.results)]=otu		
	}
	
	big.summary=data.frame(cbind("otu"=gsols,"f1"=fac1))
	...
	if(summ.plot) { 
		if (!relative) { 
			if (nfactors==2) {
				ploo=summaryPlotOTU(big.summary,xgroup=xgroup,
				facet=facet,type="line",log.base=log.base,...) 
			} else {
				ploo=summaryPlotOTU(big.summary,xgroup=names(facts[1]),type="line",log.base=log.base,...) 
			}
		} else {
			if (nfactors==2) {
				ploo=summaryPlotOTU(big.summary,xgroup=xgroup,
				...
				ploo=summaryPlotOTU(big.summary,xgroup=names(facts[1]),type="bar",log.base=log.base,...) 
			}
		}
	}

	return(list("summary"=big.summary,"otuWise"=otu.results,"ggPlot"=ploo))
}
