...
bezierArrowSmpl(x = c(0.25, 0.5, 0.5, 0.75), rez=40,
...
grid.newpage()
ctrl_pts <- data.frame(x=c(.5,.9,.1,.3),
                       y=c(.6,.9,0,.7))
...
grid.newpage()
ctrl_pts <- data.frame(x=c(.5,.9,.9, .5, 0, .3),
                       y=c(.6,.9,0, .2, .9, .7))
lns <- gnrlBezierPoints(ctrl_pts)
grid.lines(lns[,1], lns[,2])
grid.points(ctrl_pts$x, ctrl_pts$y, default.units = "npc")
...
drawBzrBasics(bezierArrowGradient, clr = "darkblue", grdt_clr = "yellow",
              width = .02,
              grdt_prop = .8,
              grdt_clr_prop = .5,
