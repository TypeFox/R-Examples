gr_ll_flexrsurv_fromto_GA0B0AB_bh<-function(GA0B0AB, Y, X0, X, Z, 
                                            expected_rate,
                                            weights=NULL,
                                            step, Nstep, 
  ...
  if(nX + nZ) {
    ...
    NPHterm <- intTD(rateTD_bh, fromT=Y[,1], toT=Y[,2], fail=Y[,3],
                     ...
                     Spline_t0=Spline_t0, Intercept_t0=Intercept_t0)
    Intb0 <-  integrate(Spline_t0, Y[,2], intercep=Intercept_t0) - integrate(Spline_t0, Y[,1], intercep=Intercept_t0) 
    Intb <- NULL
    YT0 <- fevaluate(Spline_t0, Y[,2], intercept=Intercept_t0)
    YT <- NULL
    RatePred <- ifelse(Y[,3] ,
...
#print(cbind(YT0, Fgamma0, Intb0,  Y[,2], RatioPred, RatePred, expected_rate) )
#####################################################################"
# now computes the mean score

  if (!is.null(weights)) {
# dldgamma0
    dLdgamma0 <- crossprod( YT0 * Fgamma0 - Intb0 , weights)
...
# d<dgamma0
    dLdgamma0 <- apply(   YT0 * Fgamma0 - Intb0 , 2, sum)
