...
function(style = "tcltk", sandbox = getOption('MTurkR.sandbox')) {
    ...
    credentialsWiz <- function() {
        credOK <- function() {
            Sys.setenv("AWS_ACCESS_KEY_ID" = tclvalue(accesskey))
            Sys.setenv("AWS_SECRET_ACCESS_KEY" = tclvalue(secretkey))
    ...
    getblockWiz <- function(){
        ...
        # populate scrollable listbox
        if(!is.data.frame(blockedworkers) && nrow(blockedworkers) > 0) {
            # function
