...
llsearch.QE3.CD <- function(x, y, n, jlo, jhi,start1,start2)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.CD.QE3, x = x, y = y, n = n, 
...
p.estFUN.CD.QE3 <- function(j, x, y, n,start1,start2){
  a <- p.est.QE3.CD(x,y,n,j,start1,start2)
  s2 <- a$sigma2
  t2 <- a$tau2
  return(p.ll.CD(n, j, s2, t2))
}

p.est.QE3.CD <- function(x,y,n,j,start1,start2){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
