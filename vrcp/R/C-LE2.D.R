...
con.search.LE2.D <- function(x, y, n, jlo, jhi)
{
  fjk <- matrix(0, n)
  ...
  res <- data.frame(j = jkgrid,
                    k.ll = apply(jkgrid, 1, con.parmsFUN.LE2.D, x = x, 
                                 y = y, n = n))
...
con.parmsFUN.LE2.D <- function(j, x, y, n){
  a <- con.parms.D.LE2(x,y,n,j,1,1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
  b<-con.est.D.LE2(x[j],est)
  s2<-1/b$eta1
  t2<-1/b$eta2
...
con.parms.D.LE2 <-
  function(x,y,n,j0,e10,e20){
    ...
    th[1,1] <- e10
    th[1,2] <- e20
    bc <- beta.calc.LE2.D(x,y,n,j0,e10,e20)
    th[1,3:5] <- bc$B
    ...
    for (iter in 2:100){
      ...
      ec <- eta.calc.D.LE2(x,y,n,j0,th[m,3:5])
      th[iter,1] <- ec$eta1
      th[iter,2] <- ec$eta2
      
      bc <- beta.calc.LE2.D(x,y,n,j0,ec$eta1,ec$eta2)
...
beta.calc.LE2.D <-
  ...
  {
    aa <- wmat.LE2.D(x, y, n, j, e1, e2)
    W <- aa$w
    bb <- rvec.LE2.D(x, y, n, j, e1, e2)
    R <- bb$r
    beta <- solve(W, R)
    list(B = beta)
  }
...
wmat.LE2.D <- 
  function(x, y, n, j, e1, e2)
  {
    ...
    W[2, 2] <- e1 * sum(x[1:j] * x[1:j]) + e2 * (n - j) * x[j]^2
    W[2, 3] <- e2 * x[j] * sum(log(x[jp1:n]) - log(x[j]))
    
    W[3, 3] <- e2 * sum((log(x[jp1:n]) - log(x[j])) * (log(x[jp1:n]) - log(x[j])))
