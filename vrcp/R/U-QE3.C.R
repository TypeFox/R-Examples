...
llsearch.QE3.C <- function(x, y, n, jlo, jhi,start1,start2,start3)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.C, x = x, y = y, n = n,start1=start1,start2=start2,start3=start3)
...
p.estFUN.C <- function(j, x, y, n,start1,start2,start3){
  a <- p.est.QE3.C(x,y,n,j,start1,start2,start3)
  s2 <- a$sigma2
  return(p.ll.C(n, j, s2))
}

p.est.QE3.C <- function(x,y,n,j,start1,start2,start3){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
  ...
  g2=nls(yb~fun2(xb,b0,b1,b2),data=data.frame(xb,yb),
         start=list(b0=start1,b1=start2,b2=start3)) # 5 0.2 -0.2
