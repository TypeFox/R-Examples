...
llsearch.QE3.D.WITHOUT.I <- function(x, y, n, jlo, jhi){
  ...
  for (j in jlo:jhi){ 
    ...
    s2<- sum((y[1:j] - est_a0 - est_a1 * x[1:j] - est_a2 * x[1:j]^2)^2)/j
  ...
  est_b0 <- ypred - est_b1 * (exp(est_b2*(x[est_j+1]-x[est_j]))) ## uncontinuous
