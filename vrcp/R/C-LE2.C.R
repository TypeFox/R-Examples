...
con.search.LE2.C <- function(x, y, n, jlo, jhi)
{
  fjk <- matrix(0, n)
  ...
  res <- data.frame(j = jkgrid,
                    k.ll = apply(jkgrid, 1, con.parmsFUN.LE2.C, x = x, 
                                 y = y, n = n))
...
con.parmsFUN.LE2.C <- function(j, x, y, n){
  a <- con.parms.LE2.C(x,y,n,j,1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
  b<-con.est.LE2.C(x[j],est)
  s2<-1/b$eta1
  return(p.ll.C(n, j, s2))
}

con.parms.LE2.C <- function(x,y,n,j0,e10){
  ...
  th[1,1] <- e10
  bc <- beta.calc.LE2.C(x,y,n,j0,e10)
  th[1,2:4] <- bc$B
  ...
  for (iter in 2:100){
    m <- iter-1
    ec <- eta.calc.LE2.C(x,y,n,j0,th[m,2:4])
    th[iter,1] <- ec$eta1
    bc <- beta.calc.LE2.C(x,y,n,j0,ec$eta1)
    th[iter,2:4] <- bc$B
    theta <- th[1:iter,]
...
con.est.LE2.C <- function(xj, est)
{
  eta1 <- est[1]
  a0 <- est[2]
  a1 <- est[3]
  b1 <- est[4]
  b0 <- a0 + a1 * xj - b1 * log(xj)
  list(eta1 = eta1, a0 = a0, a1 = a1, b0 = b0, b1 = b1)
}
con.vals.LE2.C <- function(x, y, n, j)
{
  a <- con.parms.LE2.C(x, y, n, j, 1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
...
beta.calc.LE2.C <- function(x, y, n, j, e1)
{
  aa <- wmat.LE2.C(x, y, n, j, e1)
  W <- aa$w
  bb <- rvec.LE2.C(x, y, n, j, e1)
  R <- bb$r
  beta <- solve(W, R)
  list(B = beta)
} 

eta.calc.LE2.C <- function(x, y, n, j, theta)
{
  jp1 <- j + 1
  a0 <- theta[1]
  a1 <- theta[2]
  b1 <- theta[3]
  b0 <- a0 + a1 * x[j] - b1 * log(x[j])
  rss1 <- sum((y[1:j] - a0 - a1 * x[1:j])^2)
  rss2 <- sum((y[jp1:n] - b0 - b1 * log(x[jp1:n]))^2)
  e1 <- n/(rss1 + rss2)
  list(eta1 = e1)
}
wmat.LE2.C <- function(x, y, n, j, e1)
{
  W <- matrix(0, 3, 3)
  jp1 <- j + 1
  W[1, 1] <- e1 * n
  ...
  W[2, 3] <- e1 * x[j] * sum(log(x[jp1:n]) - log(x[j]))
...
rvec.LE2.C <- function(x, y, n, j, e1)
{
  R <- array(0, 3)
  jp1 <- j + 1
  y1j <- sum(y[1:j])
