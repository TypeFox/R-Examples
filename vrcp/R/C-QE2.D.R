...
con.search.QE2.D <- function(x, y, n, jlo, jhi)
{
  fjk <- matrix(0, n)
  ...
  res <- data.frame(j = jkgrid,
                    k.ll = apply(jkgrid, 1, con.parmsFUN.QE2.D, x = x, 
                                 y = y, n = n))
...
con.parmsFUN.QE2.D <- function(j, x, y, n){
  a <- con.parms.D.QE2(x,y,n,j,1,1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
  b<-con.est.D.QE2(x[j],est)
  s2<-1/b$eta1
  t2<-1/b$eta2
...
con.parms.D.QE2 <-
  function(x,y,n,j0,e10,e20){
    ...
    th[1,1] <- e10
    th[1,2] <- e20
    bc <- beta.calc.QE2.D(x,y,n,j0,e10,e20)
    th[1,3:6] <- bc$B
    ...
    for (iter in 2:100){
      ...
      ec <- eta.calc.D.QE2(x,y,n,j0,th[m,3:6])
      th[iter,1] <- ec$eta1
      th[iter,2] <- ec$eta2
      
      bc <- beta.calc.QE2.D(x,y,n,j0,ec$eta1,ec$eta2)
...
beta.calc.QE2.D <-
  ...
  {
    aa <- wmat.QE2.D(x, y, n, j, e1, e2)
    W <- aa$w
    bb <- rvec.QE2.D(x, y, n, j, e1, e2)
    R <- bb$r
    beta <- solve(W, R)
    list(B = beta)
  }
...
wmat.QE2.D <- 
  function(x, y, n, j, e1, e2)
  {
    ...
    W[1, 4] <- e2 * sum(log(x[jp1:n]) - log(x[j]))
    
    W[2, 2] <- e1 * sum(x[1:j]^2) + e2 * (n - j) * x[j]^2
    W[2, 3] <- e1 * sum(x[1:j]^3) + e2 * (n - j) * x[j]^3
    W[2, 4] <- e2 * x[j] *  sum(log(x[jp1:n]) - log(x[j]))
    
    W[3, 3] <- e1 * sum(x[1:j]^4) + e2 * (n - j) * x[j]^4
    W[3, 4] <- e2 * x[j]^2 * sum(log(x[jp1:n]) - log(x[j]))
    
    W[4, 4] <- e2 * sum((log(x[jp1:n]) - log(x[j]))^2)
...
rvec.QE2.D <-
  function(x, y, n, j, e1, e2)
  {
    ...
    R[3] <- e1 * sum(x[1:j]^2 * y[1:j]) + e2 * x[j]^2 * yjn
    R[4] <- e2 * sum(xyjn - yjn * log(x[j]))
    list(r = R)
  }