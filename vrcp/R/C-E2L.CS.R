...
llsearch.E2L.CCS <- function(x, y, n, jlo, jhi)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.E2L.CCS, x = x, y = y, n = n)
...
p.estFUN.E2L.CCS <- function(j, x, y, n){
  a <- p.est.E2L.CCS(x,y,n,j)
  s2 <- a$sigma2
  return(p.ll.CCS(n, j, s2))
}

p.est.E2L.CCS <- function(x,y,n,j){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
  ...
  beta <-c(a0, a1, b0, b1)  
  s2 <- (sum((ya-a0-a1*log(xa))^2)+sum((yb-b0-b1*xb))^2)/n
