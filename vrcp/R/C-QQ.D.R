...
# Different variance for both segments

llsearch.QQ.CD <- function(x, y, n, jlo, jhi)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.QQ.CD, x = x, y = y, n = n)
...
p.estFUN.QQ.CD <- function(j, x, y, n){
  a <- p.est.QQ.CD(x,y,n,j)
  s2 <- a$sigma2
  t2 <- a$tau2
  return(p.ll.CD(n, j, s2, t2))
}

p.est.QQ.CD <- function(x,y,n,j){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
  ...
  t2 <- sum((yb-b0-g2$coef[2]*xb-g2$coef[3]*xb^2)^2)/(n-j)
