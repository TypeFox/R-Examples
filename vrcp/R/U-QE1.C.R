...
llsearch.QE1.C <- function(x, y, n, jlo, jhi)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.QE1.C, x = x, y = y, n = n)
...
p.estFUN.QE1.C <- function(j, x, y, n){
  a <- p.est.QE1.C(x,y,n,j)
  s2 <- a$sigma2
  return(p.ll.C(n, j, s2))
}

p.est.QE1.C <- function(x,y,n,j){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
