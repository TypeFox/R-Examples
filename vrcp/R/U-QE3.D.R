...
llsearch.QE3.D <- function(x, y, n, jlo, jhi,start1,start2,start3)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.QE3.D, x = x, y = y, n = n,
...
p.estFUN.QE3.D <- function(j, x, y, n,start1,start2,start3){
  a <- p.est.QE3.D(x,y,n,j,start1,start2,start3)
  s2 <- a$sigma2
  t2 <- a$tau2
  return(p.ll.D(n, j, s2, t2))
}

p.est.QE3.D <- function(x,y,n,j,start1,start2,start3){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
  yb <- y[jp1:n]
  g1 <- lm(ya ~ I(xa) + I(xa^2))
