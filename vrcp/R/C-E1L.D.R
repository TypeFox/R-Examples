...
con.search.E1L.D <- function(x, y, n, jlo, jhi)
{
  fjk <- matrix(0, n)
  ...
  res <- data.frame(j = jkgrid,
                    k.ll = apply(jkgrid, 1, con.parmsFUN.E1L.D, x = x, 
                                 y = y, n = n))
...
con.parmsFUN.E1L.D <- function(j, x, y, n){
  a <- con.parms.D.E1L(x,y,n,j,1,1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
  b<-con.est.D.E1L(x[j],est)
  s2<-1/b$eta1
  t2<-1/b$eta2
...
con.parms.D.E1L <-
  function(x,y,n,j0,e10,e20){
    ...
    th[1,1] <- e10
    th[1,2] <- e20
    bc <- beta.calc.E1L.D(x,y,n,j0,e10,e20)
    th[1,3:5] <- bc$B
    ...
    for (iter in 2:100){
      ...
      ec <- eta.calc.D.E1L(x,y,n,j0,th[m,3:5])
      th[iter,1] <- ec$eta1
      th[iter,2] <- ec$eta2
      
      bc <- beta.calc.E1L.D(x,y,n,j0,ec$eta1,ec$eta2)
...
beta.calc.E1L.D <-
  ...
  {
    aa <- wmat.E1L.D(x, y, n, j, e1, e2)
    W <- aa$w
    bb <- rvec.E1L.D(x, y, n, j, e1, e2)
    R <- bb$r
    beta <- solve(W, R)
    list(B = beta)
  }
...
wmat.E1L.D <- 
  function(x, y, n, j, e1, e2)
  {
    ...
    W[2, 3] <- e2 * exp(x[j]) * sum(x[jp1:n] - x[j])
