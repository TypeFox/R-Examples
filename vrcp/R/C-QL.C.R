...
con.search.QL.C <- function(x, y, n, jlo, jhi)
{
  fjk <- matrix(0, n)
  ...
  res <- data.frame(j = jkgrid,
                    k.ll = apply(jkgrid, 1, con.parmsFUN.QL.C, x = x, 
                                 y = y, n = n))
...
con.parmsFUN.QL.C <- function(j, x, y, n){
  a <- con.parms.QL.C(x,y,n,j,1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
  b<-con.est.QL.C(x[j],est)
  s2<-1/b$eta1
  return(p.ll.C(n, j, s2))
}

con.parms.QL.C <- function(x,y,n,j0,e10){
  ...
  th[1,1] <- e10
  bc <- beta.calc.QL.C(x,y,n,j0,e10)
  th[1,2:5] <- bc$B
  ...
  for (iter in 2:100){
    m <- iter-1
    ec <- eta.calc.QL.C(x,y,n,j0,th[m,2:5])
    th[iter,1] <- ec$eta1
    bc <- beta.calc.QL.C(x,y,n,j0,ec$eta1)
    th[iter,2:5] <- bc$B
    theta <- th[1:iter,]
...
con.est.QL.C <- function(xj, est)
{
  ...
  a2 <- est[4]
  b1 <- est[5]
  b0 <- a0 + (a1 - b1) * xj + a2 * xj^2
  list(eta1 = eta1, a0 = a0, a1 = a1, a2 = a2, b0 = b0, b1 = b1)
}

con.vals.QL.C <- function(x, y, n, j)
{
  a <- con.parms.QL.C(x, y, n, j, 1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
...
beta.calc.QL.C <- function(x, y, n, j, e1)
{
  aa <- wmat.QL.C(x, y, n, j, e1)
  W <- aa$w
  bb <- rvec.QL.C(x, y, n, j, e1)
  R <- bb$r
  beta <- solve(W, R)
  list(B = beta)
} 

eta.calc.QL.C <- function(x, y, n, j, theta)
{
  jp1 <- j + 1
  a0 <- theta[1]
  a1 <- theta[2]
...
wmat.QL.C <- function(x, y, n, j, e1)
{
  ...
  W[1, 4] <- e1 * sum(x[jp1:n] - x[j])
  
  W[2, 2] <- e1 * sum(x[1:j]^2) + e1 * (n - j) * x[j]^2
  W[2, 3] <- e1 * sum(x[1:j]^3) + e1 * x[j]^3 * (n - j)
  W[2, 4] <- e1 * x[j] * sum(x[jp1:n]-x[j])
  
  W[3, 3] <- e1 * sum(x[1:j]^4) + e1 * (n - j) * x[j]^4
  W[3, 4] <- e1 * x[j]^2 * sum(x[jp1:n] - x[j])
  
  W[4, 4] <- e1 * sum((x[jp1:n] - x[j])^2)
