...
llsearch.LE3.CCS.WITHOUT.I <- function(x, y, n, jlo, jhi, start1, start2, start3, start4, start5)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.LE3.CCS.WITHOUT.I, x = x, y = y, n = n, 
...
p.estFUN.LE3.CCS.WITHOUT.I <- function(j, x, y, n,start1,start2,start3,start4, start5){
  a <- p.est.LE3.CCS.WITHOUT.I(x,y,n,j,start1,start2,start3,start4, start5)
  s2 <- a$sigma2
  return(p.ll.CCS.WITHOUT.I(n, j, s2))
}

p.est.LE3.CCS.WITHOUT.I <- function(x,y,n,j,start1,start2,start3,start4,start5){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
  ...
  s2<- sum((ya-a0-a1*xa)^2)+sum((yb-b0-b1*exp(b2*(xb-x[j]))^2))/n
...
p.ll.CCS.WITHOUT.I <- function(n, j, s2){
  q1 <- n * log(sqrt(2 * pi))
  q2 <- 0.5 * n  * (1 + log(s2))
  - (q1 + q2)
}
