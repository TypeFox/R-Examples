...
con.search.QE1.C <- function(x, y, n, jlo, jhi)
{
  fjk <- matrix(0, n)
  ...
  res <- data.frame(j = jkgrid,
                    k.ll = apply(jkgrid, 1, con.parmsFUN.QE1.C, x = x, 
                                 y = y, n = n))
...
con.parmsFUN.QE1.C <- function(j, x, y, n){
  a <- con.parms.QE1.C(x,y,n,j,1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
  b<-con.est.QE1.C(x[j],est)
  s2<-1/b$eta1
  return(p.ll.C(n, j, s2))
}

con.parms.QE1.C <- function(x,y,n,j0,e10){
  ...
  th[1,1] <- e10
  bc <- beta.calc.QE1.C(x,y,n,j0,e10)
  th[1,2:5] <- bc$B
  ...
  for (iter in 2:100){
    m <- iter-1
    ec <- eta.calc.QE1.C(x,y,n,j0,th[m,2:5])
    th[iter,1] <- ec$eta1
    bc <- beta.calc.QE1.C(x,y,n,j0,ec$eta1)
    th[iter,2:5] <- bc$B
    theta <- th[1:iter,]
...
con.vals.QE1.C <- function(x, y, n, j)
{
  a <- con.parms.QE1.C(x, y, n, j, 1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
...
beta.calc.QE1.C <- function(x, y, n, j, e1)
{
  aa <- wmat.QE1.C(x, y, n, j, e1)
  W <- aa$w
  bb <- rvec.QE1.C(x, y, n, j, e1)
  R <- bb$r
  beta <- solve(W, R)
  list(B = beta)
} 

eta.calc.QE1.C <- function(x, y, n, j, theta)
{
  jp1 <- j + 1
  a0 <- theta[1]
  a1 <- theta[2]
...
wmat.QE1.C <- function(x, y, n, j, e1)
{
  W <- matrix(0, 4, 4)
  jp1 <- j + 1
  W[1, 1] <- e1 * n 
  ...
  W[1, 4] <- e1 * sum(exp(x[jp1:n]) - exp(x[j]))
  
  W[2, 2] <- e1 * sum(x[1:j]^2) + e1 * (n - j) * x[j]^2
  W[2, 3] <- e1 * sum(x[1:j]^3) + e1 *  (n - j) * x[j]^3
  W[2, 4] <- e1 * x[j] * sum(exp(x[jp1:n]) - exp(x[j]))
  
  W[3, 3] <- e1 * sum(x[1:j]^4) + e1 * (n - j) * x[j]^4
  W[3, 4] <- e1 * x[j]^2 * sum(exp(x[jp1:n]) - exp(x[j]))
  
  W[4, 4] <- e1 * sum((exp(x[jp1:n]) - exp(x[j]))^2)
...
rvec.QE1.C <- function(x, y, n, j, e1)
{
  R <- array(0, 4)
  jp1 <- j + 1
  y1j <- sum(y[1:j])
  ...
  R[4] <- e1 * sum( (exp(x[jp1:n]) - exp(x[j])) * y[jp1:n] )
