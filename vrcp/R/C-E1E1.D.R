...
con.search.E1E1.D <- function(x, y, n, jlo, jhi)
{
  fjk <- matrix(0, n)
  ...
  res <- data.frame(j = jkgrid,
                    k.ll = apply(jkgrid, 1, con.parmsFUN.E1E1.D, x = x, 
                                 y = y, n = n))
...
con.parmsFUN.E1E1.D <- function(j, x, y, n){
  a <- con.parms.D.E1E1(x,y,n,j,1,1)
  nr <- nrow(a$theta)
  est <- a$theta[nr,  ]
  b<-con.est.D.E1E1(x[j],est)
  s2<-1/b$eta1
  t2<-1/b$eta2
...
con.parms.D.E1E1 <-
  function(x,y,n,j0,e10,e20){
    ...
    th[1,1] <- e10
    th[1,2] <- e20
    bc <- beta.calc.E1E1.D(x,y,n,j0,e10,e20)
    th[1,3:5] <- bc$B
    ...
    for (iter in 2:100){
      ...
      ec <- eta.calc.D.E1E1(x,y,n,j0,th[m,3:5])
      th[iter,1] <- ec$eta1
      th[iter,2] <- ec$eta2
      
      bc <- beta.calc.E1E1.D(x,y,n,j0,ec$eta1,ec$eta2)
...
beta.calc.E1E1.D <-
  ...
  {
    aa <- wmat.E1E1.D(x, y, n, j, e1, e2)
    W <- aa$w
    bb <- rvec.E1E1.D(x, y, n, j, e1, e2)
    R <- bb$r
    beta <- solve(W, R)
    list(B = beta)
  }
...
wmat.E1E1.D <- 
  function(x, y, n, j, e1, e2)
  {
    ...
    jp1 <- j + 1
    W[1, 1] <- e1 * j + e2 * (n - j) 
    W[1, 2] <- e1 * sum(exp(x[1:j])) + e2 * (n - j) * exp(x[j])
    W[1, 3] <- e2 * sum(exp(x[jp1:n]) - exp(x[j]))
    
    W[2, 2] <- e1 * sum(exp(x[1:j])^2) + e2 * (n-j) * exp(x[j])^2
    W[2, 3] <- e2 * exp(x[j]) * sum(exp(x[jp1:n]) - exp(x[j]))
    
    W[3, 3] <-e2 * sum((exp(x[jp1:n]) - exp(x[j])) * (exp(x[jp1:n]) - exp(x[j])))
...
rvec.E1E1.D <-
  function(x, y, n, j, e1, e2)
  {
    ...
    R[1] <- e1 * y1j + e2 * yjn
    R[2] <- e1 * xy1j + e2 * exp(x[j]) * yjn
    R[3] <- e2 * (xyjn - yjn * exp(x[j]))
    list(r = R)
  }