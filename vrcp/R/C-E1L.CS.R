...
llsearch.E1L.CCS <- function(x, y, n, jlo, jhi)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.E1L.CCS, x = x, y = y, n = n)
...
p.estFUN.E1L.CCS <- function(j, x, y, n){
  a <- p.est.E1L.CCS(x,y,n,j)
  s2 <- a$sigma2
  return(p.ll.CCS(n, j, s2))
}

p.est.E1L.CCS <- function(x,y,n,j){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
  yb <- y[jp1:n]
  x1 <- exp(x) + ((exp(x[j])-exp(x))+exp(x[j])*(x-x[j]))*(x >= x[j])
  ...
  beta <-c(a0, a1, b0, b1)  
  s2<- (sum((ya-a0-a1*exp(xa))^2)+sum((yb-b0-b1*(xb))^2))/n
