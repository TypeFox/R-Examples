...
llsearch.E1E1.D <- function(x, y, n, jlo, jhi)
{
  fj <- matrix(0, n)
  fxy <- matrix(0, jhi - jlo + 1)
  
  jgrid <- expand.grid(jlo:jhi)
  k.ll <- apply(jgrid, 1, p.estFUN.E1E1.D, x = x, y = y, n = n)
...
p.estFUN.E1E1.D <- function(j, x, y, n){
  a <- p.est.E1E1.D(x,y,n,j)
  s2 <- a$sigma2
  t2<- a$tau2
  return(p.ll.D(n, j, s2,t2))
}

p.est.E1E1.D <- function(x,y,n,j){
  xa <- x[1:j]
  ya <- y[1:j]
  jp1 <- j+1
  xb <- x[jp1:n]
