np.nv=function(a,b,c,d,n,alpha){
		...
		np_nv=value%*%contr+c(-1,1)*(sqrt(contr%*%v%*%as.matrix(contr,ncol=1,nrow=2))*qnorm(1-alpha/2))/sqrt(ntotal)
		attr(np_nv, "conf.level") <- 1-alpha
		rval <- list(conf.int = np_nv, estimate = est)
		class(rval) <- "htest"
		return(rval)
