...
function(XTraining,YTraining,p=0.2,t.initial= NULL, nf=NULL,ns=NULL,info=TRUE){
  ...
  for (j in 1:t.initial){
    ...
    rff[[j]] <- randomForest(x=Xtraining1,y=Ytraining1, ntree=1, keep.forest=TRUE,norm.votes=FALSE,mtry =ifelse(is.null(nf),round(sqrt(length(Xtraining1))),nf),nodesize=ifelse(is.null(ns),5,ns))#sqrt(length(Xtraining1)))
    er[j]   <-  1-rff[[j]][5]$rsq
  ...
  {
    p1 <- predict(rf.all, XTraining[training2,])
    sst1 <- sum((YTraining[training2]-mean(YTraining[training2]))^2)
    sse1 <- sum((YTraining[training2]-p1)^2)
    unexp1 <- sse1/sst1
    
    rf.all<-combine(rf.all,rff[[k+1]])
    p2 <- predict(rf.all, XTraining[training2,])
    sst <- sum((YTraining[training2]-mean(YTraining[training2]))^2)
    sse <- sum((YTraining[training2]-p2)^2)
    unexp <- sse/sst
