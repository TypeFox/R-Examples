simt <- function(model, file, param=NULL, pdfout=F, lab.out="") {
 ...
 if(NS>1){
 unit <- c(input$Uni[1], rep(input$Uni[iNS],NS+1))
 tXlab <- paste(c("t",paste("X",seq(1:NS),sep=""),"Xtotal"),"[",unit,"]", sep="")
 } else {
...
# arrays to store results (include one extra for total)
  X <-  structure(1:(np*nt*(NS+1)),dim=c(nt,np,(NS+1)))
...
# loop for parameter p that varies
  for(i in 1:np) { 
     ...
     # sub-states delay
     if(is.null(model$x0)==F){
      nd <- p[length(mp)]
      X0 <- model$x0(p,X0)
      Y  <- structure(1:(nt*nd*np), dim=c(nt,nd,np));Y[,,] <- 0
     } 
     # integration
     out <- RK4(X0, t, model$f, p, dt)
     # rename x part of out for simpler use
     if(NS>1) {
     for(k in 1:NS) X[,i,k] <- out[,k]
     # sub-states delay
     if(is.null(model$x0)==F){
      Y[,,i] <- out[,(NS+1):(nd+NS)]
      for(j in 1:nt) X[j,i,1] <- out[j,1] + sum(Y[j,1:nd,i])
     } 
     # total
     for(j in 1:nt) X[j,i,(NS+1)] <- sum(X[j,i,(1:NS)]) 
     }else{
     # calculate rate for one-state systems
      X[,i,1] <- out
      for(j in 1:nt) dX.dt[j,i] <- model$f(t[j],p,X[j,i,1])
     }
