simd <- function(model, file, param=NULL, pdfout=F, lab.out="") {
...
# loop for parameter p that varies
  for(i in 1:np) { 
     ...
     # discontinuity times set
     tz <- model$z(t,p,X)
     # integration
     out <- RK4D(X0, t, model$f, p, dt, model$g, tz)
     ...
     # calculate rate for one-state systems
      ...
      X[,i,1] <- out
      for(j in 1:nt) 
      dX.dt[j,i] <- model$f(t[j],p,X[j,i,1]) + 
                    model$g(t[j],p,X[j,i,1],tz)
     }
