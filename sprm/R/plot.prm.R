...
function(x,type="yyp",alpha=.025,colors=list(bars="#0000AA",errorbars="red",background="#BBBBEE",abline="#21A0D2", scores="#0000AA",cutoffs="#00EEEE", badouts="darkred", modouts="black"),textsize=6,errorbar_width=1,data,yscale=NULL, ...)
  ...
  # plot.prm automatically generates Partial Robust M regression Weights and Regression Coefficients Plots
  # inputs : x, a "prm" class PRM regression object 
...
#  source("dbshdyv.r")
  ...
  if(!(class(x)=="prm")){stop("The PRM plot function can only be applies to prm class objects")}
  ...
  if(type=="weights"){
    ...
    plotweights$nam <- factor(plotweights$nam, levels=names(w)) # to get the observations in the plot in the right order
    ...
    plotdd <- data.frame(T2=T2prms,DModX=DModX,cutoffT2=rep(cutoffT2,n),cutoffDX=rep(cutoffDX,n))
    ...
    zerows <- which(w==0)
    if (length(zerows)==0){ 
      X <- x$inputs$Xs * sqrt(w)
      y <- x$inputs$ys * sqrt(w)
    } else {
      X <- x$inputs$Xs[-zerows,] * sqrt(w[-zerows])
   ...
   if(type=="coefficients"){
      ...
      plotty <- ggplot(plotcoefs,aes(nam,coefficients)) + geom_bar(stat="identity",size=3,fill=colors$bars, position = "identity") + labs(title=paste(names(x$YMeans),"PRM Regression Coefficients Plot")) 
      ...
      plotty <- plotty + geom_text(data=plotyyp[which(plotyyp$outs==0),],aes(x=y_original,y=y_predicted,label=nam),size=textsize,color=colors$bars) 
      ...
      plotty <- plotty + labs(title=paste("PRM Regression ", ynames," vs. ", ynames, " Predicted -", x$inputs$a," LV,  Rw = ",round(rsq,2)," df = ",round(df,1))) 
