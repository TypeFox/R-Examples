...
function(x,comps=c(1,2),colors=list(scores="#0000AA",loadings="red",background="#BBBBEE"),textsize=6,arrowshapes=c(25,.03), labelpos=0.35, ...)
  ...
  # biplot.prm automatically generates Sparse Partial Robust M regression Biplots
...
#  require(grid)
  V1 <- V2 <- V3 <- V4 <- V5 <- NULL  
  if(!(class(x)=="prm")){
    stop("The PRM biplot function can only be applied to prm class objects")
  }
  ...
  plotty <- plotty + labs(title=paste(names(x$YMeans)," PRM Components ", c1," and ",c2," Biplot"),x=colnames(plotscores)[1],y=colnames(plotscores)[2])
  plotty <- plotty + geom_segment(data=plotloadings,aes(x=V3,y=V3,xend=V1,yend=V2),color=colors$loadings,
                                  arrow=ggplot2::arrow(angle=arrowshapes[1],length=ggplot2::unit(arrowshapes[2],"npc")),na.rm=TRUE)
