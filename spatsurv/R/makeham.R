...
##' @seealso \link{tpowHaz}, \link{exponentialHaz}, \link{gompertzHaz}, \link{weibullHaz}
##' @export
...
makehamHaz <- function(){
    ...
    flist$distinfo <- function(){
        retlist <- list()
        retlist$npars <- 3
        retlist$parnames <- c("alpha","beta","mu")
        retlist$trans <- log
        retlist$itrans <- exp
        retlist$jacobian <- exp
        retlist$hessian <- list(exp,exp,exp)
        return(retlist)
    }
    
    flist$basehazard <- function(pars){
        fun <- function(t){
            return(pars[1]*exp(pars[2]*t)+pars[3]) # in this case alpha=pars[1], beta=pars[2] 
        }
        return(fun)  
    }
    ...
    flist$gradbasehazard <- function(pars){
        fun <- function(t){
            return(cbind(exp(pars[2]*t)*cbind(1,pars[1]*t),1)) # in this case alpha=pars[1], beta=pars[2]
        }
        return(fun)
    ...
    flist$cumbasehazard <- function(pars){
        fun <- function(t){
            return((pars[1]/pars[2])*(exp(pars[2]*t)-1)+pars[3]*t) # in this case alpha=pars[1], beta=pars[2]
        }
        return(fun) 
    }
    ...
    flist$gradcumbasehazard <- function(pars){
        fun <- function(t){
            return(cbind((1/pars[2])*(exp(pars[2]*t)-1),(pars[1]/pars[2])*exp(pars[2]*t)*(-1/pars[2]+t)+pars[1]/pars[2]^2,t)) # in this case alpha=pars[1], beta=pars[2]
        }
        return(fun)    
    }
    ...
    flist$densityquantile <- function(pars,other){
        fun <- function(probs){
            stop("densityquantil for Makeham not implemented yet ...")
