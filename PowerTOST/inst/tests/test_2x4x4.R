...
cat("Table 9.6.4 2x4x4, BEL 0.8-1.20, additive model,\n")
cat("approximate via shifted central t-distribution\n")
CVs  <- seq(from=0.1,  to=0.4, by=0.02)
GMRs <- seq(from=0.0, to=0.15, by=0.05)
power <- c(0.8, 0.9)
t9.6.4 <- sampsiz(power=power, CV=CVs, GMR=GMRs, logscale=FALSE, theta1=-0.2,
                  method="shifted", design="2x4x4")

cat("Table 9.6.8 2x4x4, BEL 0.8-1.25, multiplicative model model,\n");
cat("approximate via shifted central t-distribution\n")
...
power <- c(0.8, 0.9)
t9.6.8 <- sampsiz(power=power, CV=CVs, GMR=GMRs, logscale=TRUE, theta1=0.8,
                  method="shift", design="2x4x4")
