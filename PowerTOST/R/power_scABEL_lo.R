                           ...
                           nsims=1E5, details=FALSE, setseed=TRUE)
{
  ...
  p <- .power.scABEL2(mlog, sdm, C2, Emse, cvec, df, s2wR, dfRR, s2wT, dfTT, 
                      design, nsims, ln_lBEL=log(theta1),ln_uBEL=log(theta2), 
                      alpha=alpha)
...
.power.scABEL2 <- function(mlog, sdm, C2, Emse, cvec, df, 
                          s2wR, dfRR, s2wT, dfTT, design,
                          nsims, ln_lBEL=log(0.8), ln_uBEL=log(1.25), alpha=0.05)
{
  tcrit    <- qt(1-alpha,df)
  denom    <- sum(cvec)
...
#               (dfRR+1)*(s2wTs + 2*s2wRs)/3)/(dfTT+dfRR+2)
    ...
    #--- "levelling-off" BE limits
    loABEL <- scABEL_LO(CV=mse2CV(s2wRs))
    lABEL  <- log(loABEL[,"lower"])
    uABEL  <- log(loABEL[,"upper"])
    #--- 90% CIs for T-R
