...
MakeReadable= function(pkg){
if(.Platform$OS.type=="windows"){
if(Sys.which("python")!=""){
if(pkg %in% installed.packages()) {
# convert files here
VignetteFolder=paste0(system.file(package=pkg), "/doc")
RnwFiles= c(list.files(path=VignetteFolder, pattern="Rnw"), list.files(path=VignetteFolder, pattern="rnw"))
MyVignettesFolder = paste0(getOption("BrailleR.Folder"), "/Vignettes/")
...
file.copy(paste0(VignetteFolder, "/", RnwFiles), paste0(MoveTo, RnwFiles), overwrite=TRUE)
for(i in RnwFiles){
message("Copying and converting line breaks:")
shell(paste0(system.file("Python/FixLineBreaks.py", package="BrailleR"), " ", MoveTo, i))
message("Converting tabs to spaces:")
shell(paste0(system.file("Python/RemoveTabs.py", package="BrailleR"), " ", MoveTo, i))
}
}else{warning("The specified package is not installed.\n")} 
}else{warning("This function requires an installation of Python 2.7.\n")}
