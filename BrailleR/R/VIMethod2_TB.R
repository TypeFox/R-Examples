...
VI.summary.lm <- function(x)
{
  ...
  if(row.names(CoeTable)[1]=="(Intercept)"){ IsInter = 1 }
  else{ IsInter = 0}
  
  # Determine the significance of each model term
  SigPValues <- numeric(nrow(CoeTable)-IsInter)
  for(Term in 1:length(SigPValues)){
    if(CoeTable[Term+IsInter,4] <= 0.01){SigPValues[Term] = 0.01 }
    else if(CoeTable[Term+IsInter,4] <= 0.05){SigPValues[Term] = 0.05 }
    else if(CoeTable[Term+IsInter,4] <= 0.1){SigPValues[Term] = 0.1 }
  }
  ...
  for(SigLevel in c(0.01,0.05,0.1)){
    ...
    if(length(SigTerms)!=0){ 
      cat('The', ifelse(length(SigTerms)==1," term which is "," terms which are;"), 'significant to ',SigLevel*100,
          ifelse(length(SigTerms)==1, "% is","% are"),'\n', sep="")
      for(Term in 1:length(SigTerms)){
        Index = SigTerms[Term] + IsInter
        cat(row.names(CoeTable)[Index],'with an estimate of',CoeTable[Index,1],'and P-Value of',CoeTable[Index,4],'\n')
      }
      cat('\n')
    }
  }
  ...
  if(length(NonSigTerms) != 0){
    if(length(NonSigTerms) == 1){ 
      cat('The term',row.names(CoeTable)[NonSigTerms+IsInter],'is not significant to 10%.')
    }
    else{
      NoOfTerms <- length(NonSigTerms)
    cat('The terms',paste(row.names(CoeTable)[NonSigTerms[1:(NoOfTerms-1)]+IsInter], collapse=", "),'and',
        row.names(CoeTable)[NonSigTerms[NoOfTerms]+IsInter], 'are not significant to 10%.')
    }
  }
}
