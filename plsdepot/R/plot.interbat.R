...
#'
#'@description
#'Plot method for objects of class \code{"interbat"}. This function plots either
...
#'components of the other block: \code{$cor.xu} -vs- \code{$cor.yt}.
#'
#'@param x An object of class \code{"interbat"}.
...
plot.interbat <- 
  function(x, what = "variables", comps = c(1,2), where=c("t","t"),
           cex = 1, col.xlabels = "#5592e3", col.ylabels = "#e3a654",
           ...
           show.grid = TRUE, col.grid = "gray95", ...)
  {
    ...
    { 
      ...
      # arrows
      arrows(x0=rep(0,p), y0=rep(0,p), x1=x$cor.xu[,k1], y1=x$cor.xu[,k2],
      ...
      text(x$cor.xu[,k1], x$cor.xu[,k2], labels=rownames(x$cor.xu), 
