...
#'
#'@description
#'Plot method for objects of class \code{"simpls"}. This function plots either
...
#'
#'@details
#'Variables are displayed using the correlations of each block of variables with the
#'X-components: \code{$cor.xt} and \code{$cor.yt}.
#'
#'@param x An object of class \code{"simpls"}.
...
#'@param offset When \code{pos} is specified, this value gives the offset of the label.
...
plot.simpls <- 
  function(x, what = "variables", comps = c(1,2),
           ...
           col.xarrows = "#5b9cf255", col.yarrows = "#FE992955",
           ...
           show.grid = TRUE, col.grid = "gray95", ...)
  {
    ...
    { 
      ...
      # =======================================================
      # plot
      plot(x$x.scores[,k1], x$x.scores[,k2], type="n", axes=FALSE,
      ...
      if (!show.names)
      {
        points(x$x.scores[,k1], x$x.scores[,k2], col=col.points, pch=pch, bg=pt.bg,
               lwd=1.5, cex=cex)
      } else {
        text(x$x.scores[,k1], x$x.scores[,k2], labels=rownames(x$x.scores), 
             col=col.xlabels, cex=cex, pos=pos, offset=offset, xpd=xpd, ...)
      }
      if (is.null(main))
