extractKeyword <-
function(username,password,kw,nmail=-1)
{
 ...
 imap$exec(paste("nmsg = user_email.select()"))
 #imap$exec(paste("stts,msgid = user_email.uid('search', None, 'ALL')",sep=""))
 imap$exec(paste("stts, msgid = user_email.uid('search', None, '(HEADER Subject ",kw,")')",sep=""))
