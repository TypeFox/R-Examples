combine <- function(...) {
   ...
   padm0 <- function(x, len) rbind(x, matrix(0, nrow=len-nrow(x),
                                             ncol=ncol(x)))
   rflist <- list(...)
   areForest <- sapply(rflist, function(x) inherits(x, "RRF")) 
   if (any(!areForest)) stop("Argument must be a list of RRF objects")
   ...
   if (! all(numvars[1] == numvars[-1]))
       stop("Unequal number of predictor variables in the RRF objects.")
   for (i in seq_along(vlist)) {
       if (! all(vlist[[i]] == vlist[[1]]))
           stop("Predictor variables are different in the RRF objects.")
   }
