rrfImpute <- function(x, ...)
    UseMethod("rrfImpute")

rrfImpute.formula <- function(x, data, ..., subset) {
    ...
    ret <- rrfImpute.default(m, y, ...)
    names(ret)[1] <- deparse(as.list(x)[[2]])
    ret
}

rrfImpute.default <- function(x, y, iter=5, ntree=300, ...) {
    ...
    for (i in 1:iter) {
        prox <- RRF(xf, y, ntree=ntree, ..., do.trace=ntree,
