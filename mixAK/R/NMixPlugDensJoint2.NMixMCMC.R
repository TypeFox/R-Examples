...
##
##  FUNCTIONS: NMixPlugDensJoint2.NMixMCMC (28/05/2009)
##             
## ======================================================================
...
## *************************************************************
NMixPlugDensJoint2.NMixMCMC <- function(x, grid, lgrid=50, scaled=FALSE, ...)
{
  ...
  if (x$nx_w == 1){  
    ...
    dns <- NMixPlugDensJoint2.default(x=grid, scale=scale, w=x$poster.mean.w, mu=x$poster.mean.mu, Sigma=x$poster.mean.Sigma)
    dns$nx_w <- 1
    dns$lx_w <- ""
    ...
    for (ixw in 1:x$nx_w){     
      ...
      dns[[ixw]] <- NMixPlugDensJoint2.default(x = gridixw, scale = scale, w = x$poster.mean.w[(ixw - 1)*x$K[1] + 1:x$K[1]], mu = x$poster.mean.mu, Sigma = x$poster.mean.Sigma)
    }
    names(dns) <- x$lx_w
    dns$nx_w <- x$nx_w
    dns$lx_w <- x$lx_w
    
    class(dns) <- "NMixPlugDensJoint2"
    return(dns)
  }    
}  
