...
##  CREATED:   24/07/2009
##
##  FUNCTIONS: NMixPredDensJoint2.GLMM_MCMC (24/07/2009)
##             
## ======================================================================

## *************************************************************
## NMixPredDensJoint2.GLMM_MCMC
## *************************************************************
NMixPredDensJoint2.GLMM_MCMC <- function(x, grid, lgrid=50, scaled=FALSE, ...)
{
  if (missing(grid)){
    grid <- list()
    if (scaled){
  ...
  if (x$prior.b$priorK == "fixed"){
    return(NMixPredDensJoint2.default(x=grid, scale=scale, K=x$K_b, w=as.numeric(t(x$w_b)), mu=as.numeric(t(x$mu_b)), Li=as.numeric(t(x$Li_b)), Krandom=FALSE))
  }else{
    return(NMixPredDensJoint2.default(x=grid, scale=scale, K=x$K_b, w=x$w_b, mu=x$mu_b, Li=x$Li_b, Krandom=TRUE))
  }    
}  
