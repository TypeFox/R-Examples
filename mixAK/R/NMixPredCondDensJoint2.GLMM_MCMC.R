...
##  CREATED:   24/07/2009
##
##  FUNCTIONS: NMixPredCondDensJoint2.GLMM_MCMC (24/07/2009)
##             
## ======================================================================
...
## *************************************************************
NMixPredCondDensJoint2.GLMM_MCMC <- function(x, icond, grid, lgrid=50, scaled=FALSE, ...)
{
  if (missing(icond)) stop("icond must be given")
  if (x$dimb <= 2) stop("not applicable for uni- or bivariate mixtures")  
  ...
  if (x$prior.b$priorK == "fixed"){  
    return(NMixPredCondDensJoint2.default(x=grid, icond=icond, scale=scale, K=x$K_b, w=as.numeric(t(x$w_b)), mu=as.numeric(t(x$mu_b)), Li=as.numeric(t(x$Li_b)), Krandom=FALSE))
  }else{
    return(NMixPredCondDensJoint2.default(x=grid, icond=icond, scale=scale, K=x$K_b, w=x$w_b, mu=x$mu_b, Li=x$Li_b, Krandom=TRUE))    
  }  
}  
