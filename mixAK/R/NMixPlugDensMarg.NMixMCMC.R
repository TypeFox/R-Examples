##
##  PURPOSE:   Computation of the marginal (univariate) densities
##             (plug-in version)
...
##
##  CREATED:   28/05/2009
##             01/04/2015:  code allowing for a factor covariate
##                          on mixture weights implemented
##
##  FUNCTIONS: NMixPlugDensMarg.NMixMCMC (28/05/2009)
##             
## ======================================================================
...
## *************************************************************
NMixPlugDensMarg.NMixMCMC <- function(x, grid, lgrid=500, scaled=FALSE, ...)
{
  ...
  if (x$nx_w == 1){
    ...
    dns <- NMixPlugDensMarg.default(x = grid, scale = scale, w = x$poster.mean.w, mu = x$poster.mean.mu, Sigma = x$poster.mean.Sigma)
    dns$nx_w <- 1
    dns$lx_w <- ""
    ...
    for (ixw in 1:x$nx_w){     
      ...
      tmp <- NMixPlugDensMarg.default(x = gridixw, scale = scale, w = x$poster.mean.w[(ixw - 1)*x$K[1] + 1:x$K[1]], mu = x$poster.mean.mu, Sigma = x$poster.mean.Sigma)
      if (ixw == 1){
        dns$dens <- tmp$dens
      }else{
