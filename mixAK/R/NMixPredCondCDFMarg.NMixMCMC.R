...
##  CREATED:   06/05/2010
##
##  FUNCTIONS: NMixPredCondCDFMarg.NMixMCMC (06/05/2010)
##             
## ======================================================================
...
## *************************************************************
NMixPredCondCDFMarg.NMixMCMC <- function(x, icond, prob, grid, lgrid=50, scaled=FALSE, ...)
{
  ...
  if (x$prior$priorK == "fixed"){  
    return(NMixPredCondCDFMarg.default(x=grid, icond=icond, prob=prob, scale=scale, K=x$K, w=as.numeric(t(x$w)), mu=as.numeric(t(x$mu)), Li=as.numeric(t(x$Li)), Krandom=FALSE))
  }else{
    return(NMixPredCondCDFMarg.default(x=grid, icond=icond, prob=prob, scale=scale, K=x$K, w=x$w, mu=x$mu, Li=x$Li, Krandom=TRUE))    
  }  
}  
