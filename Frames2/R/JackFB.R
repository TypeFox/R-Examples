...
#' 
#' @usage JackFB(ysA, ysB, piA, piB, domainsA, domains_B, conf_level, sdA = "srs", 
...
#' @param domains_B A character vector of size \eqn{nB} indicating the domain each unit from \eqn{s_B} belongs to. Possible values are "b" and "ba".
...
#' fcpA = TRUE, fcpB = TRUE)
#' @export
JackFB = function (ysA, ysB, piA, piB, domainsA, domains_B, conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- FB(ysA, ysB, piA, piB, domainsA, domains_B)
	...
	if (sdA == "str"){
		...
		for (i in 1:nstrataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(piA))))
					YcA[k,] <- FB(ysA[-j,], ysB, piA[-j,-j], piB, domainsA[-j], domains_B)[[2]][1,]
				else
					YcA[k,] <- FB(ysA[-j,], ysB, piA[-j], piB, domainsA[-j], domains_B)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdA == "clu"){
			...
			for (i in 1:nclustersA){
				if (!is.null(dim(drop(piA))))
					YcA[i,] <- FB(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]],clusA %in% clustersA[-clustersA[i]]], piB, domainsA[clusA %in% clustersA[-clustersA[i]]], domains_B)[[2]][1,]
				else
					YcA[i,] <- FB(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]]], piB, domainsA[clusA %in% clustersA[-clustersA[i]]], domains_B)[[2]][1,]
			}
			...
			if (sdA == "strclu"){
				...
				for (i in 1:nstrataA){
					...
					for (j in 1:nclustersA){
						...
						if (!is.null(dim(drop(piA))))
							YcA[k,] <- FB(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]],clusA %in% clustersA[-clustersA[j]]], piB, domainsA[clusA %in% clustersA[-clustersA[j]]], domains_B)[[2]][1,]
						else
							YcA[k,] <- FB(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]]], piB, domainsA[clusA %in% clustersA[-clustersA[j]]], domains_B)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nA){
					if (!is.null(dim(drop(piA))))
						YcA[i,] <- FB(ysA[-i,], ysB, piA[-i,-i], piB, domainsA[-i], domains_B)[[2]][1,]
					else
						YcA[i,] <- FB(ysA[-i,], ysB, piA[-i], piB, domainsA[-i], domains_B)[[2]][1,]
				}
	...
	if (sdB == "str"){
		...
		for (i in 1:nstrataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(piB))))
					YcB[k,] <- FB(ysA, ysB[-j,], piA, piB[-j,-j], domainsA, domains_B[-j])[[2]][1,]
				else
					YcB[k,] <- FB(ysA, ysB[-j,], piA, piB[-j], domainsA, domains_B[-j])[[2]][1,]
				k <- k + 1
			}
		...
		if (sdB == "clu"){
			...
			for (i in 1:nclustersB){
				if (!is.null(dim(drop(piB))))
					YcB[i,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]],clusB %in% clustersB[-clustersB[i]]], domainsA, domains_B[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
				else
					YcB[i,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]]], domainsA, domains_B[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
			}
			...
			if(sdB == "strclu"){
				...
				for (i in 1:nstrataB){
					...
					for (j in 1:nclustersB){
						...
						if (!is.null(dim(drop(piB))))
							YcB[k,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]],clusB %in% clustersB[-clustersB[j]]], domainsA, domains_B[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						else
							YcB[k,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]]], domainsA, domains_B[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nB){
					...
					if (!is.null(dim(drop(piB))))
						YcB[i,] <- FB(ysA, ysB[-i,], piA, piB[-i,-i], domainsA, domains_B[-i])[[2]][1,]
					else
						YcB[i,] <- FB(ysA, ysB[-i,], piA, piB[-i], domainsA, domains_B[-i])[[2]][1,]
				}
