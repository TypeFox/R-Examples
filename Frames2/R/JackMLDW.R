#' @name JackMLDW
#' @aliases JackMLDW
#' @title Confidence intervals for MLDW estimator based on jackknife method
#' 
#' @description Calculates confidence intervals for MLDW estimator using jackknife procedure
#' 
#' @usage JackMLDW (ysA, ysB, pik_A, pik_B, domains_A, domains_B, xsA, xsB, x, 
...
#' }
#' @export
JackMLDW = function (ysA, ysB, pik_A, pik_B, domains_A, domains_B, xsA, xsB, x, ind_sam, conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- MLDW(ysA, ysB, pik_A, pik_B, domains_A, domains_B, xsA, xsB, x, ind_sam)[[2]] 
	...
	for (l in 1:c) {
		...
		if (sdA == "str"){
			...
			for (i in 1:nstrataA){
				...
				for (j in (cnhA[i]+1):cnhA[i+1]){
					...
					YcA[k,] <- MLDW(ysA[-j,], ysB, pik_A[-j], pik_B, domains_A[-j], domains_B, xsA[-j,], xsB, x, ind_sam[-j])[[2]][[l]][1,]
					k <- k + 1
				}
			...
			if (sdA == "clu"){
				...
				for (i in 1:nclustersA)
					...
					YcA[i,] <- MLDW(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pik_A[clusA %in% clustersA[-clustersA[i]]], pik_B, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B, xsA[clusA %in% clustersA[-clustersA[i]],], xsB, x, ind_sam[c(clusA %in% clustersA[-clustersA[i]], rep(TRUE, nB))])[[2]][[l]][1,]
				...
				if (sdA == "strclu"){
					...
					for (i in 1:nstrataA){
						...
						for (j in 1:nclustersA){
							...
							YcA[k,] <- MLDW(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pik_A[clusA %in% clustersA[-clustersA[j]]], pik_B, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B, xsA[clusA %in% clustersA[-clustersA[j]],], xsB, x, ind_sam[c(clusA %in% clustersA[-clustersA[j]], rep(TRUE, nrow(ysB)))])[[2]][[l]][1,]
							k <- k + 1
						}
					...
					for (i in 1:nA)
						...
						YcA[i,] <- MLDW(ysA[-i,], ysB, pik_A[-i], pik_B, domains_A[-i], domains_B, xsA[-i,], xsB, x, ind_sam[-i])[[2]][[l]][1,]
		...
		if (sdB == "str"){
			...
			for (i in 1:nstrataB){
				...
				for (j in (cnhB[i]+1):cnhB[i+1]){
					...
					YcB[k,] <- MLDW(ysA, ysB[-j,], pik_A, pik_B[-j], domains_A, domains_B[-j], xsA, xsB[-j,], x, ind_sam[-(nA + j)])[[2]][[l]][1,]
					k <- k + 1
				}
			...
			if (sdB == "clu"){
				...
				for (i in 1:nclustersB)
					...
					YcB[i,] <- MLDW(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pik_A, pik_B[clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]], xsA, xsB[clusB %in% clustersB[-clustersB[i]],], x, ind_sam[c(rep(TRUE, nA), clusB %in% clustersB[-clustersB[i]])])[[2]][[l]][1,]
				...
				if (sdB == "strclu"){
					...
					for (i in 1:nstrataB){
						...
						for (j in 1:nclustersB){
							...
							YcB[k,] <- MLDW(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pik_A, pik_B[clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]], xsA, xsB[clusB %in% clustersB[-clustersB[j]],], x, ind_sam[c(rep(TRUE, nrow(ysA)), clusB %in% clustersB[-clustersB[j]])])[[2]][[l]][1,]
							k <- k + 1
						}
					...
					for (i in 1:nB)
						YcB[i,] <- MLDW(ysA, ysB[-i,], pik_A, pik_B[-i], domains_A, domains_B[-i], xsA, xsB[-i,], x, ind_sam[-(nA + i)])[[2]][[l]][1,]
		...
		VJack_Phat_MLDW <- vjA + vjB

		mat[1,] <- estimation[[l]][1,]
		mat[2,] <- estimation[[l]][1,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Phat_MLDW)
		mat[3,] <- estimation[[l]][1,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Phat_MLDW)
		mat[4,] <- estimation[[l]][2,]
		mat[5,] <- estimation[[l]][2,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(1/N^2 * VJack_Phat_MLDW)
		mat[6,] <- estimation[[l]][2,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(1/N^2 * VJack_Phat_MLDW)
