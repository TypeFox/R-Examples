#' @name JackMLSW
#' @aliases JackMLSW
#' @title Confidence intervals for MLSW estimator based on jackknife method
#' 
#' @description Calculates confidence intervals for MLSW estimator using jackknife procedure
#' 
#' @usage JackMLSW (ysA, ysB, pik_A, pik_B, pik_ab_B, pik_ba_A, domains_A, 
...
#' }
#' @export
JackMLSW = function (ysA, ysB, pik_A, pik_B, pik_ab_B, pik_ba_A, domains_A, domains_B, xsA, xsB, x, ind_sam, conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- MLSW(ysA, ysB, pik_A, pik_B, pik_ab_B, pik_ba_A, domains_A, domains_B, xsA, xsB, x, ind_sam)[[2]] 
	...
	for (l in 1:c) {
		...
		if (sdA == "str"){
			...
			for (i in 1:nstrataA){
				...
				for (j in (cnhA[i]+1):cnhA[i+1]){
					...
					YcA[k,] <- MLSW(ysA[-j,], ysB, pik_A[-j], pik_B, pik_ab_B[-j], pik_ba_A, domains_A[-j], domains_B, xsA[-j,], xsB, x, ind_sam[-j])[[2]][[l]][1,]
					k <- k + 1
				}
			...
			if (sdA == "clu"){
				...
				for (i in 1:nclustersA)
					...
					YcA[i,] <- MLSW(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pik_A[clusA %in% clustersA[-clustersA[i]]], pik_B, pik_ab_B[clusA %in% clustersA[-clustersA[i]]], pik_ba_A, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B, xsA[clusA %in% clustersA[-clustersA[i]],], xsB, x, ind_sam[c(clusA %in% clustersA[-clustersA[i]], rep(TRUE, nB))])[[2]][[l]][1,]
				...
				if (sdA == "strclu"){
					...
					for (i in 1:nstrataA){
						...
						for (j in 1:nclustersA){
							...
							YcA[k,] <- MLSW(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pik_A[clusA %in% clustersA[-clustersA[j]]], pik_B, pik_ab_B[clusA %in% clustersA[-clustersA[j]]], pik_ba_A, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B, xsA[clusA %in% clustersA[-clustersA[j]],], xsB, x, ind_sam[c(clusA %in% clustersA[-clustersA[j]], rep(TRUE, nrow(ysB)))])[[2]][[l]][1,]
							k <- k + 1
						}
					...
					for (i in 1:nA)
						...
						YcA[i,] <- MLSW(ysA[-i,], ysB, pik_A[-i], pik_B, pik_ab_B[-i], pik_ba_A, domains_A[-i], domains_B, xsA[-i,], xsB, x, ind_sam[-i])[[2]][[l]][1,]
		...
		if (sdB == "str"){
			...
			for (i in 1:nstrataB){
				...
				for (j in (cnhB[i]+1):cnhB[i+1]){
					...
					YcB[k,] <- MLSW(ysA, ysB[-j,], pik_A, pik_B[-j], pik_ab_B, pik_ba_A[-j], domains_A, domains_B[-j], xsA, xsB[-j,], x, ind_sam[-(nA + j)])[[2]][[l]][1,]
					k <- k + 1
				}
			...
			if (sdB == "clu"){
				...
				for (i in 1:nclustersB)
					...
					YcB[i,] <- MLSW(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pik_A, pik_B[clusB %in% clustersB[-clustersB[i]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]], xsA, xsB[clusB %in% clustersB[-clustersB[i]],], x, ind_sam[c(rep(TRUE, nA), clusB %in% clustersB[-clustersB[i]])])[[2]][[l]][1,]
				...
				if (sdB == "strclu"){
					...
					for (i in 1:nstrataB){
						...
						for (j in 1:nclustersB){
							...
							YcB[k,] <- MLSW(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pik_A, pik_B[clusB %in% clustersB[-clustersB[j]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]], xsA, xsB[clusB %in% clustersB[-clustersB[j]],], x, ind_sam[c(rep(TRUE, nrow(ysA)), clusB %in% clustersB[-clustersB[j]])])[[2]][[l]][1,]
							k <- k + 1
						}
					...
					for (i in 1:nB)
						YcB[i,] <- MLSW(ysA, ysB[-i,], pik_A, pik_B[-i], pik_ab_B, pik_ba_A[-i], domains_A, domains_B[-i], xsA, xsB[-i,], x, ind_sam[-(nA + i)])[[2]][[l]][1,]
		...
		VJack_Phat_MLSW <- vjA + vjB

		mat[1,] <- estimation[[l]][1,]
		mat[2,] <- estimation[[l]][1,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Phat_MLSW)
		mat[3,] <- estimation[[l]][1,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Phat_MLSW)
		mat[4,] <- estimation[[l]][2,]
		mat[5,] <- estimation[[l]][2,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(1/N^2 * VJack_Phat_MLSW)
		mat[6,] <- estimation[[l]][2,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(1/N^2 * VJack_Phat_MLSW)
