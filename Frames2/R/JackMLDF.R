#' @name JackMLDF
#' @aliases JackMLDF
#' @title Confidence intervals for MLDF estimator based on jackknife method
#' 
#' @description Calculates confidence intervals for MLDF estimator using jackknife procedure
#' 
#' @usage JackMLDF (ysA, ysB, pik_A, pik_B, domains_A, domains_B, xsA, xsB, xA, xB, ind_samA, 
...
#' }
#' @export
JackMLDF = function (ysA, ysB, pik_A, pik_B, domains_A, domains_B, xsA, xsB, xA, xB, ind_samA, ind_samB, ind_domA, ind_domB, N, conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- MLDF(ysA, ysB, pik_A, pik_B, domains_A, domains_B, xsA, xsB, xA, xB, ind_samA, ind_samB, ind_domA, ind_domB, N)[[2]] 
	...
	for (l in 1:c) {
		...
		if (sdA == "str"){
			...
			for (i in 1:nstrataA){
				...
				for (j in (cnhA[i]+1):cnhA[i+1]){
					...
					YcA[k,] <- MLDF(ysA[-j,], ysB, pik_A[-j], pik_B, domains_A[-j], domains_B, xsA[-j,], xsB, xA, xB, ind_samA[-j], ind_samB, ind_domA, ind_domB, N)[[2]][[l]][1,]
					k <- k + 1
				}
			...
			if (sdA == "clu"){
				...
				for (i in 1:nclustersA)
					...
					YcA[i,] <- MLDF(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pik_A[clusA %in% clustersA[-clustersA[i]]], pik_B, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B, xsA[clusA %in% clustersA[-clustersA[i]],], xsB, xA, xB, ind_samA[c(clusA %in% clustersA[-clustersA[i]], rep(TRUE, nB))], ind_samB, ind_domA, ind_domB, N)[[2]][[l]][1,]
				...
				if (sdA == "strclu"){
					...
					for (i in 1:nstrataA){
						...
						for (j in 1:nclustersA){
							...
							YcA[k,] <- MLDF(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pik_A[clusA %in% clustersA[-clustersA[j]]], pik_B, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B, xsA[clusA %in% clustersA[-clustersA[j]],], xsB, xA, xB, ind_samA[c(clusA %in% clustersA[-clustersA[j]], rep(TRUE, nrow(ysB)))], ind_samB, ind_domA, ind_domB, N)[[2]][[l]][1,]
							k <- k + 1
						}
					...
					for (i in 1:nA)
						...
						YcA[i,] <- MLDF(ysA[-i,], ysB, pik_A[-i], pik_B, domains_A[-i], domains_B, xsA[-i,], xsB, xA, xB, ind_samA[-i], ind_samB, ind_domA, ind_domB, N)[[2]][[l]][1,]
		...
		if (sdB == "str"){
			...
			for (i in 1:nstrataB){
				...
				for (j in (cnhB[i]+1):cnhB[i+1]){
					...
					YcB[k,] <- MLDF(ysA, ysB[-j,], pik_A, pik_B[-j], domains_A, domains_B[-j], xsA, xsB[-j,], xA, xB, ind_samA, ind_samB[-j], ind_domA, ind_domB, N)[[2]][[l]][1,]
					k <- k + 1
				}
			...
			if (sdB == "clu"){
				...
				for (i in 1:nclustersB)
					...
					YcB[i,] <- MLDF(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pik_A, pik_B[clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]], xsA, xsB[clusB %in% clustersB[-clustersB[i]],], xA, xB, ind_samA, ind_samB[c(rep(TRUE, nA), clusB %in% clustersB[-clustersB[i]])], ind_domA, ind_domB, N)[[2]][[l]][1,]
				...
				if (sdB == "strclu"){
					...
					for (i in 1:nstrataB){
						...
						for (j in 1:nclustersB){
							...
							YcB[k,] <- MLDF(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pik_A, pik_B[clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]], xsA, xsB[clusB %in% clustersB[-clustersB[j]],], xA, xB, ind_samA, ind_samB[c(rep(TRUE, nrow(ysA)), clusB %in% clustersB[-clustersB[j]])], ind_domA, ind_domB, N)[[2]][[l]][1,]
							k <- k + 1
						}
					...
					for (i in 1:nB)
						YcB[i,] <- MLDF(ysA, ysB[-i,], pik_A, pik_B[-i], domains_A, domains_B[-i], xsA, xsB[-i,], xA, xB, ind_samA, ind_samB[-i], ind_domA, ind_domB, N)[[2]][[l]][1,]
		...
		VJack_Phat_MLDF <- vjA + vjB

		mat[1,] <- estimation[[l]][1,]
		mat[2,] <- estimation[[l]][1,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Phat_MLDF)
		mat[3,] <- estimation[[l]][1,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Phat_MLDF)
		mat[4,] <- estimation[[l]][2,]
		mat[5,] <- estimation[[l]][2,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(1/N^2 * VJack_Phat_MLDF)
		mat[6,] <- estimation[[l]][2,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(1/N^2 * VJack_Phat_MLDF)
