...
#' 
#' @usage JackBKA(ysA, ysB, piA, piB, pik_ab_B, pik_ba_A, domainsA, domainsB, 
...
#' @param pik_ab_B A numeric vector of size \eqn{nA} containing first order inclusion probabilities according to sampling design in frame B for units belonging 
#'  to overlap domain that have been selected in \eqn{s_A}.
#' @param pik_ba_A A numeric vector of size \eqn{nB} containing first order inclusion probabilities according to sampling design in frame A for units belonging 
...
#' @export
JackBKA = function (ysA, ysB, piA, piB, pik_ab_B, pik_ba_A, domainsA, domainsB, conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- BKA(ysA, ysB, piA, piB, pik_ab_B, pik_ba_A, domainsA, domainsB)
	...
	if (sdA == "str"){
		...
		for (i in 1:nstrataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(piA))))
					YcA[k,] <- BKA(ysA[-j,], ysB, piA[-j,-j], piB, pik_ab_B[-j], pik_ba_A, domainsA[-j], domainsB)[[2]][1,]
				else
					YcA[k,] <- BKA(ysA[-j,], ysB, piA[-j], piB, pik_ab_B[-j], pik_ba_A, domainsA[-j], domainsB)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdA == "clu"){
			...
			for (i in 1:nclustersA){
				if (!is.null(dim(drop(piA))))
					YcA[i,] <- BKA(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]],clusA %in% clustersA[-clustersA[i]]], piB, pik_ab_B[clusA %in% clustersA[-clustersA[i]]], pik_ba_A, domainsA[clusA %in% clustersA[-clustersA[i]]], domainsB)[[2]][1,]
				else
					YcA[i,] <- BKA(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]]], piB, pik_ab_B[clusA %in% clustersA[-clustersA[i]]], pik_ba_A, domainsA[clusA %in% clustersA[-clustersA[i]]], domainsB)[[2]][1,]
			}
			...
			if (sdA == "strclu"){
				...
				for (i in 1:nstrataA){
					...
					for (j in 1:nclustersA){
						...
						if (!is.null(dim(drop(piA))))
							YcA[k,] <- BKA(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]],clusA %in% clustersA[-clustersA[j]]], piB, pik_ab_B[clusA %in% clustersA[-clustersA[j]]], pik_ba_A, domainsA[clusA %in% clustersA[-clustersA[j]]], domainsB)[[2]][1,]
						else
							YcA[k,] <- BKA(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]]], piB, pik_ab_B[clusA %in% clustersA[-clustersA[j]]], pik_ba_A, domainsA[clusA %in% clustersA[-clustersA[j]]], domainsB)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nA){
					if (!is.null(dim(drop(piA))))
						YcA[i,] <- BKA(ysA[-i,], ysB, piA[-i,-i], piB, pik_ab_B[-i], pik_ba_A, domainsA[-i], domainsB)[[2]][1,]
					else
						YcA[i,] <- BKA(ysA[-i,], ysB, piA[-i], piB, pik_ab_B[-i], pik_ba_A, domainsA[-i], domainsB)[[2]][1,]
				}
	...
	if (sdB == "str"){
		...
		for (i in 1:nstrataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(piB))))
					YcB[k,] <- BKA(ysA, ysB[-j,], piA, piB[-j,-j], pik_ab_B, pik_ba_A[-j], domainsA, domainsB[-j])[[2]][1,]
				else
					YcB[k,] <- BKA(ysA, ysB[-j,], piA, piB[-j], pik_ab_B, pik_ba_A[-j], domainsA, domainsB[-j])[[2]][1,]
				k <- k + 1
			}
		...
		if (sdB == "clu"){
			...
			for (i in 1:nclustersB){
				if (!is.null(dim(drop(piB))))
					YcB[i,] <- BKA(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]],clusB %in% clustersB[-clustersB[i]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[i]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
				else
					YcB[i,] <- BKA(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[i]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
			}
			...
			if(sdB == "strclu"){
				...
				for (i in 1:nstrataB){
					...
					for (j in 1:nclustersB){
						...
						if (!is.null(dim(drop(piB))))
							YcB[k,] <- BKA(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]],clusB %in% clustersB[-clustersB[j]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[j]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						else
							YcB[k,] <- BKA(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[j]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nB){
					...
					if (!is.null(dim(drop(piB))))
						YcB[i,] <- BKA(ysA, ysB[-i,], piA, piB[-i,-i], pik_ab_B, pik_ba_A[-i], domainsA, domainsB[-i])[[2]][1,]
					else
						YcB[i,] <- BKA(ysA, ysB[-i,], piA, piB[-i], pik_ab_B, pik_ba_A[-i], domainsA, domainsB[-i])[[2]][1,]
				}
