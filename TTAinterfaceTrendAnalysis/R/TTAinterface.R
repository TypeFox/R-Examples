...
#_________________________________________________________________________________________________Creation de la fenetre principale (tt) et des onglets
 Envir$tt <- tktoplevel()                                                                                                     # creation de la page principale
         tktitle(Envir$tt) <- paste("Temporal Trend Analysis interface (v", Envir$pversion, ")", sep="")                      # on donne un nom visible a cette page
         tkwm.geometry(Envir$tt, "1200x700")                                                                                  # taille de la fenetre principale

         Envir$paned.tt <- tk2panedwindow(Envir$tt, orientation = "horizontal")
         Envir$topframe <- tkwidget(Envir$paned.tt,"labelframe", text= "",padx=0,pady=0)                                      # cadre de texte pour afficher les 2 boutons
         tkconfigure(Envir$topframe, borderwidth=0)
         tkpack(Envir$topframe, fill="both",expand=1, side="left")

	if (Envir$sel.theme == "Auto") {
		themes <- tk2theme.list()                                                                                                 # choix automatique du theme graphique de l'interface
		...
		} else if ("radiance" %in% themes) { # This must be Ubuntu
	...
	else{ tk2theme(Envir$sel.theme) }                                                                                           # choix du theme selectionne

         nb <- tk2notebook(Envir$topframe, tabs = c(" 1-Data_managment ", " 2-Parameters_selection ", " 3-TimeSeries_building ", " 4-Diagnostics/TrendAnalyses "), width=550)
         tkpack(nb, fill="both", expand = 1)
         Envir$rawdata <- tk2notetab(nb, " 1-Data_managment ")
         tkconfigure(Envir$rawdata, borderwidth=10)
         Envir$Select <- tk2notetab(nb, " 2-Parameters_selection ")
         tkconfigure(Envir$Select, borderwidth=10)
         Envir$datam <- tk2notetab(nb, " 3-TimeSeries_building ")
         tkconfigure(Envir$datam, borderwidth=10)
         Envir$trend <- tk2notetab(nb, " 4-Diagnostics/TrendAnalyses ")
         tkconfigure(Envir$trend, borderwidth=10)

#Creation de l'icon de l'interface
Envir$ico <- tk2ico.create(file.path(path.package("TTAinterfaceTrendAnalysis"),"aide","iconTTA.ico",fsep=.Platform$file.sep))
tk2ico.set(Envir$tt, Envir$ico)
...
#_______________________________________________________________________________________________________________________________________Onglet Raw Data
 ...
 getfile <- function(inipath) {
     ...
     if (ExampleValue=="1"){ inipath = file.path(path.package("TTAinterfaceTrendAnalysis"),"extdata", fsep=.Platform$file.sep )  }
     else { inipath = "C:/" }
     ...
     Envir$Data <- read.table(Envir$Name, sep="\t", na.strings="", dec=".", header=T, fill=TRUE, check.names=FALSE)       # les donnees sont enregistre dans Data
...
#__________________________________________________________________________________________fin de la boucle d'ouverture du fichier (fin de copie-colle pour les fichiers fixdata.R et selectdirectory.R)
    ...
    open <- tk2button(Envir$openframe,text="Import TXT File", command=getfile , width=15)
    tkgrid(open, column=1, row=1, sticky="w")

    tkgrid(tklabel(Envir$openframe, text="  "), column=2, row=1)

    Example <- tk2checkbutton(Envir$openframe)                                                                                                                                                     # checkbutton pour ouvrir directement l'emplacement du fichier example
...
#_________________________________________________________________________________________ce qui suit est affiche avant que les donnees ne soit importees
    logoRcran <- tclVar()
    ...
    Envir$VersionFrame <- tkwidget(Envir$rawdata,"labelframe", text="", padx=0,pady=0)
    tkconfigure(Envir$VersionFrame, borderwidth=0)
    tkgrid(Envir$VersionFrame, column=1, row=11, sticky="w")
    Text3 <- tklabel(Envir$VersionFrame, text= paste("Need R v3.0.0+ to work properly; Your R version:", getRversion() ))         # texte montrant la version minimum de R
    tkconfigure(Text3, font=tkfont.create(size=7))
    tkgrid(Text3, column=1, row=1, sticky="w")
    if(getRversion() >= "3.0.0") { Rstatus <- tklabel(Envir$VersionFrame, text= "(Good)")
       tkconfigure(Rstatus, font=tkfont.create(size=7), foreground="green4")
       tkgrid(Rstatus, column=2, row=1, sticky="w") }
    else { Rstatus <- tklabel(Envir$VersionFrame, text= "(Need update)")
       tkconfigure(Rstatus, font=tkfont.create(size=7), foreground="firebrick")
       tkgrid(Rstatus, column=2, row=1, sticky="w")}
    ...
    AdviceFrame <- tkwidget(Envir$rawdata,"labelframe",text="Important : how to well import your data in 9 steps",padx=25,pady=5, relief = "groove")
    ...
    About.but <- tk2button(Envir$HelpFrame, image=Envir$imgHelp, text=" About ", compound="right",command=function() { about() } )
    tkgrid(About.but, column=1, row=1, sticky="nw")

#_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _demande de confirmation avant fermeture de l'interface
    tkwm.protocol(Envir$tt, "WM_DELETE_WINDOW" , function ( ) {
                  response <- tkmessageBox (title="Confirm",
                  icon = "question" ,
                  message = paste("Anlysis of", Envir$Name.split, "in progress"),
                  detail = "Do you want to close the interface?" ,
                  type = "yesno" ,
                  parent = Envir$tt)
...
#_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _fin du remplissage de l'interface

#_______________________________________________________________________________________________________________________partie texte (resultats + messages) de l'interface

    Envir$result <- tkwidget(Envir$paned.tt,"labelframe", text= "",padx=0,pady=0)
    tkconfigure(Envir$result, borderwidth=0)
    tkpack(Envir$result, fill="both",expand=TRUE, side="right")

    pw.text <- tk2panedwindow( Envir$result, orientation="vertical")

    textframe1 <- tkwidget(pw.text,"labelframe",text="Messages window", padx=5, pady=5)              # cadre de la fenetre 1 (message)
    tkconfigure(textframe1, font=tkfont.create(size=10, weight="bold"),borderwidth=0)
    textframe2 <- tkwidget(pw.text,"labelframe",text="Results window", padx=5, pady=5)                # cadre de la fenetre 2 (resultats)
    ...
    Envir$txt <- tktext(textframe1,bg="white",font="courier",                                                                         # fenetre de texte 1
	         ...
	         wrap="none",
	         font=tkfont.create(family="courier",size=9), height=7)                                                                # police de remplissage par defaut et hauteur/a la fenetre2
    ...
    tkpack(tk2button(textframe1, text="-Clear-", command=function(){ tkdelete(Envir$txt, "1.0", "end") } ), side="bottom", fill="x")                   # bouton 'efface' etendu a
    ...
    Envir$txt2 <- tktext(textframe2,bg="white",font="courier",
	         ...
	         wrap='none',
	         font=tkfont.create(family="courier",size=9), height=10)
    tkpack(yscr2,side="right",fill="y")
    tkpack(xscr2,side="bottom",fill="x")
    tkpack(Envir$txt2,fill="both",expand=1)
    tkpack(tk2button(textframe2, text="-Clear-", command=function(){ tkdelete(Envir$txt2, "1.0", "end") } ), side="bottom", fill="x")

    tkadd(pw.text, textframe1)
    tkadd(pw.text, textframe2)
    tkpack(pw.text, fill = "both", expand = "yes")

    tkadd(Envir$paned.tt, Envir$topframe)
    tkadd(Envir$paned.tt, Envir$result)
	  tkpack(Envir$paned.tt, fill = "both", expand = "yes")
