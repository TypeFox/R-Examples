...
reduce.G<-function(G) ltriangle(as.matrix(G[1:3,1:3]))
#-------------------
...
RE_mat[,seq(1,2*length(teams),by=2)]<-jreo
RE_mat[,seq(2,2*length(teams),by=2)]<-jred

X_mat <- sparse.model.matrix(y_fixed_effects, J_mat, drop.unused.levels = TRUE)
...
for (it in 1:iter) {
    ...
    G.mat[it, ] <- reduce.G(G)
    R.mat[it,]<-ltriangle(as.matrix(R[1:2,1:2]))
