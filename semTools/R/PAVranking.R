...
if(as.vector(regexpr("/",parceloutput))!=-1){
    ...
    for (i in 1:nAlloc){
      ...
      write.table(Allocations[[i]],paste(parceloutput,'/parcelruns',i,'.dat',sep=''),row.names=FALSE,col.names=TRUE)
...
##Model A estimation
  ...
  for (i in 1:nAlloc){
	...
	if (any(is.na(Param_A[[i]][,5]==TRUE))) ProperSolution_A[[i]] <- 0
	...
	if (ConvergedProper_A[[i]]==0) Param_A[[i]][,4:9] <- matrix(data=NA,nrow(Param_A[[i]]),6)
  ...
  if (nConvergedProper_A==0) stop("All allocations failed to converge and/or yielded improper solutions for Model A and/or B.")
  ...
  for (i in 1:nAlloc){
    ...
    if(i>1){ParSEmn_A <-  rowSums(cbind(ParSEmn_A,Param_A[[i]][,5]),na.rm=TRUE)}
    ...
    if(i>1){Parmn_A[,4:ncol(Parmn_A)] <- rowSums(cbind(Parmn_A[,4:ncol(Parmn_A)],Param_A[[i]][,4:ncol(Parmn_A)]),na.rm=TRUE)}
    ## add together all parameter estimates
    
    if(i>1){Fitmn_A <- rowSums(cbind(Fitmn_A,Fitind_A[[i]]),na.rm=TRUE)}
  ...
  Parsum_A <- cbind(apply(Parsd_A,1,mean,na.rm=TRUE),apply(Parsd_A,1,sd,na.rm=TRUE),apply(Parsd_A,1,max,na.rm=TRUE),apply(Parsd_A,1,min,na.rm=TRUE),apply(Parsd_A,1,max,na.rm=TRUE)-apply(Parsd_A,1,min,na.rm=TRUE), Sigp_A*100)
  ...
  ParSEfn_A <- cbind(ParSEmn_A,apply(ParSE_A,1,mean,na.rm=TRUE),apply(ParSE_A,1,sd,na.rm=TRUE),apply(ParSE_A,1,max,na.rm=TRUE),apply(ParSE_A,1,min,na.rm=TRUE),apply(ParSE_A,1,max,na.rm=TRUE)-apply(ParSE_A,1,min,na.rm=TRUE))
  colnames(ParSEfn_A) <- c("lhs", "op", "rhs", "Avg SE","S.D.","MAX","MIN","Range")
  
  Fitsum_A <- cbind(apply(Fitsd_A,1,mean,na.rm=TRUE),apply(Fitsd_A,1,sd,na.rm=TRUE),apply(Fitsd_A,1,max,na.rm=TRUE),apply(Fitsd_A,1,min,na.rm=TRUE),apply(Fitsd_A,1,max,na.rm=TRUE)-apply(Fitsd_A,1,min,na.rm=TRUE))
  rownames(Fitsum_A) <- c("chisq", "df", "cfi", "tli", "rmsea", "srmr", "logl", "bic", "aic")
  ...
  if (is.na(pChisq_A[[i]])==FALSE & pChisq_A[[i]]<.05) {
  sigChisq_A[[i]] <- 1
  } else sigChisq_A[[i]] <- 0 
  }
  ...
  PerSigChisqCol_A <- c(PerSigChisq_A,"n/a","n/a","n/a","n/a","n/a","n/a","n/a","n/a")
...
##Model B estimation
  ...
  for (i in 1:nAlloc){
	...
	if (ConvergedProper[[i]]==0) Param[[i]] <- matrix(data=NA,nrow(Param[[i]]),ncol(Param[[i]]))
	## make parameter estimates null for nonconverged, improper solutions
  ...
  if (nConvergedProper==0) stop("All allocations failed to converge and/or yielded improper solutions for Model A and/or B.")
  ## stop program if no allocations converge
  ...
  for (i in 1:nAlloc){
    ...
    if(i>1) ParSEmn <- rowSums(cbind(ParSEmn,Param[[i]][,5]),na.rm=TRUE)
    ...
    if(i>1){Parmn[,4:ncol(Parmn)] <- rowSums(cbind(Parmn[,4:ncol(Parmn)],Param[[i]][,4:ncol(Parmn)]),na.rm=TRUE)}
    ## add together all parameter estimates
	
    if(i>1){Fitmn <- rowSums(cbind(Fitmn,Fitind[[i]]),na.rm=TRUE)}
    ## add together all fit indices
  ...
  Parsum <- cbind(apply(Parsd,1,mean,na.rm=TRUE),apply(Parsd,1,sd,na.rm=TRUE),apply(Parsd,1,max,na.rm=TRUE),apply(Parsd,1,min,na.rm=TRUE),apply(Parsd,1,max,na.rm=TRUE)-apply(Parsd,1,min,na.rm=TRUE), Sigp*100)
  ...
  rownames(Fitsum) <- c("chisq", "df", "cfi", "tli", "rmsea", "srmr", "logl", "bic", "aic")
  ## calculate fit S.D., minimum, maximum, range
  ...
  PerSigChisqCol <- c(PerSigChisq,"n/a","n/a","n/a","n/a","n/a","n/a","n/a","n/a")
  ## create list of Chi-square Percent Significant and "n/a" (used for fit summary table)
...
##Model Comparison (everything in this section is new)
  ...
  ## count number of allocations that converged and have proper solutions for both A and B
  ...
  LRTsum <- cbind(AvgLRT_AB,lrtestd_AB[2,3],sd(lrchisq_AB,na.rm=TRUE),max(lrchisq_AB),min(lrchisq_AB),max(lrchisq_AB)-min(lrchisq_AB),Prolrsig_AB)
  ...
  for (i in 1:nAlloc){
  if (ConvergedProper_AB[[i]]!=1) FitDiff_AB[1:9,i] <- 0
  }
  ...
  ## make average fit index difference list composed solely of values where A is preferred over B
  ...
  BICgt10DiffAvg_AB <- (Reduce("+",BICgt10DiffAvgtemp))/nBICDiffGT10_AoverB*(-1)
  ## calc average fit index difference when A is preferred over B
  
  FitDiffAvg_AoverB <- list(BICDiffAvg_AB,AICDiffAvg_AB,RMSEADiffAvg_AB,CFIDiffAvg_AB,TLIDiffAvg_AB,SRMRDiffAvg_AB)
  ...
  ## make average fit index difference list composed solely of values where B is preferred over A
  ...
  FitDiffAvg_BoverA <- list(BICDiffAvg_BA,AICDiffAvg_BA,RMSEADiffAvg_BA,CFIDiffAvg_BA,TLIDiffAvg_BA,SRMRDiffAvg_BA)
  ...
  ### create table of proportions of "A strongly preferred over B" and "B strongly preferred over A"
  
  FitDiff_AoverB <- list(nBIC_AoverB/nConvergedProper_AB*100,nAIC_AoverB/nConvergedProper_AB*100,nRMSEA_AoverB/nConvergedProper_AB*100,nCFI_AoverB/nConvergedProper_AB*100,nTLI_AoverB/nConvergedProper_AB*100,nSRMR_AoverB/nConvergedProper_AB*100)
  ### create list of all proportions of "A preferred over B" 
  FitDiff_BoverA <- list(nBIC_BoverA/nConvergedProper_AB*100,nAIC_BoverA/nConvergedProper_AB*100,nRMSEA_BoverA/nConvergedProper_AB*100,nCFI_BoverA/nConvergedProper_AB*100,nTLI_BoverA/nConvergedProper_AB*100,nSRMR_BoverA/nConvergedProper_AB*100)
  ...
  FitDiffSum_AB <- cbind(FitDiff_AoverB,FitDiffAvg_AoverB,FitDiff_BoverA,FitDiffAvg_BoverA)
  colnames(FitDiffSum_AB) <- c("% A>B","Avg Amount A>B","% B>A","Avg Amount B>A")
  rownames(FitDiffSum_AB) <- c("bic","aic","rmsea","cfi","tli","srmr")
  ## create table showing number of allocations in which A>B and B>A as well as average difference values
  ...
  BIChist <- hist(FitDiff_AB[8,1:nAlloc],ylim=c(0,BIChistMax),xlab="BIC_modA - BIC_modB", main="BIC Diff")
  AIChist <- hist(FitDiff_AB[9,1:nAlloc],ylim=c(0,AIChistMax),xlab="AIC_modA - AIC_modB", main="AIC Diff")
  RMSEAhist <- hist(FitDiff_AB[5,1:nAlloc],ylim=c(0,RMSEAhistMax),xlab="RMSEA_modA - RMSEA_modB", main="RMSEA Diff")
  CFIhist <- hist(FitDiff_AB[3,1:nAlloc],ylim=c(0,CFIhistMax),xlab="CFI_modA - CFI_modB", main="CFI Diff")
  TLIhist <- hist(FitDiff_AB[4,1:nAlloc],ylim=c(0,TLIhistMax),xlab="TLI_modA - TLI_modB", main="TLI Diff")
  ...
  ConvergedProperSum <- rbind(nConverged_A/nAlloc,nConverged/nAlloc,nConverged_AB/nAlloc,nConvergedProper_A/nAlloc,nConvergedProper/nAlloc,nConvergedProper_AB/nAlloc)
  rownames(ConvergedProperSum) <- c("Converged_A","Converged_B","Converged_AB","ConvergedProper_A","ConvergedProper_B","ConvergedProper_AB")
  ...
  Output_AB <- list(round(LRTsum,3),"LRT results are interpretable specifically for nested models",round(FitDiffSum_AB,3),round(FitDiffBICgt10,3),ConvergedProperSum)
  names(Output_AB) <- c('LRT Summary, Model A vs. Model B','Note:', 'Fit Index Differences','Percent of Allocations with |BIC Diff| > 10','Converged and Proper Solutions Summary')
