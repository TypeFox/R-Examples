...
items=paste("q",31:51,sep="")

if(length(which(is.element(items,colnames(X))))<21){
stop("At least one item is missing: items must be named q31 to q51");
break
}

if(length(which(match(items,colnames(X))==sort(match(items,colnames(X)))))<21){
stop("Items must be named q31 to q51 and presented on that order in the dataset");
break
}

if(sum(apply(X[,items],2,is.integer))<21){
