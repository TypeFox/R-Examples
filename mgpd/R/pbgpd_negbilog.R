...
if(!error){
            ...
            Hxy           = -1/c0*log(
            pbvevd(c(x,y), model="negbilog", mar1=c(mux,sigx,gamx), mar2=c(muy,sigy,gamy), alpha=(-a), beta=(-b))/
            pbvevd(cbind(pmin(x,0),pmin(y,0)), model="negbilog", mar1=c(mux,sigx,gamx), mar2=c(muy,sigy,gamy), alpha=(-a), beta=(-b))
            )
