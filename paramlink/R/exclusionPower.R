...
exclusionPower = function(ped_claim, ped_true, ids, alleles, afreq=NULL, known_genotypes=list(), loop_breakers=NULL, Xchrom=F, plot=T) {
   ...
   if(isTRUE(plot) || plot=='plot_only') {
      ...
      widths = ifelse(sapply(c(ped_claim, ped_true), inherits, what='singleton'), 1, 2)
