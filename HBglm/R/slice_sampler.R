...
#unislice.calls <- 0	# Number of calls of the slice sampling function
#unislice.evals <- 0	# Number of density evaluations done in these calls
...
uni.slice <- function (x0, f, ..., w=1, m=0, lower=-Inf, upper=+Inf, gx0=NULL)
{
  g <- function(x) f(x,...)
...
#  unislice.calls <<- unislice.calls + 1

  # Find the log density at the initial point, if not already known.

  if (is.null(gx0)) 
  { #unislice.evals <<- unislice.evals + 1
    gx0 <- g(x0)
  }
