mpPTA <- function(data, column.design, make.columndesign.nominal = TRUE, DESIGN = NULL, make.design.nominal = TRUE, graphs = TRUE)
{	
	main <- deparse(substitute(data))
	DESIGN <- designCheck(data, DESIGN, make.design.nominal)
	
	res <- mpSTATIS(data, column.design, make.columndesign.nominal = make.columndesign.nominal, statis.prepro.option = 'PTA', graphs = FALSE)
