n4meansEB <- function(ICC, varICC=0, delta, from, to, sigma, m, iter=1000, alpha=0.05, power=0.8, two.tailed=TRUE, digits=3, plot=TRUE)
{
if (sigma <=0) 
...
r <- NULL
r$delta <- delta; r$sigma <- sigma; r$ICC <- ICC; r$varICC <- varICC; r$m <- m; 
...
k <- (((ZA + ZB)^2)*2*sigma*(1 + (m -1)*rho))/(m*delta^2);
...
#Summary Method
summary.n4meansEB <- function(object, ...)
{
