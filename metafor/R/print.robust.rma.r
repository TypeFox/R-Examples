    ...
    signif.legend = signif.stars, ...) 
{
    ...
    if (all(x$tcl[1] == x$tcl)) {
        ...
        cat("Outcomes per cluster: ", min(x$tcl), "-", max(x$tcl), 
            " (mean: ", formatC(mean(x$tcl), format = "f", digits = 2), 
            ", median: ", median(x$tcl), ")\n", sep = "")
    }
    cat("\n")
    if (x$p > 1) {
    ...
    if (x$int.only) {
        res.table <- c(estimate = x$b, se = x$se, tval = x$tval, 
        ...
        res.table <- cbind(estimate = x$b, se = x$se, tval = x$tval, 
            pval = x$pval, ci.lb = x$ci.lb, ci.ub = x$ci.ub)
        colnames(res.table) <- c("estimate", "se", "tval", "pval", 
            "ci.lb", "ci.ub")
        signif <- symnum(x$pval, corr = FALSE, na = FALSE, cutpoints = c(0, 
            0.001, 0.01, 0.05, 0.1, 1), symbols = c("***", "**", 
            "*", ".", " "))
