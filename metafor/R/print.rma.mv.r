    ...
    signif.legend = signif.stars, ...) 
{
    ...
    if (!x$withS && !x$withG && !x$withH) {
        ...
        if (x$withS) {
            vc <- cbind(estim = sigma2, sqrt = sigma, nlvls = x$s.nlevels, 
                ...
                factor = x$s.names, R = ifelse(x$Rfix, "yes", 
                  "no"))
            colnames(vc) <- c("estim", "sqrt", "nlvls", "fixed", 
                "factor", "R")
            if (!x$withR) 
                vc <- vc[, -6, drop = FALSE]
        ...
        if (x$withH) {
            ...
            cat("outer factor: ", paste0(x$h.names[2], paste(rep(" ", 
                max(0, mng - nchar(x$h.names[2]))), collapse = ""), 
                collapse = ""), " (nlvls = ", x$h.nlevels[2], 
                ")\n", sep = "")
            cat("inner factor: ", paste0(x$h.names[1], paste(rep(" ", 
                max(0, mng - nchar(x$h.names[1]))), collapse = ""), 
                collapse = ""), " (nlvls = ", x$h.nlevels.f[1], 
                ")\n", sep = "")
            cat("\n")
            if (is.element(x$struct[2], c("CS", "AR"))) {
                vc <- cbind(gamma2, gamma, ifelse(x$vc.fix$gamma2, 
                  "yes", "no"))
                vc <- rbind(vc, c(phi, "", ifelse(x$vc.fix$phi, 
                  "yes", "no")))
                colnames(vc) <- c("estim", "sqrt", "fixed")
            ...
            if (is.element(x$struct[2], c("HCS", "HAR"))) {
                vc <- cbind(gamma2, gamma, x$h.levels.k, ifelse(x$vc.fix$gamma2, 
                  "yes", "no"), x$h.levels.f[[1]])
                vc <- rbind(vc, c(phi, "", "", ifelse(x$vc.fix$phi, 
                  "yes", "no"), ""))
                colnames(vc) <- c("estim", "sqrt", "k.lvl", "fixed", 
                  "level")
            ...
            if (is.element(x$struct[2], c("UN", "UNHO"))) {
                if (x$struct[2] == "UN") {
                  ...
                  vc <- cbind(rep(gamma2, length(x$h.levels.k)), 
                    rep(gamma, length(x$h.levels.k)), x$h.levels.k, 
                    ifelse(rep(x$vc.fix$gamma2, length(x$h.levels.k)), 
                      "yes", "no"), x$h.levels.f[[1]])
                }
                colnames(vc) <- c("estim", "sqrt", "k.lvl", "fixed", 
                  "level")
                if (length(x$h.levels.k) == 1) {
                  ...
                  rownames(vc) <- paste("gamma^2.", 1:length(x$h.levels.k), 
                    "  ", sep = "")
                }
                print(vc, quote = FALSE, right = right, print.gap = 2)
                cat("\n")
                if (length(x$phi) == 1) {
                  ...
                  H <- matrix(NA_real_, nrow = x$h.nlevels.f[1], 
                ...
                if (length(x$phi) == 1) {
                  ...
                  H.info <- matrix(NA_real_, nrow = x$h.nlevels.f[1], 
                    ncol = x$h.nlevels.f[1])
                }
                H.info[upper.tri(H.info)] <- x$h.levels.comb.k
                H.info[lower.tri(H.info)] <- t(H.info)[lower.tri(H.info)]
                H.info[upper.tri(H.info)] <- ifelse(x$vc.fix$phi, 
                  "yes", "no")
                diag(H.info) <- "-"
                vc <- cbind(H, "", H.info)
                colnames(vc) <- c(paste("phi.", abbreviate(x$h.levels.f[[1]]), 
                  sep = ""), "", abbreviate(x$h.levels.f[[1]]))
    ...
    if (!is.na(x$QE)) {
        if (x$int.only) {
            ...
            cat("Q(df = ", x$k - x$p, ") = ", formatC(x$QE, digits = digits, 
                format = "f"), ", p-val ", .pval(x$QEp, digits = digits, 
                showeq = TRUE, sep = " "), "\n\n", sep = "")
        }
        else {
            cat("Test for Residual Heterogeneity: \n")
            cat("QE(df = ", x$k - x$p, ") = ", formatC(x$QE, 
                digits = digits, format = "f"), ", p-val ", .pval(x$QEp, 
