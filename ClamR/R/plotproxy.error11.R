...
plotproxy.error11 <- function(x,y,gout, type=1, xlim=NULL, ylim=NULL, ylab="", xlab="", main="" )
{
...
###########  use output of proxyJK to plotall sinusoids
	...
	if(type==1)
	{
		plot(x,y, xlim=xlim, ylim=ylim, ylab=ylab, xlab=xlab, pch=15, col='blue', cex=0.7)
		points(omids,  pmids, col='brown', pch=20)
		lines(omids,  pmids, col='brown', lwd=1)
	...
	if(type==2)
	{
		...
		plot(range(c(x, omids) )  , range(c(y, pmids-delw, pmids+delw)) ,type='n',  xlim=xlim, ylim=ylim, ylab=ylab, xlab=xlab, main=main, pch=15, col='blue',cex=0.7)
		
		
		polygon(c(omids, rev(omids)) , c(pmids-delw, rev(pmids+delw)), col=grey(.8), border=NA)
		
		points(x,y, pch=15, col='blue',cex=0.7)
		
#points(omids,  pmids, col='brown', pch=20)
		lines(omids,  pmids, col='red', lwd=1)
