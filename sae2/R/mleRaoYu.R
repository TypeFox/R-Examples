# mle.Rao.Yu - maximum likelihood estimation of the
# multivariate Rao-Yu model


mle.Rao.Yu <- function(y, X, M, T, NV=1, vcov_e, maxiter=100, iter.tol=.1e-5,  
  ...
  fit <- list(model="T: Rao-Yu, ML", convergence=TRUE)
  ...
  if(!starting.delta) if (NV>1){ delta<- c(sig2_u=sig2_u[1:NV],
      ...
      } else delta<- c(sig2_u=sig2_u[1], sig2_v=sig2_v[1],rho=rho)
