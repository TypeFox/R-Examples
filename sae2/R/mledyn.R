# mle.dyn - maximum likelihood estimation of the multivariate
# dynamic model
# requires DynGamma.R

mle.dyn <- function(y, X, M, T, NV=1, vcov_e, maxiter=100, iter.tol=.1e-5, 
  ...
  fit <- list(model="T: Dynamic, ML", convergence=TRUE)
 ...
 if(!starting.delta) if (NV>1){ delta<- c(sig2_u=sig2_u[1:NV],
     ...
     } else delta<- c(sig2_u=sig2_u[1:NV], sig2_v=sig2_v[1:NV],rho=rho)
 ...
 if(iter.history) {
   ...
   warning.hist<-matrix(0,nrow=2,ncol=maxiter)
 }

# initial fit
 rho <- delta[index.rho]
 rho_u <- delta[(index.rho+1):len.delta]
 Gamma.list <- Gamma_u_f(rho,T)
 Gamma_u <- Gamma.list[[1]]
 d.Gamma_u <- Gamma.list[[2]]
 Gamma.list <- Gamma_v_f(rho,T)
 ...
 for (iter in 1:maxiter) {
     if(iter.history) {
        ...
        llikelihood.hist[iter] <- llike - .5*NV*M*T*log(2*pi)
     }
...
# compute the current information matrix corresponding to delta  
     ...
     V.j.m <- array(0,dim=c(T*NV,T*NV,len.delta))
     for (i in 1:NV) { 
     ...
     V.j[,,index.rho]  <- ((sqrt_u%*%t(sqrt_u)) * u_corr) %x% d.Gamma_u +
     ...
     if(NV>1) {
       for (i in 2:NV) for (j in 1:(i-1)){ 
          ...
          c.m <- matrix(0,nrow=NV,ncol=NV)
          c.m[i,j]<-c.m[j,i]<-1
          V.j[,,index.rho+k]<- ((sqrt_u%*%t(sqrt_u)) * c.m) %x% Gamma_u +
