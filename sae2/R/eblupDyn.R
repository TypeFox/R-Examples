...
eblupDyn <- function(formula, D, T, vardir, method = c("REML","ML"),
 ...
 if (NV == 1) {
    ...
    y <- formuladata[,1]
    if(method=="REML") {
      result <- reml.dyn(y, X, M=D, T=T, NV=1, vcov_e=vardir,
           maxiter=MAXITER, iter.tol = PRECISION, ...) 

    } else {
      result <- mle.dyn(y, X, M=D, T=T, NV=1, vcov_e=vardir,
           maxiter=MAXITER, iter.tol = PRECISION, ...)
    }
 } else {
    depvarlist <- rep(" ", NV)
    ...
    if(method=="REML") {
      result <- reml.dyn(y, X, M=D, T=T, NV=NV, vcov_e=vardir,
         maxiter=MAXITER, iter.tol = PRECISION, ...) 

    } else {
      result <- mle.dyn(y, X, M=D, T=T, NV=NV, vcov_e=vardir,
           maxiter=MAXITER, iter.tol = PRECISION, ...)
    }
