    ...
    fixed = NULL, negative.k = FALSE, rand = 0, rep = 1, ...) 
{
    if (rep > 1) {
        ...
        for (r in 1:rep) {
            fit <- try(sraAutoregHerit(sradata = sradata, active = active, 
    ...
    default.start <- list(mu0 = NA, logith20 = NA, logvarP0 = NA)
    if (negative.k) {
        if (active[1]) {
    ...
    default.fixed <- list(mu0 = 0, logith20 = 0, logvarP0 = 0, 
    ...
    mlewrapper.neg <- function(mu0, logith20, logvarP0, relativekA0, 
        kA1, kA2, kA3, relativekE0, kE1, kE2, kE3) {
        sraMinuslogL(sradata = sradata, FUNtimeseries = sraAutoregHeritTimeseries, 
            mu0 = mu0, logith20 = logith20, logvarP0 = logvarP0, 
            relativekA0 = relativekA0, kA1 = kA1, kA2 = kA2, 
            kA3 = kA3, relativekE0 = relativekE0, kE1 = kE1, 
            kE2 = kE2, kE3 = kE3)
    }
    mlewrapper.noneg <- function(mu0, logith20, logvarP0, relativekA0, 
        logkA1, logkA2, logkA3, relativekE0, logkE1, logkE2, 
        logkE3) {
        sraMinuslogL(sradata = sradata, FUNtimeseries = sraAutoregHeritTimeseries, 
            mu0 = mu0, logith20 = logith20, logvarP0 = logvarP0, 
            relativekA0 = relativekA0, logkA1 = logkA1, logkA2 = logkA2, 
            logkA3 = logkA3, relativekE0 = relativekE0, logkE1 = logkE1, 
            logkE2 = logkE2, logkE3 = logkE3)
    }
    fit <- NULL
    if (negative.k) {
    ...
    return(sraMakeObject(sradata = sradata, model = fit, start = start, 
        fixed = fixed, FUNtimeseries = sraAutoregHeritTimeseries))
}
