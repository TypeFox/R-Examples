#' Marginal Density for Given Scale Parameter and Approximated Uniform Prior for \eqn{\tau}
#' 
#' This function computes the marginal density of \eqn{z_p'\beta} for approximated uniform hyperprior 
#' for \eqn{\tau} 
...
#' @param theta denotes the scale parameter of the approximated uniform hyperprior for \eqn{\tau}.  
...
mdf_aunif <- function(f, theta, Z, Kinv) 
  {
  ...
  for(countf in 1:length(f))
    ...
    integrand <- function(tau2) 
	  {
      dnorm(f[countf], mean = 0, sd = sqrt(tau2 * ztKz)) * dapprox_unif(tau2,scale=theta)
      }
