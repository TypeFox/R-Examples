# wrapper functions for FourierChebyshev.cpp 
# 
# Author: Allison Irvine, Tan Dang
###############################################################################


FCmoments <- function(I=NULL,radius=NULL,theta=NULL,order=NULL) {
	...
	if(is.null(order)) {
		errors = "Missing maximum order and repetition"
		return(list(M,errors));
	}

	#check operating system
	#if(.Platform == "unix") {
	#	dyn.load("cppFiles/FourierChebyshev.so")
	#}else if(.Platform == "windows") {
	#	dyn.load("cppFiles/FourierChebyshev.dll")
	#}
	
	#CHFMbyC(double *I, double *ReM, double *ImM,  double *radius, 
	#		double *theta, int *D1, int *D2, int *Nmax, int *Mmax)
	
	#call C function
	out=.C("CHFMbyC",I=as.double(I),ReM=as.double(mat.or.vec(order[1]+1,order[2]+1)),
...
#reconstruct image
FCrecon <- function(M=NULL,radius=NULL,theta=NULL,order=NULL) {
	...
	#CHFMReconstructbyC(double *IR, double *ReM, double *ImM, double *radius, 
	#		double *theta, int *D1, int *D2, int *Nmax, int *Mmax){
	
	#call C function
	out = .C("CHFMReconstructbyC",IR=as.double(mat.or.vec(d[1],d[2])), ReM=as.double(Re(M[1:(order[1]+1),1:(order[2]+1)])),ImM=as.double(Im(M[1:(order[1]+1),1:(order[2]+1)])),
			radius=as.double(radius),theta=as.double(theta), 
			D=as.integer(d), Nmax=as.integer(order[1]), Mmax=as.integer(order[2]), PACKAGE="IM");
