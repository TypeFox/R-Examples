# wrapper functions for FourierMellin.cpp 
# 
# Author: Allison Irvine, Tan Dang
###############################################################################


FMmoments <- function(I=NULL,radius=NULL,theta=NULL,order=NULL) {
	#FMbyC(double *I, double *ReM, double *ImM,  double *radius, 
	...
	#if(.Platform == "unix") {
	#	dyn.load("cppFiles/FourierMellin.so")
	#}else if(.Platform == "windows") {
	#	dyn.load("cppFiles/FourierMellin.dll")
	#}
	
	#call C function
	out=.C("FMbyC",I=as.double(I),ReM=as.double(mat.or.vec(order[1]+1,order[2]+1)),
...
#reconstruct image
FMrecon <- function(M=NULL,radius=NULL,theta=NULL,order=NULL) {
	...
	#FMReconstructbyC(double *IR, double *ReM, double *ImM, double *radius, double *theta, 
	#int *D1, int *D2, int *Nmax, int *Mmax)

	#call C function
	out = .C("FMReconstructbyC",IR=as.double(mat.or.vec(d[1],d[2])), ReM=as.double(Re(M[1:(order[1]+1),1:(order[2]+1)])),ImM=as.double(Im(M[1:(order[1]+1),1:(order[2]+1)])),
