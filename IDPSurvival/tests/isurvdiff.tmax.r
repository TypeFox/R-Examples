## File IDPSurvival/R/isurvdiff.tmax.r
##
...
.isurvdiff.tmax <- function(formula, data, groups=c(1,2),tmax,...) {
  Call <- match.call()
...
#   if (max(Y[,1]>tmax))  Y[Y[,1]>tmax,1]<-tmax
  
  if (attr(Y, 'type') == "right") {
    temp <- do.call(".isurvdiff.tmax.test",  list(x=X,y=Y,groups=groups,tmax=tmax,...))
  } else {
...
.isurvdiff.tmax.test <- function(x, y, groups=c(1,2), tmax, s=0.25,
                    alternative = c("two.sided", "less", "greater"),
                    exact=NULL,  level = 0.95, display=TRUE, 
                    nsamples=10000, rope=0, ...) {
...
# TEST
  D.sUP <- list(-diff(c(1,survUP[[1]],0)),-diff(c(1,survUP[[2]],0)))
...
#       terms[which(Hrisk==0)]<-1 # avoids 0/0 
      ...
      terms <- (Hrisk-Hevent)/Hrisk
      terms[which(Hrisk==0)]<-1 # avoids 0/0
      SsampUP <- cbind(matrix(1,nsamples,1), t(apply(terms[,-(n.y+1)], 1, cumprod)),matrix(0,nsamples,1))
