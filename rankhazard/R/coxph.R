rankhazardplot.coxph <- function (
  coxphobj, data, select = NULL, refpoints = NULL, 
  ...
  graphsbefore = 0, args.legend = NULL, ...)		
{
  ...
  if (!is.matrix(coxphobj$x) && is.null(x_CI))
    stop("To calculate confidence intevals covariate data must be provided either as argument 'x_CI' or as 'coxphobj$x'.")
  ...
  labelcheck <- !is.element(labels, names(data))
  if (any(labelcheck)) stop (paste("Covariate(s) not in the argument 'data':", paste(paste(labels[labelcheck], collapse = ", "), ".", sep ="")))
