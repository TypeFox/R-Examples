    ...
    quiet = FALSE)
{
    ...
    resList <- list(K = res$nNeiFinal, 
        ...
        y.old1 = matrix(res$datold1, nrow = nrow(dat), ncol = ncol(dat)),
        y.old2 = matrix(res$datold2, nrow = nrow(dat), ncol = ncol(dat)))   
 
    return(resList)
}
 
get_Silhouette <- function(y, mem, disMethod = "Euclidean")
{
    disMethod <- match.arg(disMethod, c("Euclidean", "1-corr"))
    ...
    res <- .Fortran("silhouette", 
        ...
        as.integer(size), 
        as.integer(disMethod2),
        memNei = as.integer(memNei), 
        sIndex = as.double(sIndex), 
        avgs = as.double(avgs), 
        PACKAGE = "clues") 
