attrisk.est <- function(response, stressor, response.levels=c("Poor", "Good"),
...
################################################################################
# Function: attrisk.est
# Purpose: Compute the attributable risk estimate
...
#   explanatory (stressor) variable for an unequal probability design (Van
#   Sickle and Paulsen, 2008).  The attributable risk of the stressor variable
#   is the percent reduction in the first level of the response variable that
#   would result from elimination of the stressor variable.  Cell totals are
#   estimated using the Horvitz-Thompson estimator.  The standard error of the
#   log of the attributable risk estimate and confidence limits for the estimate
#   also are calculated.  The standard error is calculated using a first-order
...
#     the denominator of the attributable risk estimate.  If response.levels is
#     not supplied, then values "Poor" and "Good" are used for the first level
...
#     the attributable risk estimate and the second level is used for
...
# Results:
#   If the function was called by the attrisk.analysis function, then output is
...
#   attrisk.var - calculate values required for estimating variance of the
...
# Calculate the variance-covariance estimate for the cell and marginal totals
         if(cluster.ind) {
            temp <- attrisk.var(response.st, stressor.st, response.levels,
...
# Assign cell weight totals
      ...
      total2 <- wgt.total[response.levels[2], stressor.levels[1]]
      total3 <- wgt.total[response.levels[1], stressor.levels[2]]
      total4 <- wgt.total[response.levels[2], stressor.levels[2]]
...
# Calculate the estimate of attributable risk for all strata combined
      if(total2 == 0 || total4 == 0) {
         ...
         theta <- (popsize.hat*total3)/((total1 + total3)*(total3 + total4))
         ar <- 1 - theta
      }
...
# strata combined
      if(total3 == 0 || (total1 + total3) == 0 || (total3 + total4) == 0) {
         ...
         pder <- numeric(4)
         pder[1] <- 1/popsize.hat - 1/(total1 + total3)
         pder[2] <- 1/popsize.hat
         pder[3]<-  1/popsize.hat + 1/total3 - 1/(total1 + total3) -
                    1/(total3 + total4)
         pder[4]<-  1/popsize.hat - 1/(total3 + total4)
         pder <- 1/c(total1, -total2, -total3, total4)
         arlog.se <- sqrt(t(pder) %*% varest %*% pder)
      }
...
# Calculate the estimate of attributable risk
      if(total3 == 0 & total1 != 0 & total4 != 0) {
         ...
         ar <- NA
         warn.ind <- TRUE
         warn <- paste("Since there are no observations for level \"", response.levels[1], "\" of the response \nvariable, the attributable risk estimate and its standard error cannot be \ncalculated.\n", sep="")
         ...
         ar <- NA
         warn.ind <- TRUE
         warn <- paste("Since there are no observations for level \"", stressor.levels[2], "\" of the stressor \nvariable, the attributable risk estimate and its standard error cannot be \ncalculated.\n", sep="")
         act <- "The attributable risk estimate and its standard error were not calculated.\n"
         warn.df <- rbind(warn.df, data.frame(func=I(fname),
            subpoptype=warn.vec[1], subpop=warn.vec[2], indicator=warn.vec[3],
            stratum=NA, warning=I(warn), action=I(act)))
...
# Calculate the variance-covariance estimate for the cell totals
         if(cluster.ind) {
            temp <- attrisk.var(response, stressor, response.levels,
...
# Calculate confidence limits for the estimate of attributable risk
   if(is.na(arlog.se)) {
      ...
      cl <- c(1 - exp(log(theta) + arlog.se * mult),
              1 - exp(log(theta) - arlog.se * mult))
   }
...
# Create the Results list
   Results <- list(AttRisk=ar, ARlog.se=arlog.se, ConfLimits=cl,
      WeightTotal=popsize.hat, CellCounts=cc, CellProportions=cp )

#
# Depending on whether the function was called directly or was called by
# attrisk.analysis, return appropriate results
#
   if(is.na(warn.vec[1])) {
