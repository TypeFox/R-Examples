catvar.prop <- function(z, wgt, x, y, prop, stratum.ind, stratum.level,
...
#   variance estimator or the local mean variance estimator is calculated, which 
#   is subject to user control.  The SRS variance estimator uses the independent 
...
# Calculate the weighted residuals matrix
         ...
         n <- length(z.lst[[i]])
         im <- tapply(wgt2.lst[[i]], z.lst[[i]]) == matrix(rep(1:m, n), nrow = n, byrow = TRUE)
         rm <- (im - matrix(rep(prop, n), nrow = n, byrow = TRUE)) * matrix(rep(wgt2.lst[[i]], m), nrow = n)
...
# Calculate the variance estimate
      ...
      if(vartype == "Local") {
         ...
         varest <- (pcfactor*apply(total2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, localmean.var, weight.lst) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum)) / tw2
      } else {
         varest <- (pcfactor*ncluster*apply(total2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, var) + apply(var2est * matrix(rep(wgt1.u, m), nrow = ncluster), 2, sum))/ tw2
      }
...
# Calculate the weighted residuals matrix
      ...
      rm <- (im - matrix(rep(prop, n), nrow = n, byrow = TRUE)) * matrix(rep(wgt, m), nrow = n)
