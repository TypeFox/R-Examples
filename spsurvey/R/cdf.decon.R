...
#   error of the estimated CDF and confidence bounds are calculated.  
...
#   user can supply the set of values at which the CDF is estimated.  For the CDF 
#   of a proportion, the Horvitz-Thompson ratio estimator, i.e., the ratio of 
#   two Horvitz-Thompson estimators, is used to calculate the CDF estimate.  For 
#   the CDF of a total, the user can supply the known size of the resource or 
#   the known sum of the size-weights of the resource, as appropriate.  For the 
#   CDF of a total when either the size of the resource or the sum of the size-
#   weights of the resource is provided, the classic ratio estimator is used to 
#   calculate the CDF estimate, where that estimator is the product of the known 
#   value and the Horvitz-Thompson ratio estimator.   For the CDF of a total 
#   when neither the size of the resource nor the sum of the size-weights of the 
#   resource is provided, the Horvitz-Thompson estimator is used to calculate 
#   the CDF estimate.  Variance estimates for the estimated CDF are calculated 
...
#   user control. The local mean variance estimator requires the x-coordinate 
#   and the y-coordinate of each site.  The SRS variance estimator uses the 
...
#   multiplier.  In addition the function uses the estimated CDF to calculate 
#   percentile estimates.  Estimated confidence bounds for the percentile 
#   estimates are calculated.  The user can supply the set of values for which 
#   percentiles estimates are desired.  Optionally, the user can use the default 
#   set of percentiles.  The function can accommodate a stratified sample.  For 
#   a stratified sample, separate estimates and standard errors are calculated
