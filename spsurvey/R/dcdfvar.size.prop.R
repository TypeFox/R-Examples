dcdfvar.size.prop <- function(g, dg, var.sigma, wgt, x, y, cdfest, stratum.ind,
...
################################################################################
# Function: dcdfvar.size.prop
# Programmer: Tom Kincaid
# Date: December 3, 2002
# Last Revised: June 5, 2008
# Description:
#   This function calculates variance estimates of the estimated, deconvoluted, 
#   size-weighted cumulative distribution function (CDF) for the proportion of a
#   discrete resource.  Either the simple random sampling (SRS) variance 
...
# Calculate the variance estimate
      ...
      if(vartype == "Local") {
         ...
         varest <- (pcfactor*ncluster*apply(total2est.g * matrix(rep(wgt1.u, m),
...
# If variance of the estimated measurement error variance is nonzero, add the correction factor to the variance estimate
      ...
      if(!is.null(var.sigma)) {
         ...
         varest <- varest + (var.sigma * (dg.sum^2) / tw2)
      }
...
# Calculate the weighted residuals matrix
      ...
      rm <- (g - matrix(rep(cdfest, n), nrow = n, byrow = TRUE)) * matrix(rep(wgt, m), nrow = n)
...
# Adjust the variance estimator for small sample size
      ...
      if(vartype == "Local" && n < 4) {
         ...
         act <- "The simple random sampling variance estimator was used.\n"
         if(stratum.ind) {
            warn <- paste("There are less than four response values for stratum ", stratum.level, ", \nthe simple random sampling variance estimator was used to calculate variance of \nthe CDF estimate.\n", sep="")
            warn.df <- rbind(warn.df, data.frame(func=I(fname),
               subpoptype=warn.vec[1], subpop=warn.vec[2],
               indicator=warn.vec[3], stratum=I(stratum.level), warning=I(warn),
               action=I(act)))
...
# If variance of the estimated measurement error variance is nonzero, add the
# correction factor to the variance estimate
