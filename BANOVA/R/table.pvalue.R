...
function (coeff_table, row_indices, l1_names, l2_names){
  if (length(l1_names) == 1 && length(l2_names) == 1) coeff_table <- matrix(coeff_table, nrow = 1) # the case that there is only one intercept 
  n_l1_p <- length(l1_names)
  n_l2_p <- length(l2_names)
  ptable <- matrix(NA, nrow =  n_l1_p, ncol = n_l2_p)
  for (i in 1:n_l1_p)
    for (j in 1:n_l2_p){
