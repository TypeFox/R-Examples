...
function (samples_l2_param, l2_parameters, X_names, Z_names, X_assign, Z_assign, samples_cutp_param = array(dim = 0)){
  ...
  result_table <- matrix(NA, nrow = n_p, ncol = 1+1+2+1+1) # the table stores statistics of coefficients 
  rownames(result_table) <- row_names
  colnames(result_table) <- c("mean","SD","Quantile0.025","Quantile0.975","p-value","Signif.codes")
  for (i in 1:n_p){
    result_table[i,1] <- mean(samples_l2_param[,i])
    result_table[i,2] <- sd(samples_l2_param[,i])
    result_table[i,3:4] <- quantile(samples_l2_param[,i],c(0.025,0.975))
  }
  p_values <- pValues(samples_l2_param)
  result_table <- round(result_table, digits = 5)
  result_table[,5] <- round(p_values,4)
  coeff_table <- result_table[, c(1,3,4,5)]
  for (i in 1:n_p){
    if (p_values[i] <= 0.001) {
      result_table[i,6] <- '***'
      if (p_values[i] == 0) result_table[i,5] <- '<2e-16'
    }
  ...
  if (length(samples_cutp_param) != 0){
    ...
    for (i in 1:ncol(samples_cutp_param)){
      cutpresults[i,1] <- round(mean(samples_cutp_param[,i]),6)
      cutpresults[i,2] <- round(sd(samples_cutp_param[,i]),6)
      cutpresults[i,3:4] <- round(quantile(samples_cutp_param[,i],c(0.025,0.975)),6)
    }
  }
  ...
  results$coeff_table <- coeff_table
  if (length(samples_cutp_param) != 0)
