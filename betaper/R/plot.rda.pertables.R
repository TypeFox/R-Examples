...
function (x, pch = 18, ...) 
{
    ...
    factor <- data.frame(RDA1 = paste(rownames(allsites), "RDA1"), 
        RDA2 = paste(rownames(allsites), "RDA2"))
    ...
    text(x$raw$rda.raw, display = "bp", col = "red")
    text(x$raw$rda.raw, display = "sites", col = "blue", cex = 0.8, 
        pos = 2)
}
