ineq.md <-
function(n,dist1,dist2,theta,p0,p1,p2,dist.para.table,zero.approx){
    ...
    MLD   <- entropy(y.sim2,alpha=0)
    Theil <- entropy(y.sim2,alpha=1)
    Gini  <- gini(y.sim)$Gini
    
    list(MLD=MLD,Theil=Theil,Gini=Gini,y=y.sim,y2=y.sim2,zero.replace=zero.repl,stat=s.sim)
  }
