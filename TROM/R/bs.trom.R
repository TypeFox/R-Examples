...
#    sp2_associated_idx <- sapply(2:ncol(sp2_ortholog_z), FUN=function(i) which(sp2_ortholog_z[,i]>z_thre ), simplify=FALSE)
    ...
    write.xlsx(overlap, "between-species TROM scores.xlsx", row.names=TRUE, colNames=TRUE)
  }
  ...
  if (save_overlap_genes == TRUE){
    ...
    write.xlsx(overlap_genes_sp1, "between-species overlapping genes (of species 1) between sample pairs", colNames=TRUE)
    
    ################## output overlap genes of species 2
    overlap_genes_sp2 <- sapply(1:length(sp1_associated_idx), FUN=function(k) 
      sapply(1:length(sp2_associated_idx), FUN=function(j) {
        if (provide == TRUE){
          index <- which(sp1_sp2_orthologs[,1] %in% sp1_interested_ortholog[[k]] & 
                           sp1_sp2_orthologs[,2] %in% sp2_interested_ortholog[[j]])
          temp2 <- sp1_sp2_orthologs[,2][index]
        }else {
          index <- which(sp1_sp2_orthologs[,1] %in% sp1_genes[sp1_associated_idx[[k]]] & 
                           sp1_sp2_orthologs[,2] %in% sp2_genes[sp2_associated_idx[[j]]])
          temp2 <- sp2_genes[index]
        }
        if (length(temp2)==0){temp2 <- ""}
        names(temp2) <- paste(k,"_",j)
        temp2
      }), simplify = FALSE)
    
    overlap_genes_sp2 <- unlist(overlap_genes_sp2, recursive = FALSE)
    names(overlap_genes_sp2) <- sapply(1:length(overlap_genes_sp2), FUN=function(x)
      names(overlap_genes_sp2[[x]][1]))
    
    listlen<-as.vector(unlist(lapply(overlap_genes_sp2,length)))
    mlen<-max(listlen)
    overlap_genes_sp2 <- lapply(overlap_genes_sp2, FUN=function(x,mlen){   
      x<-list(c(as.vector(unlist(x)),rep("",mlen-length(x))))
    },mlen=mlen)
    
    overlap_genes_names2 <- names( overlap_genes_sp2 )
    overlap_genes_sp2 <- unlist(overlap_genes_sp2, recursive = FALSE)
    overlap_genes_sp2 <- data.frame(matrix(unlist(overlap_genes_sp2), ncol = length(sp1_associated_idx)*length(sp2_associated_idx), byrow=FALSE), 
                                    stringsAsFactors=FALSE)
    colnames(overlap_genes_sp2) <- overlap_genes_names2
    
    write.xlsx(overlap_genes_sp2, "between-species overlapping genes (of species 2) between sample pairs", colNames=TRUE)
    ############################## end output
  }
  return(t(overlap))
}
