...
prop=.8,Imass,Jmass,a,b,fr,g,dmu,tauden,inertiapc,plottype="biplot",biptype="row",pos=2,arrow=T,length=0,graphy=T) 
 {
  I <- nrow(Imass)
    J <- nrow(Jmass)
n<-sum(N)
...
if (M > 2) {
        ...
        for (i in 1:I) {
            hlax1.row[i] <- dmu[1, 1] * sqrt(chisq.val/t.inertia)
            hlax2.row[i] <- dmu[2, 2] * sqrt(chisq.val/t.inertia) 
        }
        for (j in 1:J) {
            hlax1.col[j] <- dmu[1, 1] * sqrt((chisq.val/t.inertia) * 
                1/Jmass[j,j])
            hlax2.col[j] <- dmu[2, 2] * sqrt((chisq.val/t.inertia) * 
                1/Jmass[j,j])
        }
    }
...
#plot(fr[, a1], fr[, a2],xlim=picsize, ylim=picsize, 
 ...
 asp=0,  col=rowgrlab[[4]][as.integer(rowgroup[[2]])],cex=cex)
abline(h=0,v=0)
text(fr[, a1], fr[, a2], labels = Inames[[1]],pos=pos,col = cols[1], 
        cex = cex)
...
#----------------------------------------
   eccentricity <- (1 - (dmu[2, 2]/dmu[1, 1])^2)^(1/2)
