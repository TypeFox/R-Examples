glarmaBinomialPearson <- function(y, X, offset = NULL, delta, phiLags, thetaLags,
    ...
    for (time in 1:n) {
        ...
        W.d[, tmpq] <- matrix(c(X[time, ], rep(0, p + q)), ncol = 1) +
                       Z.d[, tmpq]
        if (method  == "NR") {
            W.dd[, , tmpq] <- Z.dd[, , tmpq]
        }
        pt <- exp(W[tmpq])/(1 + exp(W[tmpq]))
        mu[tmpq] <- Ntrial[time] * pt
        e[tmpq] <- (y[time] - mu[tmpq]) / sqrt(Ntrial[time] * pt * (1 - pt))
        e.d[, tmpq] <- ((y[time] - Ntrial[time]) * exp(0.5 * W[tmpq]) /
                       (2 * sqrt(Ntrial[time])) - y[time] * exp((-0.5) *
                       W[tmpq]) / (2 * sqrt(Ntrial[time]))) * W.d[, tmpq]

        if (method  == "NR") {
            e.dd[, , tmpq] <- ((y[time] - Ntrial[time]) * exp(0.5 * W[tmpq]) /
                              (4 * sqrt(Ntrial[time])) + y[time] *
                              exp(-0.5 * W[tmpq])/(4 * sqrt(Ntrial[time]))) *
                              W.d[, tmpq] %o% W.d[, tmpq] +
                              ((y[time] - Ntrial[time]) * exp(0.5 * W[tmpq]) /
                              (2 * sqrt(Ntrial[time])) - y[time] *
                              exp(-0.5 * W[tmpq]) / (2 * sqrt(Ntrial[time]))) *
                              W.dd[, , tmpq]
        }
        ## update likelihood and derivatives.
        ll <- ll + y[time] * W[tmpq] - Ntrial[time] * log(1 + exp(W[tmpq])) +
              log(choose(Ntrial[time], y[time]))

        ll.d <- ll.d + (y[time] - mu[tmpq]) * W.d[, tmpq]
        if (method  == "FS") {
            ll.dd <- ll.dd - Ntrial[time] * pt * (1 - pt) * W.d[, tmpq] %o%
                     W.d[, tmpq]
        }
        if (method  == "NR") {
            ll.dd <- ll.dd + (y[time] - mu[tmpq]) * W.dd[, , tmpq] -
                     Ntrial[time] * pt * (1 - pt) * W.d[, tmpq] %o% W.d[, tmpq]
        }
    }
    ## end loop on time.
