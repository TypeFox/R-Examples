EMpoisson<-
function (y, X, w, beta, tol = 10^-6, max.iter = 1000, verbose = FALSE)
{
    ...
    while (error > tol & iter < max.iter) {
        ...
        lambda <- exp(eta) ##
        dY <- sapply(1:J, function(j) dpois(y, lambda[, j])) ##
        wp <- dY * w
        wp <- wp/rowSums(wp)
        coeffic <- poisson.fit(x = X.expand, y = y.expand, weights = as.vector(wp)) ##
        beta <- vector2matrix(betav = coeffic, variant = variant,
            J = J)
