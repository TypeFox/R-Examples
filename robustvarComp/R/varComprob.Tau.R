...
varComprob.Tau <- function(y, x, V, beta=NULL, gamma=NULL, eta0=NULL, scale=10, control=varComprob.control(), ...) {
...
##BEGIN# Iterations
  ...
  while ((dbeta > control$rel.tol.beta  | dgamma > control$rel.tol.gamma | dscale > control$rel.tol.scale) & iter < control$max.it) {
    ...
    if (control$psi!="rocke") {
      ...
      T1 <- mean(rhostar(m=RR, scale=scale1, cc=control$tuning.psi, psi=control$psi))
      ...
      T1 <- mean(rho.rk2.f(x=(RR/scale1), p=p, alpha=control$arp.psi))
      W <- vcrobweightsrocke(m=RR, scale=scale1, p=p, arp=control$arp.chi)      
    }
    tau <- scale1*T1
    Wdot <- scale1*W/drop(W%*%RR)
    if (control$psi!="rocke")
      Wdot <- Wdot*(rhostar(m=RR, scale=scale1, cc=control$tuning.psi, psi=control$psi) - vcrobweights(m=RR, scale=scale1, cc=control$tuning.psi, psi=control$psi)*RR/scale1)
    else
      Wdot <- Wdot*(rho.rk2.f(x=(RR/scale1), p=p, alpha=control$arp.psi) - vcrobweightsrocke(m=RR, scale=scale1, p=p, arp=control$arp.psi)*RR/scale1)
    if (k > 0) {
      ...
      dbeta <- 0
    gamma1 <- drop(doGammaClassicTaustep(gamma=gamma, resid=rr, scale=scale1, V=V, Tmax=tau+10, control=control))
...
##BEGIN# Eta0
  ...
  if (control$psi=="bisquare")
    eta0 <- doSstep(m=RSR/s0, scale=1, bb=control$bb, cc=control$tuning.chi, tol=control$rel.tol.scale, verbose=(control$trace.lev>2))
  else
...
##BEGIN# VCOV
  if (control$cov) {
    vcov <- VCOV.ClassicTau(beta=beta, gamma=gamma, scale=scale, y=y, x=x, V=V, control=control)
...
##END# VCOV
  ...
  result$control$method <- "Tau"  
  class(result) <- 'varComprob.Tau'
  return(result)
}
