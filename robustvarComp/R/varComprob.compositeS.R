#############################################################
# 
#	varComprob.compositeS function
...
varComprob.compositeS <- function(y, x, V, beta=NULL, gamma=NULL, eta0=NULL, scales=rep(10, p*(p-1)/2), control=varComprob.control(), ...) {
...
##BEGIN# Iterations
  ...
  while ((dbeta > control$rel.tol.beta  | dgamma > control$rel.tol.gamma | dscale > control$rel.tol.scale) & iter < control$max.it) {
    ...
    Wdot <- vcrobweightsdotpw(W=W, RR=RR, scale=scales1)
    if (control$psi=="optimal")
      Wdot <- Wdot*control$tuning.chi^2
    if (k > 0) {
      XX <- xssx(x=x, Sigma=Sigma)
      XY <- xssy(x=x, y=y, Sigma=Sigma)
      beta1 <- drop(doBetastep(Wdot=Wdot, XX=XX, XY=XY))
      dbeta <- max(abs(beta-beta1))
    } else
      dbeta <- 0
    gamma1 <- drop(doGammaCompositeSstep(gamma=gamma, resid=rr, scales=scales1, V=V, control=control))
...
##BEGIN# VCOV
  if (control$cov) {
    vcov <- VCOV.CompositeS(beta=beta, gamma=gamma, scales=scales, y=y, x=x, V=V, control=control)
...
##END# VCOV
  ...
  result$control$method <- "compositeS"  
  class(result) <- 'varComprob.compositeS'
  return(result)
}
