...
summary.itree <- function(object, cp=0, digits=getOption("digits"), file,  ...)
{
    if(!inherits(object, "itree")) stop("Not legitimate itree object")
	...
	#itree adds this check:
	if(!is_onesided){
	    print(x$cptable, digits=digits)
	}else{
		cat("No cptable for this method. \n") 
	}
	...
	# make a replacement for frame$yval2
	if(is_classification){
		#what rpart calls "yval2"
		newyval2 <- cbind(ff$yval,ff[,grep("wt.",colnames(ff))])
		colnames(newyval2)[1] <- "yval"
		newyval2 <- as.matrix(newyval2) #make everything numeric
	}
	#itree: use is_classification here...
    if (!is_classification)
        tprint <- x$functions$summary(ff$yval[rows], ff$dev[rows],
                                      ff$wt[rows], ylevel, digits)
    else
    	#use newyval2
        tprint <- x$functions$summary(newyval2[rows,,drop=FALSE], ff$dev[rows],
                                      ff$wt[rows], ylevel, digits)
