"morphology" <- function(x, operation = c("erode", "dilate"), nt=5)
{
    ...
    if (!inherits(x,"SpatialPixelsDataFrame"))
      stop("should be of class SpatialPixelsDataFrame")
    pfs <- proj4string(x)
    ...
    res<-.C("erodil", as.double(o), as.integer(nr+2), as.integer(nc+2),
            as.integer(nt), as.integer(ope), PACKAGE="adehabitatMA")
    ...
    maa$z <- gr
    gr <- image2Grid(maa)
    gr <- as(gr, "SpatialPixelsDataFrame")
    gridded(gr) <- TRUE
    if (!is.na(pfs))
        proj4string(gr) <- CRS(pfs)
