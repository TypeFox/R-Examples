...
function(x, z, delta, beta0=rep(1, length(as.matrix(z)[1,])), max.loop=250, eps=1e-5, eps.beta=1e-15, print=FALSE){
	...
	beta.new	<-	beta0
	mle			<-	find.increasingMLE(x, w=as.vector(exp(z%*%as.matrix(beta.new))), delta)
	...
	return(list(beta=beta.new, h.range=mle$ranges, h.val=mle$mle, phi=phi.new, H=mle$H))
	}  # find.increasingFULLMLE
