...
for(i in i1:i2)
{
    ...
    class=as.matrix("IEDBarb_mhci",ncol=1,byrow=T);
    ...
    write.table(out,file=paste("IEDBarb_mhci_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".txt",sep=""),sep="\t",row.names=F)
    write.table(metadata,file=paste("IEDBarb_mhci_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],"_metadata.txt",sep=""),sep="\t",row.names=F)
    #save.image(file=paste("IEDBarb_mhci_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".RData",sep=""))
