...
for(i in i1:i2)
{
    ...
    temp4=NULL; for(k in 1:length(temp2[[rv]])){for(l in 1:length(temp2)){temp4=c(temp4, length(intersect(temp2[[l]],temp2[[rv]][k])))}}# to intersect the epitopes between species
    temp5=matrix(temp4, nrow=length(temp2[[rv]]), ncol=length(temp), byrow=T)# to create matrix
    ...
    class=as.matrix("IEDBconsensus_mhci",ncol=1,byrow=T);
    ...
    checka=unlist(lapply(get(temp[rv]),function(x){x@consensus_percentile_rank}))
    ...
    write.table(out,file=paste("IEDBconsensus_mhci_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".txt",sep=""),sep="\t",row.names=F)
    write.table(metadata,file=paste("IEDBconsensus_mhci_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],"_metadata.txt",sep=""),sep="\t",row.names=F)
    #save.image(file=paste("IEDBconsensus_mhci_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".RData",sep=""))
