...
for(i in i1:i2)
{
    ...
    class=as.matrix("iedb_consensus_mhcii",ncol=1,byrow=T);
    ...
    checka=unlist(lapply(get(temp[rv]),function(x){x@comblib_score}))
    ...
    write.table(out,file=paste("iedb_consensus_mhcii_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".txt",sep=""),sep="\t",row.names=F)
    write.table(metadata,file=paste("iedb_consensus_mhcii_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],"_metadata.txt",sep=""),sep="\t",row.names=F)
    #save.image(file=paste("iedb_consensus_mhcii_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".RData",sep=""))
