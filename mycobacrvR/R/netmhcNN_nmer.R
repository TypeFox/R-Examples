...
for(i in i1:i2)
{
    ...
    class=as.matrix("netmhcNN",ncol=1,byrow=T);
    ...
    checka=unlist(lapply(get(temp[rv]),function(x){x@Log_aff}))
    ...
    write.table(out,file=paste("netmhcNN_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".txt",sep=""),sep="\t",row.names=F)
    write.table(metadata,file=paste("netmhcNN_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],"_metadata.txt",sep=""),sep="\t",row.names=F)
   # save.image(file=paste("netmhcNN_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".RData",sep=""))
