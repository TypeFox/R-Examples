...
for(i in i1:i2)
{
    ...
    class=as.matrix("netmhcWT",ncol=1,byrow=T);
    ...
    write.table(out,file=paste("netmhcWT_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".txt",sep=""),sep="\t",row.names=F)
    write.table(metadata,file=paste("netmhcWT_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],"_metadata.txt",sep=""),sep="\t",row.names=F)
    #save.image(file=paste("netmhcWT_",epi_length,"_",strsplit(temp[rv],split="epitopes",fixed=T)[[1]][2],".RData",sep=""))
