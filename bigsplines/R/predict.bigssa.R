predict.bigssa <-
  function(object,newdata=NULL,se.fit=FALSE,include=object$tnames,
    ...
    trmfrm <- terms.formula(newform); et=attr(trmfrm,"factors")
    etdim <- dim(et)
    ...
    if(se.fit | design | smoothMatrix){
      predssa <- list(fit=as.numeric(yhat))
      if(se.fit){predssa <- c(predssa,list(se.fit=pse))}
      if(design){predssa <- c(predssa,list(X=cbind(dps$Kmat,Jmat),ix=c(kidx,jidx)))}
      if(smoothMatrix){predssa <- c(predssa,list(S=tcrossprod(cbind(dps$Kmat,Jmat)%*%object$modelspec$coef.csqrt[c(kidx,jidx),])))}
    } else {
      predssa <- as.numeric(yhat)
    }
    return(predssa)
