svocc.step <-
function (object, model, trace = 1, steps = 1000, criter = c("AIC", "BIC", "cAUC"), test = FALSE, k = 2, 
control, ...)
{
...
#        AIC <- sapply(models, "[[", "AIC")
        ...
        heading <- c("Single visit site-occupancy model\n Stepwise Model Path",
...
#    }
    ...
    while (steps > 0) {
        ...
        if (backward && length(scope$drop)) {
            aod <- drop1.svocc(fit, scope$drop, model,
