drop1.svocc <-
function (object, scope, model, criter = c("AIC", "BIC", "cAUC"), test = FALSE, k = 2, control, ...)
{
    if (missing(control))
    ...
    if (isAUC && test) {
        test <- FALSE
        warning("'test = TRUE' can be used with 'model = \"cAUC\"'")
    }
...
#            nclones(object$results$mle) else 1000
        if (model == "sta") {
                z <- svocc.fit(y, x[, jj, drop = FALSE], xother, 
                    link.sta = object$link$sta, link.det = object$link$det,
                    penalized = object$penalized, auc = isAUC,
                    method=object$method, ...)
            } else {
                z <- svocc.fit(y, xother, x[, jj, drop = FALSE],
                    link.sta = object$link$sta, link.det = object$link$det,
                    penalized = object$penalized, auc = isAUC,
                    method=object$method, ...)
            }
...
#                return(z)
    ...
    if (test)
        dev <- 2 * llik
    if (isAUC)
        auc <- c(AUC(object), auc) # this can be object$auc
    if (criter == "AIC") {
        aic <- -2 * llik + k * dfs
    ...
    if (criter == "cAUC") {
        ## here take complementary AUC (1-AUC=cAUC)
        critpart <- data.frame(cAUC=1-auc, Delta.cAUC=(1-auc) - (1-auc[1]))
    }
    dfs <- dfs[1] - dfs
    dfs[1] <- NA
...
#        check.names = FALSE)
    ...
    if (!is.null(Excl))
        aod[Excl,] <- NA
    mpart <- if (model == "sta") "occurrence" else "detection"
    head <- c("Single visit site-occupancy model",
