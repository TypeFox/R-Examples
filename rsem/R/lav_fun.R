...
representation.LISREL <- function(partable=NULL, target=NULL, 
    ...
    for(g in 1:ngroups) {
        ...
        if(gamma) {
            ov.names <- lavNames(partable, "ov.nox",  group=g)
        } else {
            ov.names <- lavNames(partable, "ov",  group=g)
        }
        nvar <- length(ov.names)
        lv.names   <- lavNames(partable, "lv",  group=g); nfac <- length(lv.names)
        ov.th      <- lavNames(partable, "th",  group=g); nth  <- length(ov.th)
        ov.names.x <- lavNames(partable, "ov.x",group=g); nexo <- length(ov.names.x)
        ov.names.nox <- lavNames(partable, "ov.nox",group=g)
