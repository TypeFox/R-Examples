...
#	probablity calculation for any case
	...
	count = 1
	while(temp[count]<percentil&&count<length(temp)){count= count+1}
	infLimResul = bks[count-1]
	supLimResul =bks[count]
	howMuchLess =percentil-temp[count-1]

	

	result = howMuchLess*(bks[count]- bks[count-1])/x[count]

	result = result+ bks[count-1]
