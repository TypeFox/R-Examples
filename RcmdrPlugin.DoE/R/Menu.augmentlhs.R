...
if (!exists(".stored.designaugmentlhs",where="RcmdrEnv")) 
           assign(".stored.designaugmentlhs", .default.designaugmentlhs,pos="RcmdrEnv")
...
## define called functions
 ...
 onHelpTab1 <- function(){
     ...
     print(help("Menu.augmentlhsTab1"))
 }
 onHelpTab6 <- function(){
     if (GrabFocus() && .Platform$OS.type != "windows") 
            tkgrab.release(topdes2)     
...
storeRcmdr <- function(){
        ...
        class(hilf) <- c("menu.designaugmentlhs","list")
        putRcmdr(".stored.designaugmentlhs",hilf)
}
    ...
    ## seed is not used from previously stored design
     closeDialog(window=topdes2)
        ...
        if (!is.valid.name(name)){
            errorCondition(window=topdes2,recall=Menu.augmentlhs, 
...
onLoad <- function(){
    ...
    putRcmdr("lb", variableListBox(deschoose2, variableList=hilf, title="Choose stored design form"))
        tkgrid(lb$frame)
    onOK <- function() {
        putRcmdr(".stored.designaugmentlhs",get(lb$varlist[as.numeric(tclvalue(tcl(lb$listbox, "curselection")))+1]))
        if ("design" %in% class(getRcmdr(".stored.designaugmentlhs"))) 
            putRcmdr(".stored.designaugmentlhs", design.info(getRcmdr(".stored.designaugmentlhs"))$creator)
        tkfocus(CommanderWindow())
        tkdestroy(topdes2)
...
onReset <- function(){
        assign(".stored.designaugmentlhs",.default.designlhs,pos="RcmdrEnv")
        tkfocus(CommanderWindow())
  tkdestroy(topdes2)
  Menu.augmentlhs()
}
...
putRcmdr("nameVar", tclVar(.stored.designaugmentlhs$nameVar))
...
mVar <- tclVar(.stored.designaugmentlhs$mVar)
mEntry <- tkentry(baseFrame, width="8", textvariable=mVar)
mHint <- ttklabel(baseFrame, text="(positive integer number)", foreground="#888888")

## radio buttons for choosing design type
typeradioFrame <- ttklabelframe(baseFrame, text=gettextRcmdr("Type of lhs ?"))
typerbVariable <- tclVar(.stored.designaugmentlhs$typerbVariable)
optAugmentrb <- tkradiobutton(typeradioFrame,text=gettextRcmdr("optAugment"),variable=typerbVariable, value="optAugment")
optSeededrb <- tkradiobutton(typeradioFrame,text=gettextRcmdr("optSeeded"),variable=typerbVariable, value="optSeeded")
augmentrb <- tkradiobutton(typeradioFrame,text=gettextRcmdr("augment"),variable=typerbVariable, value="augment")
tkgrid(optAugmentrb, sticky="w")  
tkgrid(augmentrb, sticky="w")
tkgrid(optSeededrb, sticky="w")
tkgrid(warnlab <- ttklabel(typeradioFrame, 
     text=gettextRcmdr("Caution: type optSeeded may modify the EXISTING design as well! \nUse in exceptional situations only!")),
     sticky="w",pady=10)
...
etyperbVariable <- tclVar(.stored.designaugmentlhs$etyperbVariable)
...
decimalrbVariable <- tclVar(.stored.designaugmentlhs$decimalrbVariable)
...
dirVar <- tclVar(.stored.designaugmentlhs$dirVar)
...
## export file name
putRcmdr("fileVar", tclVar(.stored.designaugmentlhs$fileVar))
...
replacecbVariable <- tclVar(.stored.designaugmentlhs$cbInitials[8])
...
OKCancelHelp(window=topdes2, helpSubject="Menu.augmentlhs")
