...
function(observ,dam,sire,response,position=NULL,block=NULL,ml=F) {
 ...
 if (is.null(position) && is.null(block)) {  n<- 1
 if (ml == F) {
 ...
 if (ml == T) {
  ...
  if (!is.null(position) && !is.null(block)) { n<- 2
  if (ml == F) {
  ...
  comp<- data.frame(effect= as.data.frame(VarCorr(m))$grp[1:(3+n)],variance= colSums(diag(VarCorr(m)))[1:(3+n)])
