JackGlmer2 <-
function(observ,dam,sire,response,fam_link,position=NULL,block=NULL,quasi=F,size=1,first=NULL) {
...
if (size == 1) {
 if (is.null(position) && is.null(block) && quasi == F) { jack<- matrix(0,ncol=5,nrow=nrow(observ)) }
 if (is.null(position) && is.null(block) && quasi == T) { jack<- matrix(0,ncol=6,nrow=nrow(observ)) }
 if (!is.null(position) && is.null(block) && quasi == F) { jack<- matrix(0,ncol=6,nrow=nrow(observ)) }
 if (!is.null(position) && is.null(block) && quasi == T) { jack<- matrix(0,ncol=7,nrow=nrow(observ)) }
 if (is.null(position) && !is.null(block) && quasi == F) { jack<- matrix(0,ncol=6,nrow=nrow(observ)) }
 if (is.null(position) && !is.null(block) && quasi == T) { jack<- matrix(0,ncol=7,nrow=nrow(observ)) }
 if (!is.null(position) && !is.null(block) && quasi == F) { jack<- matrix(0,ncol=7,nrow=nrow(observ)) }
 if (!is.null(position) && !is.null(block) && quasi == T) { jack<- matrix(0,ncol=8,nrow=nrow(observ)) }
...
if (is.null(position) && is.null(block)) {
  ...
  m<- glmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,") + (1|dispersion)",sep="")),
    family=fam_link,data=observ[-i,])  }
  if(paste(fam_link)[2]== "log") { m_err<- log(1/exp(fixef(m)[1]) + 1) } } #poisson
if (!is.null(position) && is.null(block)) {
  if (quasi == F)  {
  m<- glmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,") + (1|",position,")",sep="")),
    family=fam_link,data=observ[-i,])  }
  if (quasi == T)  {
  observ$dispersion<- as.factor(1:length(observ[,1]))
  m<- glmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,") + (1|",position,") + (1|dispersion)",sep="")),
    family=fam_link,data=observ[-i,])  }
  if(paste(fam_link)[2]== "log") { m_err<- log(1/exp(fixef(m)[1]) + 1) } } #poisson
if (is.null(position) && !is.null(block)) {
  if (quasi == F)  {
  m<- glmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,") + (1|",block,")",sep="")),
    family=fam_link,data=observ[-i,])  }
  if (quasi == T)  {
  observ$dispersion<- as.factor(1:length(observ[,1]))
  m<- glmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,") + (1|",block,") + (1|dispersion)",sep="")),
    family=fam_link,data=observ[-i,])  }
  if(paste(fam_link)[2]== "log") { m_err<- log(1/exp(fixef(m)[1]) + 1) } } #poisson
if (!is.null(position) && !is.null(block)) {
  if (quasi == F)  {
  m<- glmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,") + (1|",position,") + (1|",block,")",sep="")),
    family=fam_link,data=observ[-i,])  }
  if (quasi == T)  {
  observ$dispersion<- as.factor(1:length(observ[,1]))
  m<- glmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,") + (1|",position,") + (1|",block,") + (1|dispersion)",sep="")),
    family=fam_link,data=observ[-i,])  }
  if(paste(fam_link)[2]== "log") { m_err<- log(1/exp(fixef(m)[1]) + 1) } } #poisson
...
if (size > 1) {
 ...
 if (is.null(position) && is.null(block) && quasi == F) { jack<- matrix(0,ncol=5,nrow=n_mod) }
 if (is.null(position) && is.null(block) && quasi == T) { jack<- matrix(0,ncol=6,nrow=n_mod) }
 if (!is.null(position) && is.null(block) && quasi == F) { jack<- matrix(0,ncol=6,nrow=n_mod) }
 if (!is.null(position) && is.null(block) && quasi == T) { jack<- matrix(0,ncol=7,nrow=n_mod) }
 if (is.null(position) && !is.null(block) && quasi == F) { jack<- matrix(0,ncol=6,nrow=n_mod) }
 if (is.null(position) && !is.null(block) && quasi == T) { jack<- matrix(0,ncol=7,nrow=n_mod) }
 if (!is.null(position) && !is.null(block) && quasi == F) { jack<- matrix(0,ncol=7,nrow=n_mod) }
 if (!is.null(position) && !is.null(block) && quasi == T) { jack<- matrix(0,ncol=8,nrow=n_mod) }
