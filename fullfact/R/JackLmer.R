JackLmer <-
function(observ,dam,sire,response,ml=F,size=1,first=NULL) {
...
if (size == 1) {
 jack<- matrix(0,ncol=5,nrow=nrow(observ)) #dam,sire,inter,residual,tot
...
if (size > 1) {
 ...
 jack<- matrix(0,ncol=5,nrow=n_mod)   #dam,sire,inter,residual,tot
...
if (ml == F) {
  m<- lmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,")",sep="")),data=observS2[-c(beg[i]:finn[i]),]) }
if (ml == T) {
  m<- lmer(formula= noquote(paste(response,"~ (1|",dam,") + (1|",sire,") + (1|",dam,":",sire,")",sep="")),data=observS2[-c(beg[i]:finn[i]),],REML=F) }
  tot<- sum(colSums(diag(VarCorr(m))),attr(VarCorr(m),"sc")^2)
  jack[i,]<- c(colSums(diag(VarCorr(m))),attr(VarCorr(m),"sc")^2,tot)
  col_names<- c(as.data.frame(VarCorr(m))$grp,"Total")  } #end loop
}  #end block Jack
