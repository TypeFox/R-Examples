...
ssize.oneSamp<-function(delta,sigma,fdr=0.05,power=0.8,pi0=0.95,maxN=35,side="two-sided",cex.title=1.15,cex.legend=1){
	ds<-delta/sigma
	N<-maxN
	a<-fdr
	...
	for(i in 1:length(pi0)){
		...
		for(n in 2:N){
			ni<-n
			t<-ds/sqrt(1/ni)
			ci<-try(optimize(f=TSfun2,interval=c(0,up),a=a,p=p,n=n,t=t)$min)
 			up<-ci
