           ...
           name.save = NULL)
{
    ...
    else
    {
      ...
      if (is.logical(row.names)) {
        if (isTRUE(row.names)) {
          ...
          if (is.null(row.names)) row.names = paste0("X", 1:p)
        }
        else
          row.names = rep("  ", p)
      }
      ...
      if (is.logical(col.names)) {
        if (isTRUE(col.names)) {
          ...
          if (is.null(col.names)) col.names = paste0("Y", 1:q)
        }
        else
          col.names = rep("  ", q)
      }
    ...
    if(class.object[1] %in% object.blocks)
    {
      ...
      if (is.vector(shape.node)) {
        if (length(shape.node) == 1) shape.node = rep(shape.node, length(blocks))
      }
      ...
      if (is.null(names(shape.node))) names(shape.node) = blocks
