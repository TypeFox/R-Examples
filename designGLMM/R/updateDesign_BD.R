...
updateDesign_BD <- function(des,ntmt,blksz,sigb=0,sige=0,means=c(1,1),probs=c(1)) {  # Generate new design
  ...
  newtmt<-sample(1:ntmt,size=numchange,replace = TRUE) # Choose a new treatment to replace the old
