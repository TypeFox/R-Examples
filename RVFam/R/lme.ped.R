...
lme.ped=function(phenfile,genfile,pedfile,phen,covars=NULL,mafRange=c(0,0.05),chr,snpinfoRdata,sep.ped=",",sep.phe=",",sep.gen=" ",
...
### ********************************************** ###
  ...
  #library(coxme); library(kinship2)
  ...
  pedigree <- read.csv(pedfile)
  kmat     <- 2*makekinship(pedigree$famid, pedigree$id, pedigree$fa, pedigree$mo)
  ...
  if (singleSNP.outfile==F) { 
     ...
     if (is.null(covars)){
        h0 <- try(lmekin(test.dat[,phen] ~ (1|id),varlist=kmat,data=test.dat))     
        for (x in 1:snplist.len) temp.out[x,] <- lme.EC(snp=test.dat[,snplist[x]],phen=phen,test.dat=test.dat,kmat=kmat,chr=chr)
     } else {     
        xcovars <- as.matrix(test.dat[,covars] )
        h0 <- try(lmekin(test.dat[,phen] ~ xcovars+(1|id),varlist=kmat,data=test.dat)) 
        for (x in 1:snplist.len) temp.out[x,] <- lme.EC(snp=test.dat[,snplist[x]],phen=phen,test.dat=test.dat,kmat=kmat,covar=covars,chr=chr)
     }

     if (class(h0)!="try-error") sey <- sqrt(sum(c(h0$vcoef$id*h0$sigma^2,h0$sigma^2))) else stop('try-error in reduced model!') 
  ...
  for (i in 1:length(gene.list)){
      ...
      if (singleSNP.outfile==F) { 
         ...
         scores <- temp.out[temp.out$gene==gene,"beta"]/(temp.out[temp.out$gene==gene,"se"]^2)   
         ...
         af[is.na(af)] <- 0
         Rdata[[i]] <- list(scores=scores,cov=Matrix(icov,sparse=T),n=n,maf=af,sey=sey)
      }  
         ...
         gene.mbweights.out[i,] <- list(gene,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA) 
          ...
          if (is.null(covars)) {
             ...
             if (length(rsnps.in.gene)==1) counts.out <- temp.out[temp.out$Name==rsnps.in.gene,-(1:3)] else {   
                ...
                counts.out <- lme.EC(snp=rsnps.dat$counts,phen=phen,test.dat=rsnps.dat,kmat=kmat,chr=chr)
             }   
             ...
             mbweights.out <- lme.EC(snp=rsnps.dat$mbweights,phen=phen,test.dat=rsnps.dat,kmat=kmat,chr=chr)
             ...
             if (length(rsnps.in.gene)==1) counts.out <- temp.out[temp.out$Name==rsnps.in.gene,-(1:3)] else {   
                rsnps.dat$counts <- rowSums(rsnps.dat[,rsnps.in.gene], na.rm=TRUE)     
                counts.out <- lme.EC(snp=rsnps.dat$counts,phen=phen,test.dat=rsnps.dat,covar=covars,kmat=kmat,chr=chr)
             }   
             gene.counts.out[i,] <- list(gene,
                                    counts.out[4],counts.out[5],counts.out[6],
                                    cmafTotal,cmafUsed,nsnpsTotal,nsnpsUsed,nmiss,
             ...
             for (rsnp in rsnps.in.gene) rsnps.dat[,rsnp] <- rsnps.dat[,rsnp]/(1-rsnps.maf[rsnps.maf$Name==rsnp,"maf_ntotal"])/rsnps.maf[rsnps.maf$Name==rsnp,"maf_ntotal"]
             if (length(rsnps.in.gene)==1) rsnps.dat$mbweights <- rsnps.dat[,rsnps.in.gene] else rsnps.dat$mbweights <- rowSums(rsnps.dat[,rsnps.in.gene],na.rm=TRUE)
             mbweights.out <- lme.EC(snp=rsnps.dat$mbweights,phen=phen,test.dat=rsnps.dat,covar=covars,kmat=kmat,chr=chr)
             gene.mbweights.out[i,] <- list(gene, 
                                       mbweights.out[4],mbweights.out[5],mbweights.out[6],
                                       cmafTotal,cmafUsed,nsnpsTotal,nsnpsUsed,nmiss,
                                       mbweights.out[7], mbweights.out[8])
