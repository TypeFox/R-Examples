crd <-
function(treat, resp, quali=TRUE, mcomp='tukey', sigT=0.05, sigF=0.05) {
...
if(quali==TRUE) {
  ...
  if(mcomp=='tukey'){
     tukey(resp,Trat,tab[[1]][2,1],tab[[1]][2,2],sigT)
                    }
  if(mcomp=='duncan'){
     duncan(resp,Trat,tab[[1]][2,1],tab[[1]][2,2],sigT)            
                    }                   
  if(mcomp=='lsd'){
     lsd(resp,Trat,tab[[1]][2,1],tab[[1]][2,2],sigT)
                    }
  if(mcomp=='lsdb'){
     lsdb(resp,Trat,tab[[1]][2,1],tab[[1]][2,2],sigT)
                    }
  if(mcomp=='sk'){
     scottknott(resp,Trat,tab[[1]][2,1],tab[[1]][2,2],sigT)
                    }
  if(mcomp=='snk'){
     snk(resp,Trat,tab[[1]][2,1],tab[[1]][2,2],sigT)
                    }
  if(mcomp=='ccboot'){
     ccboot(resp,Trat,tab[[1]][2,1],tab[[1]][2,2],sigT)
                    }
        }

else {        
    reg.poly(resp, treat, tab[[1]][2,1], tab[[1]][2,2], tab[[1]][1,1], tab[[1]][1,2])
