...
hystenv$warningcountHysteresis <- 0
    ...
    bootdat <- mapply(geometricbootwrapper, j=1:N, MoreArgs=list(wr1=wr1,wr2=wr2,x.pred=g$pred.x,y.pred=g$pred.y,n=n,cbb=cbb,joint=joint))
    ...
    if (hystenv$warningcountHysteresis > 0) warning("Failed to converge ",hystenv$warningcountHysteresis," times.")
