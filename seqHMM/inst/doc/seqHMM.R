...
## ----plottingsequences, fig.width=5, fig.height=3, dev.args=list(pointsize=10), fig.cap="Stacked sequence plot of annual state distributions in the three-channel \\code{biofam} data. This is the default output of the \\code{ssplot} function.", fig.keep='last', fig.align='center', cache=FALSE, echo = FALSE----
...
## ----MCtoSCdata, fig.width=7, fig.height=3, dev.args=list(pointsize=10), fig.keep='last', fig.align='center', cache=FALSE, fig.cap= "Three-channel \\texttt{biofam3c} data converted into single-channel data."----
...
# Vignette: One thread and less restarts
set.seed(1001)
mhmm_fit <- fit_model(
...
## ----code_plottingHMMbasic, out.width='\\linewidth', fig.height=4.5, dev.args=list(pointsize=10), echo=TRUE, fig.align='center', fig.keep='last', cache = FALSE, eval = TRUE, fig.cap="A default plot of a hidden Markov model."----
plot(hmm_biofam)
