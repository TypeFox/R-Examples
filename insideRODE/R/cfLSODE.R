...
function (model, data, LogParms = TRUE, JAC = FALSE, SEQ = FALSE, 
    rtol = 1e-4, atol = 1e-4, tcrit = NULL,dllname = NULL, hmin = 0, hmax = Inf) 
{
        ...
        for (subj in unique(as.character(Subject))) {
            ...
            if (!is.null(DoseInfo$Tcrit) & max(DoseInfo$Ndose[DoseInfo[, 
                ...
                for (i in 1:length(Info[[subj]])) {
                  ...
                  if (i == 1) {
                    if (is.null(nameType)) {
                      xhat[[i]] <- lsode(Info[[subj]][[as.character(i)]]$Init *
                        BioComb, Time[subj == Subject & Time <= 
                        Info[[subj]][[as.character(i + 1)]]$StartTime], 
                        func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, tcrit = Info[[subj]][[as.character(i + 
                      ...
                      xhat[[i]] <- lsode(Info[[subj]][[as.character(i)]]$Init *
                        BioComb, Time[subj == Subject & Time <= 
                        Info[[subj]][[as.character(i + 1)]]$StartTime & 
                        Type == 1], func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, tcrit = Info[[subj]][[as.character(i + 
                        1)]]$StartTime, parms = eval(parse(text = paste("c(", 
                        sep = "", paste(lsodaparms, collapse = ","), 
                        ")"))), rtol = rtol, atol = atol,
                        hmin = hmin, hmax = hmax)
                    ...
                    if (i != length(Info[[subj]])) {
                      if (is.null(nameType)) {
                        xhat[[i]] <- lsode(xhat[[i - 1]][dim(xhat[[i -
                          1]])[1], -1] + Info[[subj]][[as.character(i)]]$Init * 
                          BioComb, Time[subj == Subject & Time >= 
                          Info[[subj]][[as.character(i)]]$StartTime & 
                          Time <= Info[[subj]][[as.character(i + 
                            1)]]$StartTime], func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, tcrit = Info[[subj]][[as.character(i + 
                          1)]]$StartTime, parms = eval(parse(text = paste("c(", 
                          sep = "", paste(lsodaparms, collapse = ","), 
                          ")"))), rtol = rtol, atol = atol,
                          hmin = hmin, hmax = hmax)
                        ...
                        xhat[[i]] <- lsode(xhat[[i - 1]][dim(xhat[[i -
                          ...
                          Info[[subj]][[as.character(i)]]$StartTime & 
                          Time <= Info[[subj]][[as.character(i + 
                            1)]]$StartTime & Type == 1], func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, 
                      ...
                      if (is.null(nameType)) {
                        xhat[[i]] <- lsode(xhat[[i - 1]][dim(xhat[[i -
                          ...
                          BioComb, Time[subj == Subject & Time >= 
                          Info[[subj]][[as.character(i)]]$StartTime], 
                          func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, parms = eval(parse(text = paste("c(", 
                            sep = "", paste(lsodaparms, collapse = ","), 
                            ")"))), rtol = rtol, atol = atol, hmin = hmin, hmax = hmax)
                      }
                      else {
                        xhat[[i]] <- lsode(xhat[[i - 1]][dim(xhat[[i -
                          ...
                          BioComb, Time[subj == Subject & Time >= 
                          Info[[subj]][[as.character(i)]]$StartTime & 
                          Type == 1], func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, parms = eval(parse(text = paste("c(", 
                          sep = "", paste(lsodaparms, collapse = ","), 
                          ")"))), rtol = rtol, atol = atol, hmin = hmin, hmax = hmax)
                      }
                    }
                ...
                if (is.null(nameType)) {
                  x <- lsode(Info[[subj]][["1"]]$Init * BioComb,
                    Time[subj == Subject], func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, parms = eval(parse(text = paste("c(", 
                      sep = "", paste(lsodaparms, collapse = ","), 
                  ...
                  x <- lsode(Info[[subj]][["1"]]$Init * BioComb,
                    Time[subj == Subject & Type == 1], func="derivs", dllname = dllname,initfunc = "initmod", nout = NoS, 
