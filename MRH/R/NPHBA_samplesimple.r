...
NPHBA_simple = function(Mval, Xfixed, delta, outfilename, prune.indc, burnIn, maxIter, thin, Rmp, a, k, gamma.mp,
...
indices, continue.chain, loopctr.start){
    ...
    if(continue.chain == FALSE){
        write.table(matrix(c('iteration', dnames, paste('beta.', betaNames, sep = ''),
                         ...
                         paste('a', namesHazGroups, sep = '_'), paste('lambda', namesHazGroups, sep = '_')),
                         nrow = 1),
			...
			if(numPHParams > 0){
				outdata = rbind(outdata, 
							matrix(c(loopctr, d.iter, betas, betaholder,
								H00, matrix(Rmp, nrow = 1, byrow = TRUE), a, lambda), nrow = 1))
			} else {
				outdata = rbind(outdata, 
