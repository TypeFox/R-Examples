...
PH_samplesimple = function(Mval, delta, X, Xstdz, outfilename, burnIn, maxIter, thin, k, gamma.mp,
...
continue.chain, loopctr.start){
	...
	if(numParams > 0){
		...
		write.table(matrix(c('iteration', paste('d',1:2^Mval, sep = ''), paste('beta', betaNames, sep = '.'), 
				'H00', paste('Rmp', RmpRowNums, sep = ''), 'a', 'lambda'), nrow = 1),
    ...
    # number of rounding values needed: 2^M (d) + length(betaNames) (optional) + 1 (H00) + 2^M-1 (Rmp) + 1 (a) + 1 (lambda)
    round.values = rep(16, 2^Mval + 1 + 2^Mval-1 + 1 + 1)
	...
	# Test for convergence and plot:
	# betas, H00, Rmps that are sampled.
	assessIndex = c((2^Mval+2):(2^Mval+2+numParams), ((2^Mval+3+numParams):(2^(Mval+1)+numParams+1))[RmpNoPruneIndx])
	...
	while((loopctr <= maxIter & convergence == FALSE & fix.max == FALSE) | 
        ...
        if((loopctr %% thin) == 0 & loopctr >= burnIn){
			...
			if(numParams > 0){
				outdata = rbind(outdata, matrix(c(loopctr, d.iter, beta, H00, Rmp, a, lambda), nrow = 1))
			} else {
				outdata = rbind(outdata, matrix(c(loopctr, d.iter, H00, Rmp, a, lambda), nrow = 1))
			}
		}
	...
	outputdata = list(assessIndex = assessIndex, numberofEstParams = 2 + 1 + numParams + length(RmpNoPruneIndx),
