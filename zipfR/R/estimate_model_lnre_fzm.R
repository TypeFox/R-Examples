                                     ...
                                     debug=FALSE, ...)
{
    ...
    if (model$exact) {
      ...
      if (.diff >= 0) { # if E[V] < V isn't met, don't know how to search for better solution
        ...
        if (.diff < 0) { # can't find suitable lower boundary -> fall back to inexact estimate
          if (debug) warning("Can't find exact estimate for B, since lowest possible value ",
                             "B = ",lower.B," has E[V] - V = ",.diff," < 0")
        }
  ...
  if ("B" %in% param.names) {           
    ...
    warning("parameter B cannot be fixed in fZM estimation, ignoring specified value")
  }
  ...
  compute.cost <- function (P.vector, param.names, model, spc, m.max=15, debug=FALSE)
    {
      ...
      P$B <- max(1, 10 * P$A) # make sure that B is valid (just a dummy, correct value is computed below)
  ...
  if (length(param.names) > 1) {        # multi-dimensional minimization with Nelder-Mead
    result <- optim(param.values, compute.cost, method="Nelder-Mead",
