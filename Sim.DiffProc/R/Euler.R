...
.Euler1D <- function(N =100,M=1,x0=1,t0=0,T=1,Dt,drift,diffusion,
                          type=c("ito","str"),...)
                       {					   
    ...
    x0 <- rep(x0,M)[1:M]	
    X <- .Call("Euler1d", x0, t0, Dt, as.integer(N), as.integer(M), A, S, .GlobalEnv, PACKAGE="Sim.DiffProc")
    name <- "X"
...
.Euler2D <- function(N =100,M=1,x0=2,y0=1,t0=0,T=1,Dt,driftx,diffx,drifty,diffy,
                          type=c("ito","str"),...)
                       {					    
    if (type=="ito"){
    ...
    x0 <- rep(x0,M)[1:M]
    y0 <- rep(y0,M)[1:M]	
    Val <- .Call("Euler2d", x0, y0, t0, Dt, as.integer(N), as.integer(M), Ax, Sx, Ay, Sy, .GlobalEnv, PACKAGE="Sim.DiffProc")
    name <- c("X","Y")
...
.Euler3D <- function(N =100,M=1,x0=2,y0=1,z0=1,t0=0,T=1,Dt,driftx,diffx,drifty,diffy,
    ...
    y0 <- rep(y0,M)[1:M]
    z0 <- rep(z0,M)[1:M]
    Val <- .Call("Euler3d", x0, y0, z0, t0, Dt, as.integer(N), as.integer(M), Ax, Sx, Ay, Sy, Az, Sz, .GlobalEnv, PACKAGE="Sim.DiffProc")
    name <- c("X","Y","Z")
