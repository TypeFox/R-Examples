...
.PredCorr1D <- function(N =100,M=1,x0=2,t0=0,T=1,Dt,alpha=0.5,mu=0.5,
    ...
    x0 <- rep(x0,M)[1:M]	
    X <- .Call("Predcorr1d", x0, t0, Dt, as.integer(N), as.integer(M), alpha, mu, A, S, Sx, .GlobalEnv, PACKAGE="Sim.DiffProc")
    name <- "X"
...
.PredCorr2D <- function(N =100,M=1,x0=2,y0=1,t0=0,T=1,Dt,alpha=0.5,mu=0.5,driftx,
    ...
    x0 <- rep(x0,M)[1:M]
    y0 <- rep(y0,M)[1:M]
    Val <- .Call("Predcorr2d", x0, y0, t0, Dt, as.integer(N), as.integer(M), alpha, mu, Ax, Sx, dSx, Ay, Sy, dSy, .GlobalEnv, PACKAGE="Sim.DiffProc")
    name <- c("X","Y")
...
.PredCorr3D <- function(N =100,M=1,x0=2,y0=1,z0=1,t0=0,T=1,Dt,alpha=0.5,mu=0.5,driftx,diffx,drifty,diffy,
    ...
    y0 <- rep(y0,M)[1:M]
    z0 <- rep(z0,M)[1:M]
    Val <- .Call("Predcorr3d", x0, y0, z0, t0, Dt, as.integer(N), as.integer(M),alpha, mu, Ax, Sx, dSx, Ay, Sy, dSy, Az, Sz, dSz, .GlobalEnv, PACKAGE="Sim.DiffProc")
    name <- c("X","Y","Z")
