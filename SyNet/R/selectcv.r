  selectcv <- function (entryCleav){
      tf <- tktoplevel()
      tkwm.title(tf, "Choose Cleavogram")
      done <- tclVar(0)
      tlb <- tklistbox(tf)
      ...
      for (i in obj) {
          ...
          if (class(xobj)[1]== "cleavogram") tkinsert(tlb, "end", i)
      }
      ...
      tkdelete(entryCleav, 0, "end")
      tkinsert(entryCleav, "end", choix)
      tkdestroy(tf)
  }
