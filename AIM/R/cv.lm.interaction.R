...
for(b in 1:num.replicate)
   {g=sample(rep(1:K.cv,(n+K.cv-1)/K.cv))
    ...
    for(i in 1:K.cv)
       {cva[[i]]=lm.interaction(x[g!=i ,], trt[g!=i],  y[g!=i], nsteps=nsteps, mincut=mincut, backfit=backfit, maxnumcut=maxnumcut, dirp=dirp)
        for(ii in 1:nsteps)
           {aa=index.prediction(cva[[i]]$res[[ii]],x[g==i,])
            fit=lm(y[g==i]~aa*(trt[g==i]))
