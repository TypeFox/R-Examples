...
test.valid <- gl.check.lambda.alt(valid.init[, 1], valid.init[, 2], valid.init[, 3], valid.init[, 4],
param = param)
...
if(maxmin == TRUE) {
test.max <- apply(valid.init, 1, function(x, param)
qgl(1, x[1], x[2], x[3], x[4], param = param), param) >= max(data)
test.min <- apply(valid.init, 1, function(x, param)
qgl(0, x[1], x[2], x[3], x[4], param = param), param) <= min(data)
...
optim.check<-is.notinf(apply(valid.init,1,function(x,data,param) optim.fun3(x,data,param),data,param))
valid.init<-valid.init[optim.check,]
...
optim.result <- lapply(1:nrow(init.sol), function(i, init.sol, optim.fun3, data, param)optim(init.sol[i,  ], optim.fun3, data = data, param = param, control = list(maxit = 200000)), init.sol,optim.fun3, data, param)
