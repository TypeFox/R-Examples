`fun.fit.gl.v4` <-
function (a, b, data, fun, no = 10000, maxmin = TRUE, nclass = nclass.scott, 
    leap = 3, FUN = "runif.sobol") 
{
    e <- a
    d <- b - e
    ...
    optim.check <- is.notinf(apply(valid.init, 1, function(x, 
        data, param) optim.fun7(x, data, param), data, param))
    valid.init <- valid.init[optim.check, ]
    test.ind <- apply(valid.init[, 3:4], 1, function(x, data, 
    ...
    optim.result <- lapply(1:nrow(init.sol), function(i, init.sol, 
        optim.fun7, data, param) optim(init.sol[i, ], optim.fun7, 
        data = data, param = param, control = list(maxit = 2e+05)), 
        init.sol, optim.fun7, data, param)
