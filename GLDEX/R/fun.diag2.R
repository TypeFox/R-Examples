"fun.diag2" <-
function(result, data, no.test = 1000,len=100)
{
...
ks.gof(test[[i]], perc.rs[[i]])$p.value,  test,  perc.rs) > 0.05)
fmkl <- sum(sapply(1:no.test, function(i,  test, mm.fmkl)
ks.gof(test[[i]], mm.fmkl[[i]])$p.value,  test,  mm.fmkl) > 0.05)
star <- sum(sapply(1:no.test, function(i,  test,  star.fmkl)
ks.gof(test[[i]], star.fmkl[[i]])$p.value,  test,  star.fmkl) > 0.05)
