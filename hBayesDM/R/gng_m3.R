...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model ("gng_m3").}
...
#' output <- gng_m3("example", 2000, 1000, 3, 3)
#' 
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        ...
        rho_p = rep(log(inits_fixed[5]), numSubjs)
      )
    }
  } else {
    genInitList <- "random"
