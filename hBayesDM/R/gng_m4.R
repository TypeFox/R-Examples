#' Orthogonalized Go/Nogo Task (Cavanagh et al., 2013)
#' 
#' @description 
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model ("gng_m4").}
...
#' output <- gng_m4("example", 2000, 1000, 3, 3)
#' 
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(
        ...
        b_p      = rep(inits_fixed[3], numSubjs),
        pi_p     = rep(inits_fixed[4], numSubjs),
        rhoRew_p = rep(log(inits_fixed[5]), numSubjs),
        rhoPun_p = rep(log(inits_fixed[6]), numSubjs)
      )
    }
  } else {
    genInitList <- "random"
