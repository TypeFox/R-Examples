...
for(l in 1:L){
EZil<- d1*bRi[,l]*g0[l]*exp(xb0)/dz
EWil<- d2*(bRi[,l]*g0[l]-bLi[,l]*g0[l])*exp(xb0)/dw
g1[l]<-sum(EZil+(d2+d3)*EWil)/sum(((d1+d2)*bRi[,l]+d3*bLi[,l])*exp(xb1))
}
return(g1)
}
...
EWil<- d2*(bRi[,l]*g0[l]-bLi[,l]*g0[l])*exp(xb0)/dw
g1[l]<-sum(EZil+(d2+d3)*EWil)/sum(((d1+d2)*bRi[,l]+d3*bLi[,l])*exp(xb1))
p2<-p2+sum( EZil*log(g1[l]) + EWil*log(g1[l]) )
}
...
ll<- sum(log(1-exp(-GRi[d1==1]*exp(xb[d1==1]))))+sum(log(exp(-GLi[d2==1]*exp(xb[d2==1]))-exp(-GRi[d2==1]*exp(xb[d2==1]))))+ sum(log(exp(-GLi[d3==1]*exp(xb[d3==1]))))
