computez.prodjlogj <-
function(xmat,beta,nu,max){
# This code computes the (in)finite sum (from i=0 through to i=max) for the terms jlog(j!)*lambda^j/((j!)^nu)
...
# Compute the terms used to sum for the (in)finite summation
   ...
   for (j in 1:max){
     ...
     for (k in 1:length(lambda)){forans[k,j+1] <- j*log(factorial(j))*prod(temp[k,])}  
     }
