...
setClass(Class = "texreg", 
         representation = representation(
           ...
           isRand = "logical"         #tests for random or fixed effects
         ), 
...
createTexreg <- function(coef.names, coef, se = numeric(0), pvalues = numeric(0), 
                         ci.low = numeric(0), ci.up = numeric(0), 
  ...
  new("texreg", coef.names = coef.names, coef = coef, se = se, 
      ...
      gof = gof, gof.decimal = gof.decimal, isRand = isRand)
}
...
setMethod(f = "show", signature = "texreg", definition = function(object) {
  ...
  if(object@isRand)
    colnames(coefBlock) <- c("Chi square statistics", "p-values")
  else
    colnames(coefBlock) <- c("F-statistics", "p-values")
  #     } else {
