...
sffsBinary1 <- function(profile_data, sens, sp = 1, max_k = 2, loo = TRUE, verbosity = FALSE) {
    # sffs for modified binary timma model
    ...
    while (change) {
        ...
        while (sum(k_set) <= max_k) {
            ...
            timma_temp <- timmaBinary1(profile_temp, sens, loo)
            ...
            for (i in 1:kinase_num) {
                if (k_set[i] == 0) {
                  ...
                  err[i] <- 0
                  err1 <- timmaSearchBinary1(profile_data[, i], space, sens, loo)
                  err[i] <- sum(err1)
                }
            }
            ...
            while (sum(k_set) > 2) {
                ...
                for (i in 1:kinase_num) {
                  ...
                  if (k_set[i] == 1) {
                    ...
                    timma_temp <- timmaBinary1(profile_data[, which(k_temp == 1)], sens, loo)
                    err[i] <- sum(timma_temp$error)
