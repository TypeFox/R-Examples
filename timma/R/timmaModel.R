...
timmaModel <- function(drug_target_profile, sens, loo = TRUE) {
    ...
    # get drug number
    drug_number <- nrow(drug_target_profile)
    # get target number
    target_number <- ncol(drug_target_profile)
    ...
    for (i in 1:drug_number) {
        ...
        temp_matrix <- IM_d[, , i]
        temp_matrix[drug_index[i]] <- 1 * sens[i]
        IM_d[, , i] <- temp_matrix
    ...
    if (loo == FALSE) {
        ...
        for (i in 1:drug_number) {
            ...
            if (length(j_max) != 0 && length(j_min) == 0) {
                ...
                pred[i] <- max_superset_loo[drug_index[i]]
                ...
                pred[i] <- min_subset_loo[drug_index[i]]
