...
#' 
#' @param profile_data the drug-target interaction data. See \code{\link{timma}}.
...
timmaCategoryWeighted1 <- function(profile_data, sens, loo = TRUE, class) {
    # parameter 1: profile_data, drug with selected target profile parameter 2: sens, the actual
    ...
    for (i in 1:drug_number) {
        # get the decimal dec<-strtoi(paste(profile_data[i,],collapse=''),base=2)
        IM_d[i, identical_idx[i]] <- 1 * sens[i]
        # temp_matrix<-IM_d[,,i] temp_matrix[drug_index[i]]<-1*sens[i] IM_d[,,i]<-temp_matrix
        
        # get the binary set: superset and subset bin_set<-binary_set(profile_data[i,])
        # bin_set<-new_bin1(profile_data[i,],profile_data)
        
        bin_set <- getBinary1(profile_data[i, ], profile_data)
        # bin_set<-binary_set_cate(profile_data[i,],6)
