...
#' results<-sffsCategoryWeighted1(tyner_interaction_multiclass, tyner_sensitivity[, 1], class = 6)
#' }

sffsCategoryWeighted1 <- function(profile_data, sens, sp = 1, max_k = 2, loo = TRUE, class, verbosity = FALSE) {
    ...
    while (change) {
        ...
        while (sum(k_set) <= max_k) {
            ...
            timma_temp <- timmaCategoryWeighted1(profile_temp, sens, loo, class)
            ...
            for (i in 1:kinase_num) {
                if (k_set[i] == 0) {
                  ...
                  set_add_one <- k_set
                  set_add_one[i] <- 1
                  err1 <- timmaCategoryWeighted1(profile_data[, which(set_add_one == 1)], sens, loo, class)$error
            ...
            while (sum(k_set) > 2) {
                ...
                for (i in 1:kinase_num) {
                  ...
                  if (k_set[i] == 1) {
                    ...
                    timma_temp <- timmaCategoryWeighted1(profile_data[, which(k_temp == 1)], sens, loo, class)
                    err[i] <- mean(timma_temp$error)
